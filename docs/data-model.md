...
| 成本預估 | `cost_estimate` | | | | 小數（decimal 12,2） | 印務主管填入；觸發「評估完成」時必填 | 歷史版本透過報價紀錄追蹤 |
| 報價單價（未稅） | `price_per_unit` | | | | 小數（decimal 10,2） | 業務填寫；系統計算毛利率 | |
| 毛利率 | `profit_margin` | | | ✓ | 小數（decimal 5,2） | = (報價單價 - 成本預估) / 報價單價 × 100% | 系統自動計算，唯讀 |
| 費用（未稅） | `amount_excl_tax` | | | | 小數（decimal 12,2） | 業務可手動填寫；若未填寫則自動計算 = 單價 × 數量 | 可編輯（手動覆寫）或自動計算 |
