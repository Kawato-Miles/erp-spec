...
| **新增 / 修改 User Story** | 「寫 user story」「新增故事」「補 user story」「寫 [模組] 故事」「推 user story 到 Notion」/ 識別到 spec 需求須具體化為業務故事 | 觸發 `erp-user-story` skill（單階段：純業務情境 + Gherkin 成功條件含守衛型，2026-06-01 自兩階段收斂）：mode A 新增（INVEST 自審 + 中英夾雜 lint + 全卡禁 UI 措辭） / mode B 推送 Notion（單向、以 us-id 為鍵）；產 `13-user-stories/<module>/US-XX-NNN-<簡述>.md`；UI 點擊驗收歸 test case（業務級）+ Prototype e2e |
| **新增 / 修改 Test Case** | 「寫 test case」「建立測試案例」「補 test case」「推 test case 到 Notion」 | 觸發 `erp-test-case` skill：Vault 索引卡（`15-test-cases/<module>/TC-XX-NNN`，source↑user-story）+ Notion 正文；驗收種子取自 user-story Gherkin 成功條件（正向→happy、守衛→edge）；步驟業務動作、UI 點擊歸 Prototype e2e |
| **對外發布 / 迭代同步**（依這次更新同步 Notion / Linear）| 「依這次更新同步」「推迭代差異」「同步發布」「更新外部資料」「推 X 到 Notion / Linear」 | 依 [[iteration-delta-publish]]：算 delta（只取 archived change、active 不外露）→ 內部正本先到位 → 路由（USDB / 資料欄位 → `erp-user-story` mode B / sync-workflow 流程 1-C + notion-publish-rubric 閘門；Linear → `linear-delivery`，中台 vs 業務平台 project 分流）→ 每面寫入前列清單給 Miles → 強制回填追蹤 |
...
- **wiki（BRD 層，memory/Sens_wiki/wiki/）= 商業推理單一正本**：商業目標 / 痛點 / 營運原則 / 業務規則與恆定約束（invariant）/ 角色分權動機 / 領域知識 / KPI / OQ。回答 why 與「什麼必須成立」。
- **OpenSpec（PRD 層，openspec/）= 系統行為規格單一正本**：功能 Requirement / 實體欄位（Data Model）/ 狀態列舉與轉換 / 角色權限規格 / 驗收 Scenario / User Story / change 工作流。回答 how，驅動 Prototype 與 Notion 發布版。
- **單一正本鐵則（同一內容只有一個家）**：系統設計細節（欄位 / 狀態列舉 / 權限 / 驗收）只寫在 OpenSpec；商業推理（規則為什麼對 / invariant / 分權動機）只寫在 wiki。wiki 卡 MUST NOT 複寫 OpenSpec 已有的欄位 / 狀態 / 權限清單，只寫業務意義並以 implemented-by 連下去。正本歸屬非一律 OpenSpec：角色 R&R 與業務規則的正本是 wiki，欄位 / 狀態列舉的正本是 OpenSpec。
- **引用方向**：下游引上游（OpenSpec change 的 Why 段以相對連結引 wiki 卡）；上游 wiki 的 source 禁指 openspec/specs/（違者報錯）。
...
| 使用者故事 / 業務情境 | Vault `13-user-stories/<module>/`（**User Story 內部正本，2026-05-21 新增；單階段 + Gherkin 成功條件，2026-06-01**）+ Vault `07-scenarios/README.md`（跨模組端到端情境）| 各模組 spec § Scenarios（Acceptance Scenarios，Given/When/Then 工程驗收）、Vault `03-roles/` |
| 驗收項目 / Test Case | Vault `15-test-cases/<module>/`（**索引卡內部正本，2026-05-31 新增**，source↑user-story；正文存 Notion ERP Test Case DB）+ 觸發 `erp-test-case` skill | Notion ERP Test Case DB（正文發布版）：https://www.notion.so/2b93886511fa817fbd65e7608726f036 |
...
→ 完整 ERP 資源清單見 [resource-index](memory/shared/resource-index.md) § ERP 資源

### 共用資源

→ 完整共用資源清單見 [resource-index](memory/shared/resource-index.md) § 共用資源

### Spec 規格檔清單

→ 完整 Spec 規格檔清單見 [spec-registry](memory/erp/spec-registry.md)

### Prototype 實作進度

→ Prototype 實作進度見 [resource-index](memory/shared/resource-index.md) § Prototype 實作進度

### 工具

→ 完整工具清單見 [resource-index](memory/shared/resource-index.md) § 工具
# CLAUDE.md
