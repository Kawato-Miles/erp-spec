...
- Spec / BRD / 欄位 / 流程 / 角色有任何異動或設計方向確立時，主動收尾前觸發雙視角審查（`ceo-reviewer` + `erp-consultant` 平行執行），再執行 `doc-audit`；純 OQ 文字更新或措辭修正跳過
- 雙視角審查的觸發不限於正式寫 Spec：只要討論中有設計傾向出現（即使未觸發 erp-spec），也應審查；兩個 agent 先載入全部背景知識再給意見
