...
| 訂單管理 | `openspec/specs/order-management/spec.md` | https://www.notion.so/32c3886511fa806bad41d755349b0567 | v1.6 | 草稿（v1.6：add-order-note-section-with-template-tool 2026-05-20 歸檔 — ADDED Order 3 個 free-text 欄位（order_note / delivery_note / payment_note，UI label「訂單須知 / 交貨備註 / 付款備註」，500 字）；ADDED「訂單詳情頁訂單備註 section」Requirement（read-only 顯示 + 點編輯開 OrderNotesEditDialog + 訂單完成後 lock）；ADDED「訂單階段訂單備註編輯權限與時機」Requirement（業務 / 諮詢 / 業務主管 / 訂單管理人可編，訂單完成前可編輯）；ADDED「訂單備註與 payment_terms_note 共存策略」Requirement（位置 / label 加註區分）；ADDED「訂單客戶溝通備註 Data Model」+ 三維度並存（既有「訂單備註三類分欄」customer_note/internal_note/production_note 按「來源／可見性」 vs 新三類按「業務主題」）；NoteTemplatePopover 共用元件（prototype-shared-ui spec ADDED）；含 10 個新 OQ（ORD-005~013 + XM-006 + ORD-014）；v1.5 refine-after-sales-refund-and-add-supplementary-print 2026-05-20 歸檔延續）|
