...
- Spec / PRD 撰寫前必須讀且對照以下文件，確保認知對齊：
  - Notion 商業流程（https://www.notion.so/32c3886511fa81ccaaf9fbfd3882f19a）：業務流程與核心規則
  - Notion 狀態變化 § 上層（https://www.notion.so/32c3886511fa81539eb9d3c97630caa0）：需求單 / 訂單 / 工單 / 印件
  - Notion 狀態變化 § 下層（同上頁面）：任務 / 生產任務 / QC / 出貨單
  - Notion 業務情境 DB：具體業務情境驗證
  - Notion 使用者情境（https://www.notion.so/32c3886511fa8144b38adc9266395d15）：確認角色權責
...
**5. 文件即規格**
- 所有決策應記錄在相關檔案（Notion 商業流程、Notion 資料欄位 DB、Notion 業務情境 DB）
- 設計時應對照檔案檢查一致性（如本次發現 Notion 資料欄位 DB 缺欄位）
- 每次修訂應註記版本與理由

**6. 角色驗證優於假設**
- 流程中的每個動作都應明確指定執行角色（Who）
- Spec 撰寫時必須讀 Notion 使用者情境（https://www.notion.so/32c3886511fa8144b38adc9266395d15），確認角色權責是否合理
- 反例：「業務出貨」、「印務質檢」（角色錯誤）
- 正例：逐一檢查流程步驟 → 對照 Notion 使用者情境的角色權責 → 若發現矛盾，先修正 Notion 使用者情境再撰寫 Spec
...
- **術語對應表**（v1.0，2026-03-03）：
  ...
  - 「min() 聚合」→「最少工單原則」或「木桶原則」（描述取最小值的邏輯）
- 應用位置：Notion 業務情境 DB、Notion 狀態變化、Notion 商業流程等所有相關文件
...
- Spec / BRD / 欄位 / 流程 / 角色有任何異動或設計方向確立時，主動收尾前觸發三視角審查（`senior-pm` + `ceo-reviewer` + `erp-consultant` 平行執行），再執行 `doc-audit`；純 OQ 文字更新或措辭修正跳過
- 三視角審查的觸發不限於正式寫 Spec：只要討論中有設計傾向出現（即使未觸發 erp-spec），也應審查；三個 agent 先載入全部背景知識再給意見
...
| 功能設計有傾向（尚無 BRD）| 「我覺得應該這樣設計」「這個邏輯你怎麼看」「XX 要怎麼處理」 | `senior-pm` + `erp-consultant` 輕量審查 |
| BRD / Spec 撰寫 | 「寫 spec」「寫 BRD」「規劃文件」「需求文件」 | 觸發 `erp-spec` skill（含完整工作流）|
| BRD 草稿審查 | BRD 草稿完成後 | 三視角平行（`senior-pm` + `ceo-reviewer` + `erp-consultant`）|
| 系統一致性 / 狀態機確認 | 「會不會跟 XX 衝突」「這個狀態怎麼設計」「資料怎麼連」 | `erp-consultant` |

**路由注意事項**：
- 同一段討論若跨越多個類型（如從痛點探索轉入設計討論），在類型轉換時主動帶入對應 agent
- 純澄清 / 查詢（「XX 術語是什麼」「狀態機在哪裡」）不觸發 agent，直接回答
- 已在執行 erp-spec 工作流時，agent 呼叫遵循 erp-spec SKILL.md 定義，不重複觸發
...
| Task 類型 | 必讀 | 視需要 |
|----------|------|--------|
| 訂單 / 需求單 / 審稿 / 工單設計 | Notion 狀態變化 § 上層 | Notion 商業流程 |
| QC / 任務 / 生產任務 / 出貨設計 | Notion 狀態變化 § 下層 | Notion 商業流程 |
| 跨層流程（含上下層）| Notion 狀態變化（上層 + 下層）| — |
| **撰寫 Spec / PRD** | 觸發 `erp-spec` Skill（包含完整工作流 + 必讀檔案清單） | 迭代時參考 `memory/erp/spec-iteration-workflow.md` |
| 情境驗證 / 補情境 | Notion 業務情境 DB + Notion 狀態變化 對應章節 | Notion 使用者情境 |
...
### Spec 規格檔清單
> Spec 統一存於 Notion Feature Database，每次新增 spec 後更新此表。
...
| 訂單管理 BRD | https://www.notion.so/32d3886511fa81a9bf64e82753a80031 | v0.1 | 草稿 |
