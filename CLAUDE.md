...
- 凡涉及 Spec / BRD / 需求 / OQ 的實質討論，在對話收斂後主動執行下列收尾動作，**不等 Miles 提示**：
  ...
  10. **change archive 後 MUST 對齊 wiki 商業邏輯卡**（依 `erp-planning-pre-check` 產出的「wiki 商業邏輯卡回補清單」執行），確保 ERP_Vault 商業邏輯正本（04-business-logic / 05-entities / 06-state-machines / 07-scenarios / 13-user-stories）不停留舊版（converge change 漏對齊 wiki 6 卡教訓）
