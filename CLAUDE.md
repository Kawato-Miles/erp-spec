...
- **OpenSpec（PRD 層，openspec/）= 系統行為規格單一正本**：功能 Requirement / 驗收 Scenario（Given/When/Then）/ 狀態轉換規則（guard 條件 + 觸發事件）/ 角色權限規格 / User Story / change 工作流。回答 how（系統在什麼條件下做什麼），驅動 Prototype 與 Notion 發布版。**不再包含 Data Model 欄位表與狀態列舉**（已遷移至 wiki）。
- **單一正本鐵則（同一內容只有一個家）**：
...
| 訂單 / 需求單 / 審稿 / 工單設計 | Vault `06-state-machines/` 對應狀態機卡 + `05-entities/` 對應實體卡 | wiki `04-business-logic/`（商業流程正本） |
...
| 角色 R&R | Vault `03-roles/`（16 角色含 _alignment-report，角色職責唯一正本） | — |
| 資料模型（查詢 / 新增 / 修改 / 移除欄位）| Vault `05-entities/` 實體卡 § 欄位（業務可見）為正本（**2026-06-09 遷移：欄位正本從 OpenSpec Data Model 移至 wiki**）| Notion 資料欄位 DB（發布版）：https://www.notion.so/32c3886511fa803e9f30edbb020d10ce |
