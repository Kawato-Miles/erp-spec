...
| 規格撰寫 / 變更 | 「寫 spec」「新增功能」「修改 XX 規格」「規劃文件」 | `/opsx:propose` → 序列協作 Phase 1-4 完整流程（含 OQ 查詢 + PM 釐清 + Phase 2 PM↔CEO 來回上限 2 + Phase 3 PM↔ERP 顧問來回上限 2 + Phase 4 PM 匯報含 verify consistency 兩張對照表，依 [[sequential-design-collaboration]] + [[dispatch-prompt-template]]）|
| 規格草稿驗收前審查 | `/opsx:verify` 前需最終確認 | **過渡期保留**三視角平行依 [[multi-agent-discussion-protocol]]（status: `deprecated-verify-only`）。**未來淘汰條件**：累積 3-5 個 change 使用 sequential Phase 4 verify consistency（兩張對照表）足以取代後，本 row 改為「不啟動，由 sequential Phase 4 verify consistency 涵蓋」|
...
- `/opsx:propose` § Why/Background → 序列協作 Phase 1-4 完整流程（Phase 2/3 PM-中介來回上限 2 + Phase 4 verify consistency 兩張對照表）
...
- 序列協作 Phase 4 由 PM 匯報七段：商業需求對齊檢核 / 採納清單 / 砍掉的功能清單 / 逐條回應 challenge / **verify consistency 兩張對照表（2026-05-28 新增，紀律 5）** / 未解爭議 OQ / 整體設計方案，**MUST NOT** 由 Claude 協調者自行彙整
