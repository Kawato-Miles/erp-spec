...
- **Prototype＝介面與互動正本（交付層，2026-07-15 新增）**：頁面結構與版型、操作動線、元件呈現以 Prototype 為準；mock 資料的值、API 形狀、權限的後端強制不在其權威範圍。交付到 Linear 時 Prototype 參照與「為準／不為準」邊界表為必要結構件；格式正本在 `linear-delivery` 交付模板（v2.0），此處不重述。設計票（DE 票）不再產出，操作方式與介面由 Prototype 承載。
