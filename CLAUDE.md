...
Spec 或 Prototype 完成後，確保參考文件同步更新。詳細流程見：
- **迭代工作流** → `memory/shared/principles.md` § 三、Spec / Prototype 迭代工作流
- **三層檢查規則** → `.claude/skills/erp-spec/SKILL.md` Step 6

**簡化檢查流程**：
1. **迭代前**：在 Spec 頂部補充驗證標準（見 principles.md）
2. **迭代中**：邊做邊檢查，對照 SKILL.md Step 6 三層檢查
3. **迭代後**：執行 `bash .claude/skills/erp-spec/scripts/audit-erp-docs.sh`，在 Spec 末尾註記「§ 多檔案驗證已完成」
