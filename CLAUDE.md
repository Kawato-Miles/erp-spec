...
Spec 或 Prototype 完成後，確保參考文件同步更新。

**詳細流程**：
- **框架** → `memory/shared/principles.md` § 五（通用工作流）
- **ERP 實施細節** → `memory/erp/spec-iteration-workflow.md`（檔案清單、驗證標準、檢查清單）
- **技術檢查規則** → `.claude/skills/erp-spec/SKILL.md` Step 6（三層檢查）

**快速檢查**：
1. **迭代前**：在 Spec 頂部補充驗證標準（見 memory/erp/spec-iteration-workflow.md 模板）
2. **迭代中**：邊做邊檢查，對照 ERP 實施細節的檢查清單
3. **迭代後**：執行稽核腳本 + 檢查清單 + Commit 時註記
...
| **Spec / Prototype 迭代** | **`memory/erp/spec-iteration-workflow.md`（實施細節）** + `memory/shared/principles.md` § 五（框架） | `.claude/skills/erp-spec/SKILL.md` Step 6 |
