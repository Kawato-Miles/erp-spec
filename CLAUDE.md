...
| 訂單管理 | `openspec/specs/order-management/spec.md` | https://www.notion.so/32c3886511fa806bad41d755349b0567 | v1.3 | 草稿（v1.3：extend-order-fields-from-vendor-feedback 2026-05-14 歸檔 — 2026-05-08 需求單位 26 條欄位備註收斂：Order 新增 contact_id / production_note / 6 個 _without_tax / tax_amount / signed_at 雙欄計價（線下未稅主顯、線上含稅主顯）、notes 拆 customer_note / internal_note / production_note 三類、補 Invoice 完整 Data Model 含 ezpay_invoice_url（移除 print_flag）、新建 SalesAllowanceFile / PaymentFile / OrderSignedFile 子表、PaymentPlan.scheduled_date 改必填、PrintItem.unit / difficulty_level 訂單階段可編輯、印件規格回簽前自由改回簽後走 OrderAdjustment、出貨單一對一綁訂單、訂單類型 enum 範圍說明不含月結 / 訂閱制、新 OQ ORD-010 線上單 case_name 規則；v1.2 OrderAdjustment 雙重身份 / 對帳警示 / OrderExtraCharge 時間邊界延續）|
