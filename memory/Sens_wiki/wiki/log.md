...
- 變更：[[_archives/2026/AFT-10-售後單文字欄位修改鎖定規則|AFT-10]] 結案封存（決議：客訴原文與結案後回饋可改、修改寫入活動紀錄）、[[_archives/2026/AFT-11-售後類型枚舉是否含色差爭議|AFT-11]] 結案封存（決議：採 7 值加色差爭議）、[[PT-006-生產三實體欄位修改規則與模型矛盾|PT-006]] 補部分拍板（工廠類別唯讀綁 BOM——工序／材料決定負責工廠、工單只能依 BOM 建生產任務；狀態枚舉與色彩模型統一以涵蓋較完整版本為準）、[[QR-003-需求單欄位修改規則與規格雛形落差|QR-003]] 補部分拍板（一般欄位成交／流失終態後全卡鎖定——成交即轉訂單、需求單轉為參考依據）、[[CR-7-諮詢單編號正本缺口與表單欄位修正規則|CR-7]] 補部分拍板（付款完成有帳款紀錄才進 ERP 防未付款雜訊；任何諮詢人員可修正表單欄位、留活動紀錄）、新增 [[PT-008-生產數量追加調整與工單異動承接|PT-008]]（實務有數量追加調整，既有工單異動流程能否承接待盤點）；同步落地五張實體卡——[[售後服務]]（售後類型加色差爭議、兩文字欄位改可改留紀錄）、[[生產任務]]（生產單位類別改唯讀綁 BOM、數量改指 PT-008）、[[工單]]（數量改指 PT-008）、[[需求單]]（全卡終態鎖定規則落各列）、[[諮詢單]]（表單欄位諮詢可修正留紀錄、營運背景補付款前提）
