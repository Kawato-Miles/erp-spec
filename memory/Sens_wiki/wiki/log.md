...
- 變更：[[生管]] v2 重寫——依拍板確立與印務的分工（印務管排程參數與交付、生管管接收／指派師傅／確認異動）、舊卡誤指 XM-003 為合批派工修正為 [[XM-004-合批派工合併邏輯|XM-004]]；[[會計]] v2 重寫——依拍板確立「主責公司現金流」定位（月結對帳、依業務主管月退款報表執行退款、特例才協助開票——發票原則歸業務／諮詢）、「已執行」改「確認可執行」、2026-05-28 月結對帳對談內容整併入職責；[[業務主管]] 補職責「每月提供退款報表給會計」（退款的決定與執行分屬兩線、報表為交接介面）；[[出貨]] 改名 [[出貨人員]]（obsidian-cli 改名、全 vault 連結自動更新）並轉 v2 草稿殼（status: draft，詳細職責待 Miles 後續提供訪談資料與實際情境，與出貨單狀態卡同批）；[[XM-004-合批派工合併邏輯|XM-004]] 補開（XM-003 與舊角色卡早已引用但卡未建檔的斷鏈）；[[ORD-001-會計階段標記是否錯誤|ORD-001]] 標 answered（會計參與涉及錢的全程、「僅審稿階段」確為錯誤標記、v2 廢除參與階段欄位後問題載體消失）。
