...
**frontmatter**：欄位樣板見 [[範本 - 業務情境]]。`variant` 必填（判定見 § 二）；`source` 往上指服務藍圖／`04-business-logic/` 規則卡／拍板 OQ／外部依據，禁指同層、下層、實作規格；**不設 `module`／`implemented-by`／`related-spec`**（與實作模組、實作文件的對應屬 PRD 層，wiki 不承載；領域歸屬由 `tags` 承載）。
