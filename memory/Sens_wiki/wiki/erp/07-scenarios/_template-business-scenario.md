...
# 業務情境卡撰寫規範（scenario 層）

> 三層結構：動筆從 [[範本 - 業務情境]]（骨架）複製起手；合規樣貌對照 [[範例 - 業務情境]]（凍結快照）；共用治理（撰寫流程／停下鐵則／撰寫紀律／修改既有卡原則）見 [[卡片撰寫共用規範]]——**本卡只寫業務情境單元的特定標準**。
> 設計依據：[[index]] § Wiki 憲章（本質、第一讀者、兩判準、單元定位模式）；方法論依據：Cockburn《Writing Effective Use Cases》（主線＋延伸掛接、目標層級）、Domain Storytelling（誰－做什麼－交給誰的敘事句型）。本規範依「雙向開發」迭代：每張卡稽核時發現的缺口回饋本卡，三層同 commit 更新。
...
六步流程（辨識 → 歸位 → 確認前提 → 撰寫 → 稽核 → 定案）與各步卡住時的出口見 [[卡片撰寫共用規範]] § 二；本單元對應的判斷表為 § 二、前提確認為 § 三、撰寫規則為 § 四、產出格式為 § 八、稽核維度為 § 十二、邊際情境出口為 § 六。
...
> 本節目的：MUST 不容裁量、SHOULD 偏離須說明、MAY 自由。每條附理由，理解理由才能在沒涵蓋的情況正確裁量。共用紀律（回引不複寫／禁中英夾雜／OQ 外移／檔名即標題／零迭代史／不編造）見 [[卡片撰寫共用規範]] § 四，不在此重列。

| 強度 | 規則 | 理由 |
|------|------|------|
| MUST | 每段內容過紙本測試與商業語言兩判準（§ 〇）；正文零實作規格引用 | 本卡是 BRD，第一讀者讀不到實作文件 |
| MUST | 段名一律依 [[範本 - 業務情境]] 骨架、不加情境名前綴（寫「主成功過程」，不寫「主成功過程（線下訂單成立確認）」）；「相關卡」下的 `###` 分類標題依骨架分類命名 | 段名跨卡統一，稽核與跨卡查找依字面即可判定 |
...
| MUST | 不寫痛點現象（要連痛點以 wiki link 指藍圖痛點段或 [[痛點]]） | 痛點正本掛接點唯一在藍圖 |
...
> 本節目的：高風險點的強制人工確認。共用四條（無來源依據／矛盾／需要假設／超出已知）與處理方式見 [[卡片撰寫共用規範]] § 三，不在此重列——本節只列業務情境特有的停下條件。

1. **行為者不明**：某步講不出誰做 → 停下問使用者，**禁止**寫「系統自動」充數。
2. **判準講不出可觀測結果**：某步「做完了沒」沒人說得出怎麼判 → 停下（這正是情境卡要逼出來的缺漏）。

---

## 八、產出格式

一張卡＝一個業務目標的完成過程。**檔名即標題**（卡內不寫 `# 標題`）。**正文零實作規格引用、無正文來源段**（正確性來源由 frontmatter `source` 承載；歷史查 wiki/log.md 與 git）。frontmatter 樣板與各段填寫提示見 [[範本 - 業務情境]]（骨架），完整合規樣貌見 [[範例 - 業務情境]]。
...
**frontmatter**：欄位樣板見 [[範本 - 業務情境]]。`variant` 必填（判定見 § 二）；`source` 往上指服務藍圖／`04-business-logic/` 規則卡／拍板 OQ／外部依據，禁指同層、下層、實作規格；狀態機卡得**並列**為參考資料（部分情境確實由狀態機推導產生），但不得為唯一或直接源頭——直接源頭仍須是藍圖／規則／拍板依據；**不設 `module`／`implemented-by`／`related-spec`**（與實作模組、實作文件的對應屬 PRD 層，wiki 不承載；領域歸屬由 `tags` 承載）。
...
共用五條（規則回引不複寫／禁中英夾雜與禁實作術語當主詞／不確定項開 OQ／檔名即標題與正文零迭代史／不編造）見 [[卡片撰寫共用規範]] § 四。業務情境特有兩條：

1. **過程只寫一次**：同一目標只有一張卡；共用過程引用主情境，禁複述。
2. **詞彙一致**：狀態名／角色名／單據名與各正本卡一字不差。
...
## 十二、稽核維度（13 維度；vault-audit 用；寫完逐項勾）

> 本節目的：把稽核從被動清單變成**可驗證的完成目標**。一張業務情境卡的「完成」＝下列維度全部通過，**標竿對照一律以 [[範例 - 業務情境]] 為通過樣本**（禁以會持續演化的實際卡當正面例子）。執行者／稽核者分離原則見 [[卡片撰寫共用規範]] § 二。
...
| 13 | 連結方向 | `source` 指藍圖／04 卡／拍板（禁同層、下層、實作文件；狀態機卡僅得並列參考、不得為唯一來源）；相關卡雙向可達、無斷鏈 |
...
- [[index]] § Wiki 憲章 — 本質、判準、單元定位模式（本規範的上位依據）
- [[範本 - 業務情境]] — 骨架（寫新卡的起手稿）
- [[範例 - 業務情境]] — 凍結範例（合規樣貌與稽核通過樣本）
...
- `06-state-machines/_template-state-machine.md`、`03-roles/_template-role.md` — 同構 sibling 規範（擴散接線中）
