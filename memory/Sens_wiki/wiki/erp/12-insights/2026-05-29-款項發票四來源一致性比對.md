...
- **指標 → 營運管理 KPI**（ceo-reviewer + CEO 誤區校正）：新增「收款/開票領域營運管理 KPI」Requirement（收款達成率/異動率/對帳差錯率/逾期收款率/預收未沖/開票及時率/變更率）；移除「業務手動覆寫率」；DSO 後驗；收款變更率公式清理
...
- **US-CR-005**：移除 invoice_option 驅動開票 + 已開發票中間態 → 自動建待開票收款項目 + 手動開票 + 訂單完成終態
