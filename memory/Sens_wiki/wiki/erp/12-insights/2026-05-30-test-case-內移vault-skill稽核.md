...
- **未竟（漸進）**：16 張抽出的 UI 內容（[[_migration-ui-from-userstory]]）seed 成 TC 卡 + Notion 正文；既有 user-story `related-test-cases` 空欄回填（併 [[BI-12-款項UserStory批次backfill business-domain與related-test-cases|BI-12]] / [[BI-13-第一輪稽核識別的領域缺漏項|BI-13]]）；sync-workflow 流程 1-C 與 scope-boundary test-case 定位補述（skill Mode C 已涵蓋推送機制）。
