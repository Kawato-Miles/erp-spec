...
related-vault:
  ...
  - "（已移除，見各 skill）"
  - "[[scope-boundary]]"
  - "`04-business-logic/_template-business-logic.md`"
...
稽核 `erp-test-case` skill 是否過時，揭露核心轉折：test case 的管理正本應從 Notion 移進 Vault（Obsidian），Notion 降為對外發布版，比照已內移的 [[wiki/erp/13-user-stories/README]]。
...
- （已移除，見各 skill）（新增流程 1-C）
- [[scope-boundary]]（補 test case 定位）
- `04-business-logic/_template-business-logic.md`（L142 憲法句）
...
- **未竟（漸進）**：16 張抽出的 UI 內容（_migration-ui-from-userstory，已隨 test-case 單元廢止刪除）seed 成 TC 卡 + Notion 正文；既有 user-story `related-test-cases` 空欄回填（併 [[BI-12-款項UserStory批次backfill business-domain與related-test-cases]]（已封存） / [[BI-13-第一輪稽核識別的領域缺漏項]]）；sync-workflow 流程 1-C 與 scope-boundary test-case 定位補述（skill Mode C 已涵蓋推送機制）。
