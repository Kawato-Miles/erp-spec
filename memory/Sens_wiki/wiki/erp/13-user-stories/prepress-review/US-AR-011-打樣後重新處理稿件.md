...
related-scenarios:
  - "[[wiki/erp/07-scenarios/README#情境 2：打樣決策流程]]"
...
4. 業務於新印件上傳新稿件 → 系統建立 ReviewRound 1 進入審稿流程（與 [[wiki/erp/07-scenarios/README#情境 14：審稿流程端到端（單模組跨角色）|07-scenarios 情境 14 審稿流程端到端]] 主流程一致；不繼承原印件的審稿輪次歷史）
...
| 「作為」應改業務（不是審稿員）| senior-pm 段 4（high）| 已採納：role 改為 [[業務]]；業務情境段加註說明改變理由 |
