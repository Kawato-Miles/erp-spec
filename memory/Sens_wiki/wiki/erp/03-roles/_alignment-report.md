...
- ✅ 專案層的利害關係人清單已引用所有 16 個角色（該清單卡已於 2026-07-30 退役，角色正本統一在 `03-roles/`）
...
1. [[XM-002-印務 vs 印務主管權責邊界]]（已封存） — 印務（執行）能直接建生產任務嗎？還是需印務主管批准？
2. [[ORD-001-會計階段標記是否錯誤]]（已封存） — Notion DB 標記會計「僅審稿階段」是否錯誤？
3. [[XM-003-訂單管理人 vs 業務權責邊界]]（已封存） — 兩者都有訂單 R/W 權限，工單異動執行責任在誰？
4. [[QC-001-OpenSpec 品管是否拆審稿與 QC]]（已封存） — OpenSpec「品管」是否該拆為「審稿」+「QC」？
