...
>
> 對齊 [[wiki-architecture]] § 二分層體系（驗收項目 = 最細的一層）+ § 七各層怎麼寫（前提→動作→驗收結果三段）+ [[business-logic-writing-guide#4.0 各類卡的單一職責（六層由大到細、只連不重抄）]]（test-case 職責）+ [[wiki-schema#十一、卡類型內容職責邊界]]。
>
...
依據鏈中本卡的位置（對齊 [[wiki-architecture#完整依據鏈範例（訂單異動補收免審）]]）：
...
> 共同 5 欄對齊 [[wiki-schema#四、各 type 必填 Frontmatter 欄位]]；往上指依據（source）、往下指實作（implemented-by）兩欄對齊 [[wiki-architecture#依據往上、實作往下，連結不繞回自己]]，為這類卡的前進標準。
...
> 共同 4 段（一句話定位 / 這張卡要回答的問題 / 營運背景 / 相關連結 / 來源）對齊 [[business-logic-writing-guide#4.1 共通骨架（business-logic / scenario / role 直接套用）]] 的共同骨架；這類卡特有的前提→動作→驗收結果三段（前置條件 / 測試步驟 / 預期結果）對齊 [[wiki-architecture#各層怎麼寫]]。
>
...
- [ ] 正文零迭代史（迭代史在 [[business-logic-changelog]]）；正文零「待確認 / 待釐清 / 需確認 / 尚未確認 / 待補」inline OQ 措辭（→ [[oq-manage]] mode B）。
...
- [[wiki-architecture]] — Wiki 結構與撰寫規範（驗收項目這層的定位、依據往上實作往下、撰寫風格、推動步驟）
- [[business-logic-writing-guide#4.0 各類卡的單一職責（六層由大到細、只連不重抄）]] — test-case 職責（正文存 Notion、source 指 user-story）
