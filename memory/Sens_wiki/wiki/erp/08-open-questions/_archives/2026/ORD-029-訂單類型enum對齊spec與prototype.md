...
- [x] 「客製單」與「點數」如何併入三值（待 EC 整合處理）。 → 移交 [[ORD-043-EC整合前置訂單類型收斂與下游分流盤點]]
- [x] 對齊後依 order_type 分流的下游邏輯（諮詢訂單終態收斂、B2B / B2C 分流）影響盤點（隨 EC 整合一併處理）。 → 移交 [[ORD-043-EC整合前置訂單類型收斂與下游分流盤點]]
...
結案（answered）：本卡主題——訂單類型 enum 幾值、叫什麼——已於 2026-06-15 裁決為三值帶「單」（線下單／線上單／諮詢單）且 wiki 與 spec 已對齊，無再裁決事項。剩餘兩項（Prototype「客製單」與舊「點數」如何併入三值、依訂單類型分流的下游邏輯盤點）皆以 EC 整合為前提，移交 [[ORD-043-EC整合前置訂單類型收斂與下游分流盤點]] 掛觸發條件，本卡不再承載。拍板：2026-08-04，Miles。

落地去處：殘項移交 [[ORD-043-EC整合前置訂單類型收斂與下游分流盤點]]；[[訂單]] 訂單類型欄用詞帶不帶「單」的字面落差（現況「線下／線上／諮詢」）隨 ORD-043 處理時一併正名。
