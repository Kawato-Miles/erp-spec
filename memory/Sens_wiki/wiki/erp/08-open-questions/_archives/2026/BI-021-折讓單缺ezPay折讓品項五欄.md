...
- [x] 折讓單是否需比照發票加「折讓商品五欄」品項結構（wiki + spec + prototype + UI dialog）？ → 見決議
- [x] 若加，折讓品項預設帶入規則（從原發票品項帶入後可改，比照開立發票）？ → 見決議
- [x] Mockup 階段是否先以「單一總額」近似、品項五欄留正式串接 ezPay 時再補？ → 見決議
...
業務層維持單一折讓總額、不補品項五欄欄位：ezPay 法定的折讓品項五欄由系統送件時以**單一合成品項**自動組成——品名帶折讓原因（截至 30 字）、數量 1、單位「式」、單價與小計為依發票稅率反推的未稅額、稅額補足「總額＝小計＋稅額」檢核等式。此機制後端現行程式已實作並驗證（現況查證：折讓開立服務的送件欄位組裝），缺口不存在。拍板：2026-08-04，Miles（指示檢查後端現況後依實作補齊 wiki）。

落地去處：[[帳務]] 折讓單欄位段補「五欄由系統合成單一品項」機制句（結構 workflow 完成後執行，避免並行編輯衝突）。
