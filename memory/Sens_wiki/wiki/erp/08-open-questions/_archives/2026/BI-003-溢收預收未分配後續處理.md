...
預收（未分配）桶的後續兩條路皆可——核銷到日後新建的收款項目、或退還客戶——由業務依客戶狀況自行判斷；不加「從預收核銷」專用按鈕、不做預收清單，桶內餘額由對帳的收款差額（負值）看得到。依「先不做」偏好：沿用既有入口、不疊機制。拍板：2026-08-03，Miles。

落地去處：[[收款核銷分配]] 溢收岔路的「尚未拍板」句改為定論。
