...
reclassify-qc 廢止 `QCRecord` 獨立實體，併入 ProductionTask。**遷移標的更新（2026-07-21）**：品檢模型再拍板為**印件層品檢紀錄**（品檢型生產任務亦廢止，見 [[QC-002-QC兩張wiki卡退役或保留]] 決議），既有資料（QCRecord 與 Prototype 品檢型任務實作）的最終遷移目標為 [[品檢紀錄]]（掛印件、分次驗、累計通過＝入庫）。Prototype 階段不處理資料遷移（依 Miles 指示）。正式上線時需決定：
