...
建 spec 時須一併修正的不一致：business-scenarios spec 分批出貨情境的出貨單狀態鏈（未處理 → 打包中 → 待出貨 → 出貨中 → 已送達）與 [[出貨單狀態]] 現行正本（未處理 / 運送中 / 已送達 / 異常 / 已作廢）不一致。**收斂方向（2026-07-08 更新）**：生產階段流程重整已拍板揀貨階段與四態融合方向（揀貨裝箱納入出貨單狀態顆粒度），新列舉細節待 [[SHP-007-揀貨裝箱回報載體與出貨單狀態顆粒度]] 拍板後，wiki 正本與 spec 一併收斂。
