...
## [2026-06-02] erp-planning-pre-check | L1.6+L1.2 訂單收退款通用方案（路 C）| 雙領域並行稽核

**議題**：訂單收退款通用方案（路 C：退款核銷對帳差額、明細時點分界=訂單完成、OA「已執行」=應收調整生效）。方向書 `~/.claude/plans/1-2-pure-dewdrop.md`。結構性變更、跨 billing-cash + order 雙領域，執行者/稽核者分離（兩稽核 sub-agent + 主對話修補）。
...
### 最關鍵發現（pre-check 攔截路 C 設計風險）
- **攔截 1（致命）報價鎖定時點**：現有鎖定點=審核通過（[[訂單狀態]] L42 動機「防核可後偷改金額繞過主管」），路 C 後移到訂單完成，推翻 [[ORD-027-訂單額外費用凍結時點與審核通過成交鎖定對齊|ORD-027]]（2026-06-01 resolved）+ 踩 spec L1822（單價製作前可改）vs L3671（報價總額審核通過鎖定）既有矛盾。**Miles 拍板：鎖定點=訂單狀態「訂單完成」，接受推翻 ORD-027。**
- **攔截 2 退款保護降級**：路 C 拆「OA 已執行=一定退完」單筆硬約束（[[訂單異動規則]] L67/L89/L161）換對帳軟兜底，重開「帳上已退、實際沒退」。**Miles 拍板：照路 C + 退款款項已完成當物理錨點 + 差額警示不可忽略。**
...
- ORD-027（額外費用凍結）re-open / 標被路 C 取代
- [[BI-9-補收OA立即執行對稱破壞表述|BI-9]] 路 C 推翻其 invariant 前提，須收斂或併入
...
### Step 5 閉環驗證
- 本輪目的=攔截設計跑偏，已達成（攔截 5 項、Miles 決策 3 項、浮現 propose blocker=業務訪談）。M 細項未轉 N（路 C 待業務訪談 + propose 序列協作定案後回補），非 false completion，延後依據已明示。

### 反模式識別
- 保護降級（路 C 拆退款硬約束換軟兜底）：已攔截，Miles 決策補強（物理錨點 + 不可忽略）
- 風險誤類比（BI-15 兜底未開票應收 ≠ 路 C 兜底退款未執行，嚴重度不同）
- 推翻剛 resolved 結論不回收（ORD-027）：已攔截，待 re-open
- 憑空新增概念無 know-how 比對（明細時點分界）：已攔截，待業務訪談
- 主對話 agent 推薦路 C 過快、未先稽核既有保護：自我識別，由 pre-check 補正
