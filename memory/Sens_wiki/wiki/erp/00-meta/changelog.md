...
## [2026-06-10] 卡片異動 | 諮詢→需求單→訂單端到端比對：「三條前段路徑」歷史殘影修正＋範本加敘事不越界檢查

- **調查結論**（systematic-debugging 四方比對）：四條端到端鏈（不做大貨／成交／流失／取消）的轉換規則層全部一致；矛盾集中在敘事與分類層。Miles 指出的訂單卡「諮詢訂單不做大貨，建立時就直接收尾」根因＝**「三條前段路徑」框架是歷史殘影**——諮詢訂單曾有自己的狀態路徑（已開發票態廢止證據在 spec），簡化為「建立即終態」後框架詞未收斂，造成來路（建立來源）與前段（本實體狀態階段）概念混淆＋「客戶先付諮詢費」實體錯位（屬諮詢單）＋「不做大貨」專名挪用。
- **修正**：[[訂單狀態]] 概述改寫為條列式「來路」框架（現階段三種、開放措辭不寫死數量，Miles 指示）；order-management spec 兩處主文同步（§ 訂單狀態機改兩條前段＋短路收尾條列、§ 訂單建立補列需求單流失情境——原主文「兩種情境」與型別註解「三種」字面衝突消解，Scenario 零變動）；[[訂單]] 實體卡三處「三種來路」開放化。撤回提案中自創的「不做大貨家族」一詞（商業上不存在），改用行為描述「收尾方式與不做大貨相同」。
- **範本優化（經回測＋盲測驗證後寫入）**：前提確認加「建立來路」一問（來路與前段是兩回事）；撰寫規則加 MUST「概述敘事不越界」；稽核維度加第 13 項。驗證：回測訂單卡原概述三個違規點全命中；盲測其餘 13 張卡零誤報、零新陽性（誠實限制：陽性樣本僅一，規則適用面為「多來路實體」結構）。
- **待裁決遺留**：訂單類型值域三處不一致（Prototype 型別四值含「客製單」、實體卡與 spec 三值）與「一般 vs 線下」正名——開放措辭已為未來類型留空間，客製單納卡時點待 Miles 指示。

## [2026-06-10] 商業規則 | 諮詢單「已轉需求單」改終態定格（方案三拍板）＋諮詢單狀態卡重寫

- **設計變更（Miles 拍板）**：諮詢單狀態改為承載商業出口分類（待服務／純諮詢結案／轉入銷售／取消退費），「已轉需求單」改為**終態定格**——下游需求單成交或流失皆不回寫諮詢單（原規則：流失時更新為「完成諮詢」）。刪除「需求單流失→諮詢單轉完成諮詢」跨單自動轉換。建諮詢訂單的觸發機制不變（靠需求單流失事件＋需求單上的諮詢來源欄位判斷，與諮詢單狀態無關——狀態給人看、欄位給系統判斷）。
- **推演依據**：三方案以 10 張樣本推演五統計（商業狀況分布／在途清單／結案量／轉化漏斗／已收未結算）對比，方案三在「狀態即商業分類報表」「漏斗第二層直接數狀態」「刪跨單轉換降實作風險」三項勝出；僅「已收未結算」便利指標退化（對帳正本在訂單層，可接受）。
- **同步修正 13 處**：consultation-request spec 7 處（含終態列舉補「已轉需求單」禁改派）、order-management spec 3 處、quote-request spec 2 處、[[諮詢收尾規則]] 流失規則行、[[需求單狀態]] 關係段。
- **[[諮詢單狀態]] 依新範本重寫**：含 mermaid UML 段、module 中文化、`implemented-by` 改 spec 檔層（原第一條指向 spec 中不存在的 Requirement「諮詢單狀態機（v2 簡化）」，標題錨點斷鏈活例）。
- **Prototype 待跟進**：若前端狀態邏輯有實作「流失回寫完成諮詢」需移除（apply 階段處理）。

## [2026-06-10] 卡片異動 | 狀態機層重構啟動：範本重寫 + 訂單／需求單兩卡走查完成

- **範本**：`_template-state-machine.md` 完全重構（同構 04 新版範式：撰寫流程六步／判斷表含單鏈・分段・雙維度三變體／前提確認／error handle／不收清單／12 項稽核維度，融入 BABOK 狀態表完整性、Wiegers 圖表互核、UML 可達性檢查）。狀態機圖載體定為卡內 `## 狀態機圖（UML）` mermaid 段（stateDiagram-v2；訂單卡 Miles 手繪版為記法正本），canvas 檔降為過渡。
- **[[訂單狀態]]**：依新範本重寫——補文字流程圖＋轉換條件表（21 條）＋mermaid UML 段；修正「製作等待中＝審稿通過」過時語意（→全部印件已確認可製作）；取消路徑顯式化；打樣視角差說破（打樣印件走完整流程、訂單停審稿段）。同步修 `order-management/spec.md` 5 處回簽 Scenario 殘留（2 處廢棄狀態名＋3 處跳過「已回簽」中繼）。
- **[[需求單狀態]]**：四方比對（06 卡↔spec↔Prototype 型別↔04 規則卡）抓出 3 矛盾＋3 過時，Miles 裁決後重寫——重新評估僅議價中可退回（已評估成本改成本不退狀態）、任一非終態皆可流失（同步補 `quote-request/spec.md` 非終態流失規則與 Scenario）、刪除不存在的「逾期未回覆」機制、成交改手動轉訂單（草稿起走）、刪線上來源句、補諮詢來源流失收尾連動。
- **[[wiki-schema]]**：module enum 改繁體中文（Miles 拍板，與 business-domain 一致）＋新舊對照表；訂單／需求單卡 module 已改中文。
- **OQ**：新增 [[QR-001-雙欄計價具體規則]]、[[QR-002-報價有效期限與議價規則]]（自 [[報價邏輯]] inline 待確認遷出，oq-manage mode D）。
- **04 卡**：[[線下訂單流程]] § 確認打樣改寫＋[[打樣流程]] 規則段補打樣印件走完整流程一句（Miles 核准提案後寫入）。
...
**輸出**：
- [[2026-05-20-售後ticket-reactive-補丁循環]]（priority high，4 actions）
- [[2026-05-20-change-archive-OQ收尾流程缺口]]（priority high，4 actions）
...
- [[2026-05-21]]：依新規約重寫整張卡（建議排序從原「priority high 在前」改為「相依性鏈：收斂 active → PM 推演 → 治理債」；移除三條建議的「產出位置」與「預估時間」欄位；改寫「為何現在做」措辭去除「快速完成」字眼）
...
- [[2026-05-21]]：再次重寫 3 條建議（套用新結構：每條 Action 標題 + 現況條列 + Next action 條列；相依性併入現況段）
...
- [[2026-05-21]]：3 條建議的相依性段重寫為「阻擋 / 被阻擋」格式（建議 1 阻擋建議 2 + 整合 change propose、被阻擋無；建議 2 阻擋 Insight 1 Action 2、被阻擋建議 1；建議 3 獨立）
...
- [[2026-05-21]]：補三段反思內容（反目標 3 條 / 決策紀錄 2 條 / 學到 2 條）
- [[2026-W21]]：補三段反思內容（未完成原因 3 條 / 決策品質 3 條 / Pre-mortem 3 條）
...
1 新 OQ：
- [[ORD-1-Notion-US-ORD-005編碼重複處理]]（已 answered，沿用 AR-2 模式）
...
3 張 user-story 卡 v2：
- [[US-AR-009-B2B業務代客戶補件]]：「我希望」縮 48 → 19 字、補業務權限邊界 / 原審稿員不在崗主管覆寫 / 補件檔案 round_id NULL 暫存 / 修正既存 client_note ActivityLog 稽核、移除下游連鎖反應改 wiki link 引 US-AR-007
- [[US-AR-010-B2C會員補件流程]]：「我希望」縮 53 → 22 字、補跨系統介面契約 EC↔ERP / 歷史輪次清單細粒度 / 原審稿員不在崗主管覆寫 / 狀態轉移 ActivityLog、role 純文字標記符合 AR-1 拍板（Lint 例外）
- [[US-AR-011-打樣後重新處理稿件]]：「我希望」縮 52 → 24 字、「作為」改業務（雙視角共識）、補根因判定者與實體機制（引 OQ AR-12）、與 US-AR-009 / 010 業務本質差異說明（跨階段重新進入審稿 vs 同週期補件）

2 新 OQ：
- [[AR-11-補件停滯處理機制與輪次上限]]（priority medium，跨 US-AR-009 / 010；合併輪次上限 / 停滯 SLA / 棄單處理三個共通痛點）
- [[AR-12-打樣後新稿件實體機制與根因判定]]（priority high，spec 級設計議題；新印件 vs 新審稿輪次 + 根因判定者）
...
3 張 user-story 卡 v2：
- [[US-AR-003-維護審稿人員能力等級]]：對齊 spec § 審稿人員能力欄位 L22-26 四要素 ActivityLog（時間 / 操作者 / 舊值 / 新值）、成功條件 1 補「必填」、business-logic 補難易度機制
- [[US-AR-004-覆寫印件分派]]：「我希望」縮 31 → 17 字、補批次選取、補離職員工阻擋、補活動紀錄 wiki link、識別 spec 內部不一致（自動破例 vs 覆寫拒絕）
- [[US-AR-008-追蹤部門審稿完成紀錄]]：成功條件 1 拆三條、補對帳明細具體欄位 + Summary Bar 聚合（spec L545-549 + L654）、補 ReviewRound 活動紀錄、「以便」量化「5 分鐘內調出印件」

2 新 OQ：
- [[AR-9-新審稿員建立時能力等級初值]]（priority medium）
- [[AR-10-主管覆寫分派是否允許破例派工]]（priority high，spec 內部不一致）
...
3 張 user-story 卡 v2：
- [[US-AR-002-設定印件難易度與免審稿]]：補 4 條 high gap（必填驗證 / 成交後不可修改 / 免審稿下游連鎖 / 以便邊界）+ frontmatter related-business-logic
- [[US-AR-005-監控當日審稿工作量]]：補 3 高 gap（3 指標計算口徑 / 4 格 vs 3 格邊界釐清 / drill-down 路徑）+ 2 medium 業務化（移除 UI 規格混入）；「我希望」縮為 25 字
- [[US-AR-006-比對審稿人員績效]]：「我希望」縮 32 → 25 字 / 移除「≥ 2 位審稿員」前置誤植 / 成功條件 1 拆三條 / 補「低樣本不標異常」spec L632 核心設計 / 補次排序「件數降冪」+ 平均處理時間「含排隊時間僅作參考」

3 新 OQ：
- [[AR-6-退件率分母是否排除技術退件]]（priority medium，跨 005/006/007）
- [[AR-7-審稿主管監控異常閾值定義]]（priority low，跨 005/006）
- [[AR-8-免審稿適用條件與核可機制]]（priority medium，跨 002）
...
**下一步建議**：
- 持續補 OQ expected-resolution-at（含本日新增 9 個 AR / ORD 系列）— 對應 [[2026-05-20-change-archive-OQ收尾流程缺口]] Action 3
...
**3 張卡處理**：
- [[../13-user-stories/prepress-review/US-AR-001-審核稿件]] **已刪除**；內容移至 [[wiki/erp/07-scenarios/README#情境 14：審稿流程端到端（單模組跨角色）]]
- [[US-CR-001-諮詢單自動建立]] 重寫為「業務查看並指派新諮詢單」（業務動作）；webhook 自動建單機制移至 prerequisites
- [[US-ORD-011-訂單取消與退款]] 重寫為「業務取消訂單與退款申請」（業務動作）；會計動作拆出
- **新增** [[US-ORD-013-會計執行退款處理]]（會計動作；用 prerequisites 連結至 US-ORD-011）
...
- 情境 15「諮詢單自動建立 webhook 串接（跨系統跨角色）」：涵蓋客戶 / 金流平台 / ERP webhook / 業務 / 諮詢人員 5 角色端到端；對應 [[US-CR-001-諮詢單自動建立|US-CR-001]] prerequisites 的跨系統情境
- 情境 16「訂單取消與退款端到端（跨角色多模組連鎖）」：涵蓋業務 / 系統 / 會計 3 角色 + top-down 連鎖；對應 [[US-ORD-011-訂單取消與退款|US-ORD-011]] + [[US-ORD-013-會計執行退款處理|US-ORD-013]] 拆分後的端到端情境
...
- [[scope-boundary|Scope Boundary]] — Vault 收 / 不收
- [[wiki/erp/12-insights/README|12-insights 總覽]] — insight 清單
...
**輸出 / 異動**：
- [[2026-W21]] 新增 § 八「週四之後增量（5/21 18:30 ~ 5/24）」共 5 個 sub-section（8.1 ~ 8.5）
- frontmatter 補 20 個 commit hash（5/21 後段 7 個 + 5/22 共 13 個）、2 個 change archive、1 個 related-vault link（[[user-story-spec]]）
...
**輸出**：[[2026-05-25-SidePanel視覺對齊迭代成本與元件抽象時機]]
...
1. 將 Rule of Three trade-off 寫入 [[ceo-review-framework]]
...
**輸出**：[[2026-05-26-archive工具假設與Vault-Schema對齊缺口]]
...
3. 與 [[2026-05-20-change-archive-OQ收尾流程缺口]] 合併處理 OQ resolve hard gate
4. doc-audit v1.4「Data Model section sync」維度已採納（本次 commit a5bfee5；reactive 防線完成）

**串接**：本 insight 串接既有 [[2026-05-20-change-archive-OQ收尾流程缺口]]（同根因），Action 3 將解雙 insight
...
1. **anti-pattern「role-stage 不匹配」**（會計卡標記僅審稿階段，實際全程）→ 追加到 [[audit-failure-patterns]] 五大反模式之 **Scope creep**（角色卡內容跨越多階段未明示）
...
下列 3 個反模式案例待追加到 [[audit-failure-patterns]]（Phase 2 收尾時補）：
...
**本次 5 卡異動專項**：3 新 OQ frontmatter 全齊 + 無規則違規 + related-change 對齊 + 非孤立；誤審卡四要素齊全 + frontmatter 更新；audit-log pre-check 條目完整。**唯一本次引入問題：CR-5 訂單異動家族 dangling（已即修，改指 [[訂單]]）**。
...
## [2026-06-02] erp-planning-pre-check | L1.6+L1.2 訂單收退款通用方案（訂單收退款模型重構）| 雙領域並行稽核

**議題**：訂單收退款通用方案（退款核銷對帳差額、明細時點分界=訂單完成、OA「已執行」=應收調整生效）。方向書 `~/.claude/plans/1-2-pure-dewdrop.md`。結構性變更、跨 billing-cash + order 雙領域，執行者/稽核者分離（兩稽核 sub-agent + 主對話修補）。

### 雙軸量化矩陣
- L1.6 Billing & Cash：角色 3/2/1、實體 4/3/2、流程 3/4/0、情境 4/3/2、User Story 8/5/3、業務邏輯 2/2/4、法規 1/0/0
- L1.2 Order Management：角色 3/1/1、實體 2/3/2、狀態機 1/1/1、情境 1/1/0、User Story 3/多/1、業務邏輯 4/4/2、Test Case 0/—/0
- 衝擊面 M 集中於流程 / User Story / 實體；法規層 0 衝擊。

### 最關鍵發現（pre-check 攔截訂單收退款模型重構 設計風險）
- **攔截 1（致命）報價鎖定時點**：現有鎖定點=審核通過（[[訂單狀態]] L42 動機「防核可後偷改金額繞過主管」），訂單收退款模型重構後移到訂單完成，推翻 [[ORD-027-訂單額外費用凍結時點與審核通過成交鎖定對齊|ORD-027]]（2026-06-01 resolved）+ 踩 spec L1822（單價製作前可改）vs L3671（報價總額審核通過鎖定）既有矛盾。**Miles 拍板：鎖定點=訂單狀態「訂單完成」，接受推翻 ORD-027。**
- **攔截 2 退款保護降級**：訂單收退款模型重構 拆「OA 已執行=一定退完」單筆硬約束（[[訂單異動規則]] L67/L89/L161）換對帳軟兜底，重開「帳上已退、實際沒退」。**Miles 拍板：照訂單收退款模型重構 + 退款款項已完成當物理錨點 + 差額警示不可忽略。**
- **攔截 3 循環定義**：退款完成←差額←退款完成。破法=退款款項已完成為錨點、差額歸零為對帳結果呈現。
- **攔截 5 明細時點分界全 wiki 無卡**（憑空新增、需真實業務）。**Miles 拍板：先補業務訪談再 propose。**

### 新建 OQ 清單（待 oq-manage 開檔）
- 明細時點分界真實業務操作流程（待訪談：完成前直接增減 / 調降把關 / 已收款減量退款）
- spec L1822 vs L3671 報價鎖定矛盾收斂
- ORD-027（額外費用凍結）re-open / 標被訂單收退款模型重構 取代
- [[BI-9-補收OA立即執行對稱破壞表述|BI-9]] 訂單收退款模型重構 推翻其 invariant 前提，須收斂或併入
- 既有 vault 問題：訂單.md / 訂單狀態.md frontmatter `business-domain` 空值；ORD-027/028 oq-id 編碼碰撞
...
### Step 5 閉環驗證
- 本輪目的=攔截設計跑偏，已達成（攔截 5 項、Miles 決策 3 項、浮現 propose blocker=業務訪談）。M 細項未轉 N（訂單收退款模型重構 待業務訪談 + propose 序列協作定案後回補），非 false completion，延後依據已明示。

### 反模式識別
- 保護降級（訂單收退款模型重構 拆退款硬約束換軟兜底）：已攔截，Miles 決策補強（物理錨點 + 不可忽略）
- 風險誤類比（BI-15 兜底未開票應收 ≠ 訂單收退款模型重構 兜底退款未執行，嚴重度不同）
- 推翻剛 resolved 結論不回收（ORD-027）：已攔截，待 re-open
- 憑空新增概念無 know-how 比對（明細時點分界）：已攔截，待業務訪談
- 主對話 agent 推薦訂單收退款模型重構 過快、未先稽核既有保護：自我識別，由 pre-check 補正

---

## 商業規則迭代史（合併自 business-logic-changelog，2026-06-10）

> 以下內容原在 business-logic-changelog.md，以「卡」為主鍵分節，每節反向時序。

### 訂單異動規則
→ 正本卡：[[訂單異動規則]]

- **2026-06-02**（訂單收退款模型重構）：退款「已執行」認列改為主管核可即生效，移除回退機制，退款金流記在收款款項不掛異動單。
- **2026-05-30**（原子化拆分）：新建正本卡，從三處重複定義收攏為單一正本。

### 明細時點分界
→ 正本卡：[[明細時點分界]]

- **2026-06-02**（訂單收退款模型重構）：新建正本卡，鎖定分界定為訂單完成/已取消雙終態。

### 訂單狀態
→ 正本卡：[[訂單狀態]]

- **2026-05-30**：諮詢取消終態由「訂單完成」改為「已取消」。
- **2026-05-26**：諮詢訂單廢止「已開發票」中間狀態。

### 訂單異動狀態
→ 正本卡：[[訂單異動狀態]]

- **2026-06-02**（訂單收退款模型重構）：狀態走法對齊脫鉤版，移除回退轉換。
...
- **2026-06-03**：折讓退款解耦，移除 refund_payment_id。

### 對帳一致性
→ 正本卡：[[對帳一致性]]

- **2026-06-03**：spec 對帳差額公式對齊本卡三方相等規則。

### Wiki 架構
→ 正本卡：[[erp_index]]

- **2026-06-10**：meta 重構——合併 wiki-architecture 到 erp_index，刪除 5 個過時檔案，合併兩個 log 為 changelog。
- **2026-05-31**：wiki-architecture 由 draft 轉 active，operating-principles 初版定 2 條。
- **2026-06-10**：operating-principles 併入商業規則卡（現金流出把關 + 發票收款彈性），刪除原檔。
