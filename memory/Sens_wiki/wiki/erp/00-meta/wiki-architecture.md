...
- **事後檢查兜底**：一致性靠 `vault-audit` 事後 lint（可加「檢查有沒有繞回自己」+ 偵測 source 指向 OpenSpec 報 Error），不靠寫卡者自律。
...
> 核心做法：**這份範本同時是「寫的時候照著填」和「事後照著檢查」**——每層範本同時是撰寫時照著填，與事後 vault-audit 照著勾。本節定範本清單與共同骨架；歸類決策樹、新增 vs 掛既有判準、change 後增修機制的詳細執行步驟（含 fan-out 稽核）詳見 `wiki-amend` skill。
...
6. **自審稽核點（一份兩用的稽核面）**：範本末段附「Lint 自檢清單」，同一份既是撰寫檢查表也是 vault-audit 稽核維度（對齊 vault-audit 維度 13 / 14 的全面化）。
