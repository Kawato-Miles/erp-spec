...
- 過渡期實證問題：約 12+ 張 business-logic 卡的「來源」段系統性指向 OpenSpec（顛倒），現況約 5 張已遷（04-business-logic 3 + 05-entities 2）有 `source` / `implemented-by`、餘多數仍僅 `related-spec`（即下游當上游的弱版）→ 修法：移到 `implemented-by`（連 Requirement 標題層），真 source 改指營運原則 / 拍板 OQ，補不出的標 `source-gap` 待專輪。
