...
**對應紀律**：業務情境撰寫規範 [[規範 - 業務情境]]（已吸收 INVEST 與兩階段紀律）/ `oq-manage` 禁 anchor 故事
...
**領域**：Billing & Cash
**卡類型**：5. User Story
**修正方式**：批次 backfill `related-test-cases`（從 Notion ERP Test Case DB 配對）；已開 [[BI-12-款項UserStory批次backfill business-domain與related-test-cases]]（已封存）
