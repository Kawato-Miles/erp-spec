...
> **定位**：取代 [[multi-agent-discussion-protocol]] 於 `/opsx:explore` 與 `/opsx:propose` 階段的呼叫；後者降為「最終驗收前審查」（`/opsx:verify` 前），不在新流程觸發。
...
新模型下三個 agent 在 sequential 協議中的角色重新定位（**不影響** [[lightweight-review-mode]] 與 [[multi-agent-discussion-protocol]] 中的審查者角色）：

| Agent | 舊定位（multi-agent / lightweight）| 新定位（sequential）|
|-------|-------------------------------|-------------------|
...
| `/opsx:verify` 前 | 啟動 [[multi-agent-discussion-protocol]]（過渡期保留，最終驗收前審查） |
| `/opsx:archive` 前 | 不啟動本協議，執行 `doc-audit` skill |
