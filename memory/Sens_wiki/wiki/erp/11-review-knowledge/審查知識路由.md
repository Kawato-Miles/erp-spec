...
>
> **2026-08-04 試行**：新增 [[規劃品質評分準則]]——「主對話設計＋rubric 稽核」新流程的量尺正本（配套 skill `plan-design`／`plan-audit`）。回顧性重評通過後將取代序列協作三 agent 模式；試行期間以舊協議為現行。
> **2026-05-28 大改**：[[sequential-design-collaboration]] 重塑為「PM-中介來回」模型（Phase 2/3 上限 2 次來回、Phase 2.5 廢除、Phase 4 加 verify consistency）。CEO / ERP 顧問在 sequential 下角色定位變更（管理層需求提出方 / 統合需求設計者）。新建 [[dispatch-prompt-template]] 規範 sub-agent 呼叫 prompt 5 區塊標準。舊多 Agent 輪次討論協議已於 2026-07-06 除役（verify 前審查由 sequential Phase 4 verify consistency 涵蓋）。
...
11-review-knowledge/
├── README.md                                  入口（本檔）
├── 規劃品質評分準則.md                         規劃設計品質量尺（試行，plan-design／plan-audit 共同正本）
...
│  └─ `/opsx:verify` 前 ──────→ 不另啟動（由 sequential Phase 4 verify consistency 涵蓋）
│
...
統一使用 wikilink：`[[erp-design-patterns]]`、`[[language-conventions]]`、`[[sequential-design-collaboration]]`。
...
序列協作時，協調者額外載入 [[sequential-design-collaboration]] + [[dispatch-prompt-template]]，並依 Phase 編號 + Round 編號傳遞前序輸出全文（依 dispatch-prompt-template 5 區塊組裝）；單 Agent 輕量審查時載入 [[lightweight-review-mode]]。
