...
- [x] 是否更新既有 vault 卡 — 已精練為新參考卡（Miles 指示：流程改 UML、資料結構改 ER-Model、存 wiki）
- [ ] 是否升級為 OQ（觸發 oq-manage mode B）— 無（pre-check 已列 OQ 候選，不重複）
- [ ] 是否累積成 insight（≥ 3 張同主題後觸發 vault-insight）— 保留：生產階段流程重整收斂後再議

## 精練去處（Mode B 完成後填）

- [[Odoo供應鏈參考架構]] — 圖解精練版（4 張 ER-Model + 端到端泳道圖 + 各段流程 UML + 2 張狀態機圖 + 設計原則對照表），2026-07-08 Miles 指示建立
