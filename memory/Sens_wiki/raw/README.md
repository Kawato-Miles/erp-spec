...
**收什麼**：
- Prototype 試用反饋（已被 Miles 確認的觀察）
- Claude 被指派研究後產出（含真實 WebFetch / 對外資料來源）
- Miles 對話中的觀察與設計傾向（未升級為 OQ 但值得記）
- MES / ERP 競品研究筆記（含真實出處）
- Claude 主動建議「值得記」的素材（須 Miles 確認後才寫入）
- **Miles 上傳的檔案 / 截圖**（客戶訪談錄音轉文字 / 會議紀錄 / 廠商規格書 / Figma 註解 / 設計草稿 / Notion export 等；原檔存 `_attachments/`）

**不收什麼**：
- 明確未解問題 → 走 [[wiki/erp/08-open-questions/README|OQ]] 不進 raw
...
`source-slug` 六值（對應 frontmatter `source`）：
- `miles-dialogue` — Miles 對話中的觀察（口頭描述、文字陳述）
- `claude-research` — 被指派研究任務的產出（**raw-source-link 必填**）
- `claude-self-capture` — Claude 對話中主動建議（**須 Miles 確認**）
- `prototype-dogfood` — Prototype 試用反饋
- `mes-study` — 競品 / MES 業界研究
- `miles-upload` — Miles 上傳檔案 / 截圖（**raw-source-link 必填原始出處 + 原檔搬 `_attachments/`**）
...
當 source = `miles-upload` 時：

1. **原檔搬進 `raw/_attachments/<檔名>`**（保留原檔名；若同名衝突在前綴加日期）
2. **raw 卡 frontmatter `attached-files`** 列出檔名（相對路徑，如 `_attachments/富禾印務訪談2026-05-15.docx`）
3. **raw-source-link** 填原始出處（Notion URL / Google Drive / 客戶寄件 / Slack 等）
4. **raw 卡內文「原始素材」section** 由 Claude 讀檔後摘要重點（不是複製貼上全文），並標註「原檔見 [[_attachments/檔名]]」

附件類型：PDF / docx / xlsx / 圖片 / 訪談錄音轉文字 / Notion / Google Doc export / Figma 註解截圖等任意檔。

**大檔案警示（> 10 MB）**：考慮 git-lfs 或外部存放並只在 raw-source-link 留 URL（第一版不強制，但建議避免存大型影片 / 高解析設計檔）。

`_attachments/` 內檔案**不需要 frontmatter**，純粹當「素材原件保存區」。

## 六、Anti-Model-Collapse 四道防線
...
- [[wiki-schema|Wiki Schema]] — type=raw frontmatter 規格、status enum、命名規約
