...
| **並列比較類屬性用 `ErpSummaryGrid` 小區塊 + Tooltip 承載 meta** | 當詳情頁需呈現「同類別並列比較」的資訊（例：工單的目標數量 / 生產數量 / 入庫數量；印件的原始印件檔 / 審稿後印件檔 / 縮圖），用 `ErpSummaryGrid` 橫向小區塊。cell 只顯示最精簡資訊（數字 / 連結 / icon），所有 meta（上傳者 / 時間 / 檔案大小 / 角色 / 異常標記）**以 Tooltip hover 呈現**，不在 cell 內直接列出。目的：密度高、視覺乾淨；使用者需要詳情時 hover 即可，避免資訊過載。|
