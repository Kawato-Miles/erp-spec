...
| **「當前狀態」資料 vs「歷史紀錄」資料應明確區隔** | 同一實體的「當前最新資料」（如印件目前的稿件檔案 / 縮圖）與「歷史紀錄」（如所有審稿輪次、所有異動 log）**必須用不同區塊承載**。當前資料以**基本資訊卡內的 info row**（`ErpInfoTable` label/value 格式）承載，與其他屬性並列；歷史紀錄則放在 Tab 內以表格 / 時間軸呈現。目的：避免「當前」與「歷史」混雜導致誤讀，符合資料 lifecycle 的層級。範例：印件詳情頁 — 基本資訊卡含「稿件檔案」「稿件縮圖」兩個 info row（顯示當前最新檔案 + 下載 + 上傳 meta），審稿紀錄 Tab 承擔歷史輪次 Table。 |
| **info row 格式統一（label / value cell）** | 詳情頁所有屬性資訊（文字、Badge、連結、附件、縮圖）皆以 `ErpInfoTable` 的 label + value cell 格式承載，不自行用卡片 / Grid 拆獨立視覺區塊。value 可為複合 ReactNode（icon + link + meta + thumb + 下載 icon），但維持「左側 label 欄 + 右側 value 欄」的視覺對齊。目的：頁面間 info 呈現一致，使用者掃描時節奏穩定；避免同一屬性在不同頁用不同視覺語言。 |
| **並列比較類屬性用 `ErpSummaryGrid` 小區塊** | 當詳情頁需呈現「同類別並列比較」的資訊（例：工單的目標數量 / 生產數量 / 入庫數量；印件的原始印件檔 / 審稿後印件檔 / 縮圖），用 `ErpSummaryGrid` 橫向小區塊。cell 只顯示最精簡資訊（數字 / 連結 / icon），不含 meta 堆疊。meta（上傳者 / 時間 / 大小等）若需呈現，應放在詳情頁的其他專區或 Tab 內，而非 cell 內。 |
| **欄位定義備註統一用 label 右側 Info icon + Tooltip** | ERP 場景欄位多且語意易混淆，使用者需要隨時確認某欄位的定義。規則：欄位 label 右側以 `Info` icon（`lucide-react`）+ Tooltip 呈現該欄位的**定義說明**（非資料 meta）。Tooltip 內容為「這個欄位代表什麼、來源是什麼、何時填寫」。範例：「原始印件檔」hint = 「客戶 / 業務上傳的原始稿件，審稿人員以此為基礎進行加工」。實作：`ErpSummaryItem` 支援 `hint?: string` prop；`ErpInfoTable` 未來同步擴充。目的：降低欄位認知成本，使用者自助查定義，不需文件或問人。 |
