...
> 規範正本：[[README]]（兩階段紀律 / 命名 / 來源）+ [[wiki-schema#type=user-story]]（frontmatter 正式 schema + 維度 13 lint）+ [[business-logic-writing-guide#4.0 各類卡的單一職責（六層由大到細、只連不重抄）|writing-guide § 4.0]]（各類卡的職責邊界）。本範本是上述三者在 user-story 這層的展開，不衝突。

## 怎麼用這份範本

1. 複製下方 `--- 範本開始 ---` 與 `--- 範本結束 ---` 之間的內容。
2. 存為 `13-user-stories/<module>/US-<MODULE>-<NNN>-<簡述 slug>.md`（命名規約見 [[README#三、命名規約]]）。
...
- **單一角色單一情境**：禁統合多角色 / 多動作的串接故事（跨多角色端到端流程 → [[../07-scenarios/README|07-scenarios]]）。

### 可視情況調整（彈性破口）
- `stage: business-only` 階段 **UI 操作段保持空 / 待補**；Prototype 功能定案後才填，填了 MUST 將 stage 改 `ui-bound` 並補 `ui-binding`（兩階段紀律，見 [[README#二、兩階段撰寫紀律]]）。
...
- [[README]] — 13-user-stories 入口（兩階段紀律 / 命名 / 來源 / 禁串接故事）
