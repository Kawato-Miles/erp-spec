...
| **OpenSpec 部分對應** | 1 | OpenSpec 的「品管」涵蓋 Notion 的 [[審稿]] + [[QC]] 兩個角色 |
| **OpenSpec 缺漏** | 11 | [[Supervisor]]、[[訂單管理人]]、[[會計]]、[[審稿主管]]、[[印務]]、[[生管]]、[[師傅]]、[[QC]]（拆分後）、[[中國廠商]]、[[外包廠商]]、[[EC 商品管理]] |
...
| [[QC]] | 工廠平台 | 製後（打樣印製階段） | 依訂單進行 QC，記錄檢驗結果 |
...
新增 OQ（待 Phase C oq-manage skill 改寫後寫入 `08-open-questions/`）：

1. **角色 / 印務 vs 印務主管邊界**：印務（執行）能直接建生產任務嗎？還是需印務主管批准？
2. **角色 / 會計階段標記**：Notion DB 標記會計「僅審稿階段」是否錯誤？
3. **角色 / 訂單管理人 vs 業務權責邊界**：兩者都有訂單 R/W 權限，工單異動執行責任在誰？
4. **品管拆分**：OpenSpec「品管」是否該拆為「審稿」+「QC」？
