...
> 驗收項目卡（UAT 業務層）。**正文（前置條件 / 測試步驟 / 預期結果）存 Notion ERP Test Case DB，Vault 卡只承載 frontmatter（含往上指依據）+ 相關連結**。撰寫 / 稽核基準正本：[[../15-test-cases/_template-test-case]]（這份範本同時是寫的時候照著填、事後照著檢查）。對齊 [[wiki-architecture#分層體系（營運原則 → 驗收項目，由大到細）]]（驗收項目 = 最具體層）。
