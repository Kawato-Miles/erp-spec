...
### Step 2b 待辦（動到設計 / 結構實質 → 走正式 deltas change + consultant）
- **order-management:917** 發票開立 Requirement 內「發票時序與 PaymentPlan / Payment 解耦…多筆 Payment 對一張 Invoice / 一筆 Payment 對多張 Invoice」= 舊 PaymentInvoice junction 模型 → 與新 1:1 矛盾，改寫（屬發票↔期次 model 重整、非純術語）
- PlannedInvoice（諮詢收尾自動建 / 品項鏈式預填 / 發票時間點）→ BillingInstallment + source_type
- Data Model：移除 PlannedInvoice 表 + ADD BillingInstallment / PaymentAllocation Data Model section
- 入帳機制：spec「依序填滿自動分配 + 自動回填差額 + sum=金額」→ as-built「業務手動勾選 + sum≤金額 + 溢收預收桶」（Miles 拍板手動）
- 發票開票維度 enum：`已作廢回未開立` → `已作廢`
- 新 store actions（addPaymentWithAllocations / updatePaymentWithAllocations / cancelPaymentWithAllocations）+ BillingInstallment.paymentMethod 欄位（prototype-data-store）
- 入帳 UI 元件：PaymentAllocationDialog → BillingInstallmentAllocationTable inline（prototype-shared-ui）

> Step 2b 動到設計實質（手動入帳機制入 spec）+ 結構（Data Model / 發票 model），範圍橫跨 5 份 spec，建議獨立 change + 三視角審查。
