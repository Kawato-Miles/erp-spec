...
# 2026-05-20：售後 ticket 模組的 reactive 補丁循環 — 需端到端走查

## 背景

vault-audit 接續觸發；OQ 累計 32 個（28 open + 4 answered）；售後 ticket 模組在 1.5 個月內連續 5 個 change 落地，每個 change 都浮現新 gap，呈現典型 reactive 補丁模式。senior-pm 在 refine-after-sales-refund-and-add-supplementary-print 前期介入時已預先標識此議題（[XM-004 OQ](../08-open-questions/XM-004-售後流程端到端走查.md)），本 insight 收斂為具體 action 計畫。
...
| 2026-05-20 | `refine-supplementary-print-skip-review` v0.5 | Miles 業務走查發現補印審稿違反業界實務 | （目前未浮現新 gap）|

5 個 change 連續歸檔，每個間隔 < 24 小時，每次都因「Miles 業務走查」浮現新 gap。
...
| [XM-001](../08-open-questions/XM-001-款項管理頁面業務最重要決策.md) | 款項管理頁面決策 | — |
| [XM-004](../08-open-questions/XM-004-售後流程端到端走查.md) | 售後流程端到端走查（本 insight 直接前身）| medium |
| [XM-005](../08-open-questions/XM-005-Use-As-Is 退款流程串接.md) | Use-As-Is 退款流程 | **high** |

### 3. XM-004 OQ 自我預警

OQ 內 senior-pm 已具體指出：「連續四個 change 都在改 ticket 模組，且每次都暴露新的 gap，這暗示規劃時可能少了『ticket 內部端對端流程走查』這個步驟。」

### 4. 缺乏「業務 / 諮詢 / 印務 / 會計」跨角色端對端走查紀錄

Vault 內無「售後 ticket 完整流程跨角色走查」的文件（無對應 [[../07-scenarios/]] 卡）。每次 change 的 Scenario 只覆蓋本 change 範圍，未做整體 user journey 整合驗證。

## 推論

售後 ticket 模組進入「**Miles 走查 → 發現 gap → 開 change → archive → 走查 → 再發現 gap**」的補丁循環。根因有兩層：
...
- [[../08-open-questions/XM-001-款項管理頁面業務最重要決策]]
- [[../08-open-questions/XM-004-售後流程端到端走查]]
