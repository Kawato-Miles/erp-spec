...
> 給撰寫 UAT 業務層驗收項目卡（type=test-case）時複製套用，**這份範本同時是事後檢查的依據（寫的時候照著填，doc-audit / vault-audit 時照著勾）**。
>
> 對齊 [[../00-meta/wiki-architecture]] § 二分層體系（驗收項目 = 最細的一層）+ § 七各層怎麼寫（前提→動作→驗收結果三段）+ [[../00-meta/business-logic-writing-guide#4.0 各類卡的單一職責（六層由大到細、只連不重抄）]]（test-case 職責）+ [[../00-meta/wiki-schema#十一、卡類型內容職責邊界]]。
>
...
複製下方「範本開始 / 範本結束」之間的內容，存為 `15-test-cases/<module>/TC-<MODULE>-<NNN>-<簡述>.md`。`<module>` 對齊 [[../00-meta/wiki-schema#二、module Enum（多選）]]；`<MODULE>` 前綴對齊 [[../00-meta/wiki-schema#七、命名規約]]（QR / ORD / CR / AS / WO / PT / AR / QC / SHP / MM / PM / BM / XM）。
...
| 往下指實作（implemented-by）| OpenSpec Requirement 標題層 / Prototype 端對端測試（Playwright spec）—— 導航 / 覆蓋，不承載對不對 |
| 正文落點 | **Notion ERP Test Case DB**（Vault 不放正文）|
| 撰寫風格 | 前提→動作→驗收結果三段（前置條件 / 測試步驟 / 預期結果）|

依據鏈中本卡的位置（對齊 [[../00-meta/wiki-architecture#完整依據鏈範例（訂單異動補收免審）]]）：
...
> 共同 5 欄對齊 [[../00-meta/wiki-schema#四、各 type 必填 Frontmatter 欄位]]；往上指依據（source）、往下指實作（implemented-by）兩欄對齊 [[../00-meta/wiki-architecture#依據往上、實作往下，連結不繞回自己]]，為這類卡的前進標準。
...
implemented-by:                       # 往下指被誰實作 / 覆蓋，不承載對不對；指 OpenSpec Requirement 標題層 / Prototype e2e
  - "openspec/specs/<module>/spec.md#Requirement: <標題>"
  - "sens-erp-prototype/tests/e2e/<spec>.spec.ts"   # 若已實作
notion-page-url: <Notion ERP Test Case DB 該卡 URL>   # 必填（正文落點，Vault 不放正文）
...
- `implemented-by` 往下指實作 OpenSpec Requirement 標題層 + Prototype e2e；本欄不參與對不對，只供導航。
...
> 共同 4 段（一句話定位 / 這張卡要回答的問題 / 營運背景 / 相關連結 / 來源）對齊 [[../00-meta/business-logic-writing-guide#4.1 共通骨架（business-logic / scenario / role 直接套用）]] 的共同骨架；這類卡特有的前提→動作→驗收結果三段（前置條件 / 測試步驟 / 預期結果）對齊 [[../00-meta/wiki-architecture#各層怎麼寫]]。
>
...
implemented-by:
  - "openspec/specs/<module>/spec.md#Requirement: <標題>"
notion-page-url: <Notion ERP Test Case DB URL>
---
...
> 為什麼需要驗收這件事（連回 user-story 的營運動機，2-3 句、業務語言、無實作術語）。主管 / PM 入口：讀完應能判斷「這個判決點對不對應到我們的生意」。
...
> 對齊 [[../00-meta/wiki-architecture#各層怎麼寫]]。
...
- 正文零迭代史（迭代史進 [[../00-meta/business-logic-changelog]]）。
...
## 六、自審檢查清單（撰寫完逐項打勾，這份清單同時是 doc-audit / vault-audit 的檢查依據）
...
- [ ] `implemented-by` 指 OpenSpec Requirement 標題層 / Prototype e2e（導航用，可暫缺但建議補）。
- [ ] `notion-page-url` 已填（正文落點）。
...
- [ ] 正文零迭代史（迭代史在 [[../00-meta/business-logic-changelog]]）；正文零「待確認 / 待釐清 / 需確認 / 尚未確認 / 待補」inline OQ 措辭（→ [[oq-manage]] mode B）。
...
implemented-by:
  - "openspec/specs/order-management/spec.md#Requirement: <補收 OA 免審>"
notion-page-url: <Notion ERP Test Case DB URL>
---
...
- [[../00-meta/wiki-architecture]] — Wiki 結構與撰寫規範（驗收項目這層的定位、依據往上實作往下、撰寫風格、推動步驟）
- [[../00-meta/business-logic-writing-guide#4.0 各類卡的單一職責（六層由大到細、只連不重抄）]] — test-case 職責（正文存 Notion、source 指 user-story）
- [[../00-meta/wiki-schema#十一、卡類型內容職責邊界]] — 維度 14 lint 依據
- [[../00-meta/wiki-schema#七、命名規約]] — 模組前綴與檔名規約
- [[../13-user-stories/_template]] — 操作步驟（user-story）範本（這類卡 source 往上指依據、以 related-test-cases 回連這類卡的 Notion URL）
- [[../00-meta/business-logic-changelog]] — 迭代史載體（正文零迭代史）
