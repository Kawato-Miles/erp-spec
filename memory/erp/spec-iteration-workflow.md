...
觸發 `oq-manage` skill（模式 B：新增），在 Notion Follow-up DB 建立新 OQ。必填欄位：
- 任務名稱：純問題描述（不含序號）
- 序號：`{MODULE}-{NNN}`（如 `ORD-008`）
- 問題來源：Spec 頁面連結或討論連結
...
## 解決與新增的 OQ
- 解決：{MODULE}-{NNN}, {MODULE}-{NNN}（簡述內容，如 ORD-003, ORD-005）
- 新增：{MODULE}-{NNN}（簡述內容） / 無
