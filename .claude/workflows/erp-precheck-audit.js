export const meta = {
  ...
  description: 'ERP 規劃前 know-how 稽核 fan-out：依領域並行稽核（領域 × 卡類型雙軸）+ schema 結構化輸出 + 對抗式互審找漏 + 彙整修補清單。對應 erp-planning-pre-check skill 的 Step 3，維持「執行者/稽核者分離」（workflow 內 agent 為稽核者，主對話 agent 為修補者）。',
  whenToUse: '規劃 ERP 功能前、跨 ≥2 領域稽核時。單模組局部變動用 skill 既有單 agent 即可，不需此 workflow。',
  phases: [
    { title: '雙軸稽核', detail: '每領域一個 opus 稽核 agent 跑卡類型矩陣（N/M/K）', model: 'opus' },
...
const AUDIT_SCHEMA = {
  ...
  properties: {
    ...
    matrix: {
      type: 'array',
      description: '卡類型雙軸矩陣，每格 N/M/K 數字',
      items: {
        type: 'object',
        additionalProperties: false,
        properties: {
...
3. 對此領域 × 卡類型（角色 / 實體 / 流程 / 業務情境 / 業務邏輯 / 法規）逐格稽核，每格給「已涵蓋 N / 待修補 M / 待確認 OQ K」明確整數（禁「大致 OK」非量化結論）。
