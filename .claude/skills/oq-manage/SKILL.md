...
**所有討論 Spec / 規格設計延伸的 OQ，新增時預設 `內外部 = 內部`。**

- 目的：避免與外部廠商 / 客戶 / 跨部門討論時，混入僅限內部研發釐清的設計細節
- 適用範圍：所有 erp-spec / opsx:explore / opsx:propose / 其他設計討論新增的 OQ
- 例外：僅當 Miles 明確指示「這條要讓 X 外部回答」或 OQ 主體屬於外部決定（如：客戶需求確認、廠商技術回覆）時才設為「外部」
- 實作：Step B3 建立頁面屬性清單必含 `內外部: 內部`（除非 Miles 明確改）
...
屬性：
  ...
  - 狀態：未開始
  - 優先順序：高 / 中 / 低
  - 內外部：內部（預設值，見 § 預設屬性規則；外部需 Miles 明確指示）
...
> 其他資源（模組 BRD / 業務情境 / 商業流程等）的引用格式，統一參照 CLAUDE.md § 跨頁面引用規則（原則 #11）；各資源 URL 見 `memory/shared/notion-index.md`。
