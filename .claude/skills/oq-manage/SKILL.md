...
2. **去重搜尋**：載入 skill `obsidian-cli` 搜平層＋`_archives/` 全部 OQ 的標題與內文關鍵詞（指令與參數語法以該 skill 為準；CLI 不可用時才退 grep）；封存卡也搜——已拍板過的問題不重開，引用舊決議。
...
1. Miles 拍板 → 卡內補「決議」段（格式見規範 - OQ § 3.6：結論＋理由＋落地去處逐卡）。**決議落地到正本卡的寫入動作轉介 `wiki-amend`**，本 skill 只管 OQ 卡本身。
...
| 情境 | 協作 |
|------|------|
| wiki-amend | mode C 決議的落地去處若是正本卡，寫入動作轉介 `wiki-amend`；本 skill 不動 03／04／05／06／07 卡 |
