...
| **直接 spec 編輯（非經 OpenSpec change / archive）改動商業邏輯**（欄位 / 狀態 / 流程 / 對帳 / 關聯）| 強制執行 Step 2「直接 spec 編輯後 wiki 商業邏輯卡對齊」維度（見下） |
| 純 OQ 文字更新、措辭修正 | 跳過 |
...
| 直接 spec 編輯後 wiki 商業邏輯卡對齊 | 與「archive 後 wiki 對齊」同理，但觸發於**未經 OpenSpec change / archive 的直接 spec 編輯**（如修正既有 spec 的欄位 / 狀態 / 對帳公式 / 移除實體關聯）。此路徑無 archive sync、也無 change delta 可循，最易漏對齊 wiki——同一商業概念（欄位 / 規則 / 關聯）常散落多個 openspec spec 檔 + 多張 wiki 商業邏輯卡（含 `04-business-logic/` 連帶矩陣 / scenarios 卡 / `13-user-stories/` 校對紀錄），改一處易漏其餘。**MUST 做法**：(1) 以本次改動的關鍵 token（欄位名如 `refund_payment_id` / 規則關鍵字如「累計推進」）`grep -rn` 全 openspec/specs/ + 全 ERP_Vault 商業邏輯目錄，撈出所有殘留；(2) 逐處判斷是 stale active 引用（須對齊）或合法歷史（changelog / user-story 校對紀錄等 append-only 層保留，不改寫，必要時追加新一輪取代記錄）；(3) 同步更新被改卡 frontmatter `last-reviewed` + 補 `business-logic-changelog` 對應卡節。**易漏點**：spec 改完只看 spec、忘了 wiki 卡的連帶矩陣與 scenarios 仍寫舊欄位 / 舊規則（本維度即 2026-06-03 折讓退款解耦 + route-C 累計推進殘留跨 6 卡稽核教訓）|
...
| v1.8 | 2026-06-03 | 新增觸發時機「直接 spec 編輯（非經 change / archive）改動商業邏輯」+ Step 2 新增「直接 spec 編輯後 wiki 商業邏輯卡對齊」維度 | 折讓退款解耦（移除 `refund_payment_id`）以直接 spec 編輯方式進行（非經 OpenSpec change / archive），既有「archive 後 wiki 對齊」維度不涵蓋此路徑；本次靠主動 grep 才抓到殘留跨 4 spec + 6 wiki 卡（payment-invoice-scenarios / 發票法規硬約束 / 付款發票邏輯 連帶矩陣 / US-ORD-030 校對紀錄，外加 route-C「累計推進」2026-06-02 漏清殘留）。直接 spec 編輯無 archive sync 也無 delta 可循、最易漏 wiki，故補此維度強制以關鍵 token grep 全 openspec + 全 ERP_Vault 商業邏輯目錄、逐處判 stale active vs 合法歷史、同步更新 last-reviewed + changelog |
