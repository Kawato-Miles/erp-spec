...
description: >
  三視角審查 Agent（senior-pm / ceo-reviewer / erp-consultant）誤審記錄管理 skill，累積誤審案例庫讓 agent 自學避免重複犯錯。正本：Vault `11-review-knowledge/` 三個誤審卡。
  觸發：Miles 反駁 agent 審查建議（「這不對」「審錯了」「以後別這樣審」「方向錯了」「規則太學術」）、說「記下來」「這是誤審」「補誤審案例」，或主動收尾時識別到本次討論發生過誤審。識別到誤審 MUST 觸發本 skill 建檔，禁止只口頭說「下次會注意」。
  不適用（見本文）：一般 spec 修訂建議、好建議但 Miles 不採、未確認的「可能誤審」、純口語措辭調整。
...
| `vault-audit` | 上游：若同類誤審累積過多（如同一 agent 一個月內 5 次以上類似誤審），audit 的建議段可點出為系統性議題 |
...
- 若同類誤審月頻率不降反升，於 vault-audit 建議段點出為系統性議題
