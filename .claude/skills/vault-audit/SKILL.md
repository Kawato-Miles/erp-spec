...
檢查兩部分：
1. **本輪異動的正本卡**（範圍取法同維度 10）：依卡的 type 載入對應規範的稽核維度表逐項勾——entity → `規範 - 實體` § 十、role → `規範 - 角色`、state-machine → `規範 - 狀態機`、scenario → `規範 - 業務情境` § 十二、service-blueprint／business-rule → `規範 - 商業邏輯` § 十三；以該單元「範例 - 」卡為通過樣本對照。存量未異動卡不掃（避免噪音，隨異動逐步覆蓋）。
