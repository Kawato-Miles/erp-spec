...
description: >
  ...
  正本方法論：[[erp_index]]（位階 / 溯源 / 引用方向）+ 04-business-logic/_template-business-logic.md（§4.0 原子化職責 / §4.1 共通骨架）+ [[wiki-schema]]（§四 frontmatter / §十一 內容職責邊界）+ [[changelog]]（卡為主鍵迭代史）。
  觸發時機：
    1. explore 定案後、propose 之前，判斷涉及商業邏輯變動時
    2. Miles 說「更新 wiki」「對齊 wiki」「wiki 回補」「先把商業邏輯寫進 wiki」
  範圍：**只處理「商業邏輯變動增修進 wiki 各位階」**。
  輸出：對話報告（建議 + 影響分析 + 優缺點三件套）+ 增修 / 新增 wiki 卡 + 同步 [[changelog]] + 標明哪些卡被異動。
...
本 skill 把「explore 定案後的商業邏輯增修進 wiki」做成**結構性流程**：歸類決策樹判位階 → 涵攝邏輯判歸既有卡 vs 新增 → 依各位階 Template 撰寫 / 迭代 → 自審稽核 → 標明哪些卡被異動 + 同步 changelog。
...
| 驗收項目（test-case）| Notion 正文 + Vault frontmatter related-test-cases 回連 | Vault 內只更新連結，正文在 Notion 改 |

### Step 4：同步 changelog（卡為主鍵）

在 [[changelog]] 對每張被異動的正本卡 section 追加一筆：**日期 +（explore 議題識別）+ 一句話變更 + 動機**，反向時序（最新在上）。正本卡正文零迭代史，「來源」段只留 `迭代脈絡見 [[changelog]]`。

> changelog 的雙向反查：
> - **議題 → 卡**（哪個議題改了哪些卡）：全檔 grep 議題識別跨 section 聚合。
> - **卡 → 議題**（這張卡被哪些議題改過）：該卡在 changelog 的 section（反向時序）。
...
| 「某具體輸入下實作後該驗出什麼結果」（UAT 業務層）| 是 | 驗收項目 | test-case（frontmatter + 連結在 Vault、正文在 Notion）| `15-test-cases/<module>/`（待建）|
...
- 若源自某 explore 議題 → changelog 開該卡 section 記首筆。
...
5. `## 來源`：連回可獨立驗證外部出處；末行 `迭代脈絡見 [[changelog]]`。
...
| 驗收項目 / test-case | `15-test-cases/<module>/`（待建）+ `_template-test-case.md`（待建）| 前提→動作→驗收結果三段：`## 前置條件`（觸發條件，可觀測具體值）+ `## 測試步驟`（業務動作，禁 UI 點擊措辭）+ `## 預期結果`（可勾稽驗收值，非「做了沒」）。**Vault 內只放 frontmatter + source（往上指 user-story）+ implemented-by + 連結；正文存 Notion**。happy path 與 edge case 實體分區、MECE、明確結束點 | Vault 內放正文（正文在 Notion）/ SIT / UT 技術測試 |
...
- **議題 → 卡**：[[changelog]] 全檔 grep 議題識別跨 section 聚合（卡為主鍵分節，每節記議題識別）。
- **卡 → 議題**：該卡在 changelog 的 section（反向時序）。
...
- **CLAUDE.md § 主動收尾第 10 條**：change archive 後 wiki 對齊由本 skill 機制保障；但主要觸發時機已提前至 explore 定案後。
...
| 項目 | 現況 | 落地依據 |
|------|------|---------|
| `operating-principle` type | [[wiki-schema]] 現用 `product-vision` type 承載營運原則層；`operating-principle` 曾為候選**目標 type 名**，已決議沿用 `product-vision`、不新增此 type（記入 [[changelog]]）| [[erp_index]]（營運原則層沿用 product-vision type 之註）|
| `01-products/operating-principles.md` | **尚未建立**（現有 product-vision.md / success-metrics.md 等）| [[erp_index]] |
| `15-test-cases/<module>/` 目錄 | **尚未建立**（test-case 正文現於 Notion）| [[erp_index]] |
| 各位階 `_template-*.md`（business-logic / state-machine / scenario / role / entity / test-case / operating-principle）| **尚未建立**（僅 user-story `_template.md` 既有）| 04-business-logic/_template-business-logic.md § 四範式（待落地為獨立 template 檔）|
