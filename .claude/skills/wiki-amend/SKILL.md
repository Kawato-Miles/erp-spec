...
description: >
  ...
  硬性載入清單、落點判定、輸出格式（落卡產出 + log.md）見本文。
---
...
| 順序 | 檔 | 拿什麼 |
|------|----|--------|
| 1 | [[卡片撰寫共用規範]] § 二 | 寫卡流程（辨識 → 歸位 → 確認前提 → 撰寫 → 稽核 → 定案）與各步卡住時的出口 |
| 2 | 同檔 § 三、§ 四、§ 四之二 | 停下鐵則、撰寫紀律、L1 共用稽核維度 |
...
|----------------|----------------------------|
| **規劃 ERP 功能前**的 know-how 缺漏稽核（補既有真實狀況）| `erp-planning-pre-check`（雙軸：領域 × 卡類型）|
| **寫或改任何一張正本卡**（03／04／05／06／07）| **本 skill（wiki-amend）**，其他 skill 一律轉介進來 |
| ERP_Vault **整體健康檢查**（稽核維度，定期 / 週期）| `vault-audit`（唯讀，要改卡轉介本 skill）|
...
### Step 2：變更影響分析，攤開回補清單

沿 directional anchor 反查，**列出本次變動的回補清單**（Jama 變更影響分析法 + Sens 連帶矩陣）：

1. 沿七實體連帶矩陣（[[付款發票邏輯#五B、七實體連帶矩陣]]）攤開連帶影響範圍。
2. 沿正本卡 backlink 反查所有 wiki link 引用該規則的下游卡（規則卡 / state-machine / entity / role / scenario）。
3. 量化每個受影響領域（雙軸：領域 × 卡類型，每格 N/M/K 數字；**禁「大致 OK」非量化結論**，對齊 CLAUDE.md）。
...
依新內容回答的問句性質，對照位階單一職責（04-business-logic/規範 - 商業邏輯.md § 二 分類判斷表、[[erp_index]]）：
...
- **流程・狀態・角色・資料擇一**（依 [[wiki-schema]] § 十一 卡類型內容職責邊界）：
...
- 一條規則的組成要素（觸發條件 + 審核路徑 + 終態 + 不變條件 invariant）MUST 留同一張卡，不可再拆。
...
跨卡型共用的治理（寫卡流程、停下鐵則、撰寫紀律、L1 稽核維度、分層同 commit 鐵則）正本在 [[卡片撰寫共用規範]]。
...
> 每筆增修（無論掛既有或新增）MUST 在對話中附落卡產出給 Miles 決定，**不可逕自寫入既有卡 / 新建卡**。沒把握判斷要明說「不知道、背景不足」，**不假裝**（對齊全域偏好「agent 無把握時明說」+ vault-ingest 防線「禁 LLM 自迭代」）。
...
| 稽核維度 | L1 共用維度 ＋ 該卡型的 L2／L3 全通過 | 同一份維度也是 vault-audit 維度 12 逐項勾的依據 |
...
- **`vault-audit`**：唯讀稽核（稽核維度）；mode C 要改卡時轉介本 skill。兩者一寫一讀。
