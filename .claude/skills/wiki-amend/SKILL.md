...
| 業務情境（scenario）| 一卡一目標完成過程（對應情境 section）| 變體判定 → 六段 → 13 維度自審；改角色傳遞 / 狀態鏈，每步規則 wiki link 指 business-logic，不複寫 |
