...
### Step 2：讀取 OpenSpec Spec（Scenarios + Data Model）

**必讀（最小必要）：**

1. **對應模組 OpenSpec spec**（`openspec/specs/{模組}/spec.md`）
   - 重點：Purpose（問題/目標/範疇）、Requirements 內的 Scenarios（WHEN/THEN）、Data Model
   - 從 Scenario 的行為描述提取 TC 驗收標準
2. **業務情境 spec**（`openspec/specs/business-scenarios/spec.md`）
   - 端到端狀態追蹤表，用於設計跨模組流程的 TC

**不需額外讀取**：狀態機、Notion BRD（OpenSpec spec 為正本，Notion 為發布版本）
...
| ERP Scenario DB（Test Scenario 分組） | https://www.notion.so/2b93886511fa817fbb7ff9d2b37b9e05 |
| 各模組 OpenSpec spec（正本） | `openspec/specs/{模組}/spec.md` |
| 業務情境 spec（端到端狀態追蹤） | `openspec/specs/business-scenarios/spec.md` |
| 使用者角色（角色權責） | `openspec/specs/user-roles/spec.md` |
