...
- `miles` → 必為 `miles-dialogue` 或 `prototype-dogfood`（Miles 走查回饋）
...
Step 3：寫入 raw 卡
  - 複製 raw/_template.md
  - 填 frontmatter（type/status/created-at/source/captured-by/module/topic-tag/related-vault/raw-source-link）
  - 填「原始素材」section（一字不漏的 raw 內容，不要加自己的解釋）
...
### 範例 G：Anti-Model-Collapse 試煉（拒絕場景）
