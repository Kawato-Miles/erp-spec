...
description: >
  ...
  寫入位置：Vault `memory/erp/ERP_Vault/raw/`（2026-05-21 新增，採 Karpathy LLM Wiki raw 層概念）。
  觸發時機：
...
ERP 規劃是持續迭代過程：Prototype 走查、競品研究、Miles 對話中的觀察會源源不絕產生。若直接寫入既有 12 編號目錄，會混入未驗證內容；若不寫，則素材散落各處日後遺失。
...
### 4.1 目錄
`/Users/b-f-03-029/Sens/memory/erp/ERP_Vault/raw/`
...
- `miles` → 必為 `miles-dialogue` / `prototype-dogfood` / `miles-upload`（Miles 走查回饋或上傳）
...
| 「QuoteListPage 在 64px 行高時，狀態卡點擊區域變小」走查觀察 | **raw**（vault-ingest mode A） | 是已驗證觀察，待精練成 UI 規範或 spec 更新 |
...
Step 3：寫入 raw 卡（+ 附件搬檔，若 source=miles-upload）
  - **若 source=miles-upload**：
    1. 把原檔搬進 `memory/erp/ERP_Vault/raw/_attachments/<檔名>`
...
Step 2：對照 scope-boundary 判斷升級路徑
  讀 memory/erp/ERP_Vault/00-meta/scope-boundary.md § 三判斷表，判斷該升級為：
...
Step 5：觸發 vault-insight（若 step 2 判定累積到 insight 級別）
  ...
  - vault-insight 自己會寫 audit-log（`insight` 標）
...
| oq-manage 完成後 | oq-manage 自己寫 audit-log（`oq` 標）|
...
| audit-log 寫入 | vault-audit 自己寫（`audit` 標）|

### 10.4 misjudgement-record

**完全不交叉**。誤審記錄不存 raw，raw 不放誤審。誤審 skill 自己寫 audit-log（`misjudgement` 標）。

### 10.5 openspec change archive

| 銜接點 | 怎麼做 |
|--------|-------|
| change archive 後 | archive skill 寫 audit-log（`change-archive` 標）|
...
Miles 走查 prototype 後說：「QuoteListPage 在 64px 行高時，狀態卡的點擊區域變得很難按，我要記下來。」
...
5. **Step 4**：追加 audit-log（`ingest-A`）
6. **Step 5**：回報
   > 「已存 raw。候選升級路徑：可能更新 DESIGN.md § 6.1（UI 範疇）或 prototype-shared-ui spec。建議下次走查再累積 2-3 條 UI 反饋後一起精練。」
...
3. Step 4：產出報告
4. Step 5：追加 audit-log（`ingest-C`）
...
6. **Step 6**：執行寫入 spec + 更新 raw 卡 status=ingested + 追加 audit-log（`ingest-B`）
...
3. **Step 2**：分析
   ...
   - 既有 raw 無同主題（grep 通過）
4. **Step 3**：
   - 搬檔：`memory/erp/ERP_Vault/raw/_attachments/富禾印務訪談2026-05-15.docx`
...
5. **Step 4**：追加 audit-log（`ingest-A`，標 source=miles-upload）
6. **Step 5**：回報
   > 「已存。客戶訪談涉及 3 個痛點，建議累積 2-3 場訪談後跑 Mode B 拆解成 [[../01-products/erp/pain-points]] 與對應 [[../04-business-logic]] 卡。原檔保留在 `_attachments/`。」
...
- [[../../memory/erp/ERP_Vault/raw/README|raw/ 入口導覽]]
- [[../../memory/erp/ERP_Vault/raw/_template|raw 卡模板]]
- [[../../memory/erp/ERP_Vault/00-meta/wiki-schema|Wiki Schema]] § 一 type Enum / § 三 status Enum / § 四 type=raw frontmatter / § 七命名規約
- [[../../memory/erp/ERP_Vault/00-meta/scope-boundary|Scope Boundary]] § 一 / § 三判斷準則
- [[../../memory/erp/ERP_Vault/00-meta/vault-charter|Vault Charter]] § 一 升級說明
- [[../../memory/erp/ERP_Vault/00-meta/audit-log|Audit Log]] — 所有 ingest 操作記錄於此
