...
Step 4：追加 wiki/log.md 一筆（動作=納入、標籤=ingest-A）
  追加至 memory/Sens_wiki/wiki/log.md（最新在上，加在檔首說明分隔線下方）：
  ## [YYYY-MM-DD HH:MM] 納入(ingest-A) | <source> | <主題簡述>
  - 變更：新增 raw 卡 [[../raw/<檔名>]]
  - 動機：納入素材免填（納入類）
  - 衝突：無
...
Step 4：觸發 oq-manage mode B（若 step 3 識別 OQ 候選）
  ...
  - oq-manage 自己寫 wiki/log.md（`oq` 標）
...
Step 5：觸發 vault-insight（若 step 2 判定累積到 insight 級別）
  ...
  - vault-insight 自己寫 wiki/log.md（`insight` 標）
...
Step 6：Miles 確認後執行寫入（**這步必須等 Miles 明確說 OK**）
  ...
  - 追加 wiki/log.md 一筆（動作=納入、標籤=ingest-B）：
    ## [YYYY-MM-DD HH:MM] 納入(ingest-B) | <主題簡述>
    - 變更：[[<更新的既有卡>]] 補入 raw 內容、[[../raw/<檔名>]] status 轉 ingested（如有新 OQ / insight 一併列出）
    - 動機：納入素材免填（納入類）
    - 衝突：無
```
...
Step 5：追加 wiki/log.md 一筆（動作=納入、標籤=ingest-C）
  ## [YYYY-MM-DD HH:MM] 納入(ingest-C) | 批次掃描
  - 變更：raw/ 全掃，產報告（不動檔）；待處理 N 張、處理中 M 張、警示 K 條
  - 動機：納入素材免填（納入類）
  - 衝突：無
```
...
| oq-manage 完成後 | oq-manage 自己寫 wiki/log.md（`oq` 標）|
...
| log.md 寫入 | vault-audit 自己寫 wiki/log.md（`audit` 標）|

### 10.4 misjudgement-record

**完全不交叉**。誤審記錄不存 raw，raw 不放誤審。誤審 skill 自己寫 wiki/log.md（`misjudgement` 標）。

### 10.5 openspec change archive

| 銜接點 | 怎麼做 |
|--------|-------|
| change archive | change archive 是 PRD 層事件，不動 wiki/、不寫 log.md；wiki 卡若需配合業務決策調整，依 BRD 先行原則在 propose 前已完成 |
...
5. **Step 4**：追加 wiki/log.md 一筆（納入 / `ingest-A`）
6. **Step 5**：回報
...
3. Step 4：產出報告
4. Step 5：追加 wiki/log.md 一筆（納入 / `ingest-C`）
...
6. **Step 6**：執行寫入 spec + 更新 raw 卡 status=ingested + 追加 wiki/log.md 一筆（納入 / `ingest-B`）
...
5. **Step 4**：追加 wiki/log.md 一筆（納入 / `ingest-A`，標 source=miles-upload）
6. **Step 5**：回報
...
- [[../../memory/Sens_wiki/wiki/log|操作史]] — 所有 ingest 操作記錄於此（全知識庫唯一只追加操作史）
