...
description: >
  ...
  **強制規則（禁止以下 anti-pattern）**：
    ...
    4. 禁與 changelog 重複（複述既有事件時引用而非重寫）
    5. 禁編造（git log / changelog / OpenSpec / Vault 沒寫的事 MUST NOT 出現在 brief）
...
```bash
grep -A 3 "^## \[$(date -v-1d +%Y-%m-%d)" /Users/b-f-03-029/Sens/memory/Sens_wiki/wiki/erp/00-meta/changelog.md
```
...
Step 5：寫入 daily 卡 + 追加 changelog
...
| 與 changelog 重複 | 強制規則 4 | 把 changelog 整段複製進 daily | 引用「昨日 changelog 條目 [[../00-meta/changelog#2026-05-20]]」+ 摘要 1 句 |
...
| Skill | 銜接 |
|-------|------|
| `vault-audit` | daily 不觸發 audit。但若 changelog 維度顯示閾值達標（≥ 5 卡異動 / ≥ 15 OQ）→ 在「警示與提醒」段引用 |
...
2. **Step 1**：平行收集 6 個資料來源
   ...
   - changelog：5 個 ingest-A / ingest-B / change-archive 事件
...
4. **Step 4**：產出昨日摘要
5. **Step 5**：寫入 `14-reviews/daily/2026-05-21.md` + 追加 changelog
...
| 「持續推進 X」（無下一步）| § 七 無下一步 |
| 把 changelog 整段複製 | § 七 與 changelog 重複 |
...
- [[../../memory/Sens_wiki/wiki/erp/00-meta/changelog|Audit Log]] — daily-brief 寫入記錄
