...
description: >
  ...
  正本框架：`memory/erp/ERP_Vault/00-meta/erp-planning-audit-framework.md`
  領域分類：`memory/erp/ERP_Vault/00-meta/business-domain-taxonomy.md`（6 領域 + 跨領域共用層）
  ...
  不適用：純查詢術語 / 狀態機（不涉及規劃）、Vault 整體健康稽核（用 vault-audit）、OpenSpec spec 稽核（用 doc-audit）。
...
- 不處理 OpenSpec spec 層稽核（屬 doc-audit）
...
**觸發詞 → 領域 mapping**（依 [[../../memory/erp/ERP_Vault/00-meta/business-domain-taxonomy]] § 二）：
...
- `01-products/erp/`（vision / phases / kpi / impact-score）
...
**產出「wiki 商業邏輯卡清單 + 定案後回補清單」（2026-05-30 新增，交棒 archive 階段）**：
...
- **pre-check 階段不修這些卡**（本次規劃尚未定案，提前改卡屬 premature documentation，且設計可能在序列協作 / verify 階段翻轉）
- 但 **MUST** 產此清單作為「定案後回補清單」交棒 archive 階段執行（補「pre-check 不修卡 → 定案後回補」的配對步驟，converge change 漏對齊 wiki 6 卡的教訓 — 規劃時沒列出受影響 wiki 卡，archive 後也就漏回補）
- 此清單與「Step 4 修補既有卡」的區別：Step 4 修補的是「現況 know-how 缺漏 / 錯誤」（補既有真實狀況）；本清單列的是「本次設計定案後將被改寫的 wiki 商業邏輯卡」（等定案才動，由 `doc-audit` skill archive 後雙層稽核維度執行回補）
...
wiki 商業邏輯卡清單（涉及本主題、定案後回補）：<列出 04-business-logic / 05-entities / 06-state-machines / 07-scenarios / 13-user-stories 內受影響卡；pre-check 不修，交棒 archive 階段由 doc-audit 回補>

下一步：依稽核結果修補既有卡 + 標 OQ + Step 5 閉環驗證；wiki 商業邏輯卡待本次設計定案後回補（不在 pre-check 修）
```

### audit-log.md 追加

每次稽核完成 MUST 追加到 `memory/erp/ERP_Vault/00-meta/audit-log.md`：
...
### wiki 商業邏輯卡清單（定案後回補，交棒 archive 階段）
...
詳見 `memory/erp/ERP_Vault/11-review-knowledge/_shared/audit-failure-patterns.md`（規劃中）。
...
| `doc-audit` | OpenSpec spec 稽核 — 本 skill Step 6 跨層影響檢查時參考 |
...
| v1.1 | 2026-05-30 | Step 4 + ack 模板 + audit-log 模板新增「wiki 商業邏輯卡清單 + 定案後回補清單」產出（pre-check 不修卡、交棒 archive 階段由 doc-audit 雙層稽核回補）。原因：converge change 漏對齊 wiki 6 卡 — 規劃時沒列受影響 wiki 卡，archive 後也就漏回補；補「pre-check 不修卡 → 定案後回補」配對步驟 |
