...
- 此清單與「Step 4 修補既有卡」的區別：Step 4 修補的是「現況 know-how 缺漏 / 錯誤」（補既有真實狀況）；本清單列的是「本次設計定案後將被改寫的 wiki 商業邏輯卡」（等定案才動，由 archive 後回補流程執行）
...
wiki 商業邏輯卡清單（涉及本主題、定案後回補）：<列出 04-business-logic / 05-entities / 06-state-machines / 07-scenarios / 13-user-stories 內受影響卡；pre-check 不修，交棒 archive 階段回補>
