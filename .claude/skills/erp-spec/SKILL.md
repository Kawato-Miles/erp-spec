...
ERP 文件中「Phase」有兩個層級，必須明確區分：

| 層級 | 定義 | 正確寫法 | 錯誤寫法 |
|------|------|---------|---------|
| **商業 Phase**（產品路線圖）| 產品目標 § 三 定義的整體開發階段（Phase 1 = BOM、Phase 2 = 線下訂單、Phase 3 = EC 整合）| `商業 Phase 2`、`商業 Phase 2 上線後` | `Phase 2 上線後`（不加前綴，與模組 Phase 混淆）|
| **模組 Phase**（功能迭代）| 單一模組內的功能分批交付（如需求單模組的第一批 / 第二批功能）| `需求單模組第一版`、`訂單模組功能第二批（暫定）` | `Phase 1`、`Phase 2`（無模組名稱前綴）|

**規則**：
1. 跨模組文件（如產品目標 § 五、Roadmap）引用開發階段，一律用 `商業 Phase X`
2. 模組 Spec 內的功能迭代，用「第一版」/「第二版」或「第一批功能 / 第二批功能（暫定）」，避免單獨使用 Phase 數字
3. KPI 的目標期限一律寫完整時間描述：`商業 Phase 2 上線後第 N 個月`，不簡寫為 `Phase X 上線後 N 個月`
