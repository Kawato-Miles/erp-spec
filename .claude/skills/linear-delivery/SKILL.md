...
description: >
  把已定案的規格交付到 Linear（project 描述 + Task issue），依自包含標準模板（v2.0）產出，交付前以 Rubric 評分稽核（執行者與評審分離、合格 10 / 10）才發布。對開發而言 Linear 為交付正本、wiki 為欄位與狀態正本、Prototype 為介面與互動正本（交付附參照）；OpenSpec 為 PM 內部工作版本不外露。
...
- **正本邊界**：對開發而言，**Linear 為交付正本（內容自包含）＋ wiki 為欄位與狀態正本（開發團隊直接查閱實體卡與狀態機卡，交付不複寫）＋ Prototype 為介面與互動正本（操作動線與版型以 Prototype 為準，交付附參照與邊界表，不文字重述）**；Notion 已停用為 BRD。OpenSpec spec 是 PM 內部工作版本，交付內容 MUST NOT 外露 `openspec/...` 路徑。
...
| 元素 | 本 skill 的填法 |
|------|----------------|
| **Outcome（最終成果）** | Linear project 描述（What）+ Task issue（How）+ 狀態機 UML + v2.0 結構件（Prototype 參照／邊界段／測試決策章／驗收測試段／Blocked by／milestone）齊備，且 Rubric 符合性評分達**合格分數 10 / 10**（由與實作者分離的評分 sub-agent 判定，非實作者自判）|
...
- **project 描述 = What 層（薄）**：概述（商業目標＋痛點，源 wiki）/ Use Case（業務敘事，源 wiki）/ Key Feature（Scope 主要／次要功能巢狀＋Out of Scope）/ Prototype 參照邊界 / 各 Feature 章（Prototype 參照＋Function 節＝商業邏輯＋驗收條件）/ 測試決策（受測介面 2-4 個）/ 另案處理 / 狀態機。操作方式與介面呈現歸 Prototype 參照、實作規格下放 Task 票、欄位歸 wiki 實體卡；細則見 references/delivery-template.md v2.0 與 docs/superpowers/specs/2026-07-07-linear-delivery-architecture-design.md
...
自審 MUST 含：有業務可觀測結果的功能是否已附驗收條件、驗收條件是否 outcome 導向且二元可勾稽、是否誤把通用品質門檻（Definition of Done）混入驗收條件、驗收條件條目是否殘留內部路徑或檔名；以及 v2.0 結構件——有介面的 Feature 是否附 Prototype 參照、Prototype 參照邊界段與測試決策章是否在、每張票是否有驗收測試段（每條標受測介面）與 Blocked by 段、是否誤開設計票。**結構性交付缺 v2.0 結構件者不得發布（由 D5 承接評分）。**
...
| 5 | 交付結構完整性 | v2.0 結構件齊備：Prototype 參照／參照邊界段／測試決策章／驗收測試段／Blocked by／milestone，且不另開設計票 | 一般 |
