...
- **project 描述 = What 層（薄）**：概述（商業目標＋痛點，源 wiki）/ Use Case（業務敘事，源 wiki）/ Key Feature（Scope 主要／次要功能巢狀＋Out of Scope）/ 各 Feature 章（Function 節＝商業邏輯＋驗收條件兩段）/ 另案處理 / 狀態機。操作方式歸 DE 票、實作規格下放 FE / BE 票、欄位歸 wiki 實體卡；細則見 references/delivery-template.md v1.9 與 docs/superpowers/specs/2026-07-07-linear-delivery-architecture-design.md
- **Task issue = How 層**：DE 票（操作方式＋UI / UX，每個有介面的 Feature 一張）；FE / BE 票（實作 checklist，含自 project 下放的演算法細節、欄位、事件）；概述指回「Key Feature：<功能名>」章
