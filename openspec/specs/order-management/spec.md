...
- **AND** 諮詢單狀態 SHALL 從「已轉需求單」更新為「完成諮詢」（最終結局）
...
訂單狀態位於審稿段（稿件未上傳 / 等待審稿 / 待補件）時，訂單狀態 SHALL 由其下所有印件的審稿狀態派生，依以下優先序（4 條規則）：

1. 若存在任一印件審稿狀態 = 「不合格」 → 訂單狀態 = **待補件**
2. 否則，若所有印件審稿狀態 = 「合格」 → 訂單狀態 = **製作等待中**（進入製作段）
3. 否則，若存在任一印件審稿狀態 = 「等待審稿」或「已補件」 → 訂單狀態 = **等待審稿**（該印件稿件已上傳 / 已補件，球在審稿人員）
4. 否則（全部「稿件未上傳」；或「合格」+「稿件未上傳」混合）→ 訂單狀態 = **稿件未上傳**

**規則 3 的設計**：審稿狀態 = 「等待審稿」本身即隱含「稿件已上傳、等待審稿人員處理」；EC 混合訂單中需審稿但尚未上傳稿件的印件狀態是「稿件未上傳」而非「等待審稿」，規則 3 不會誤觸發。

**觸發時機**：任何會改動印件審稿狀態的動作 SHALL 於完成後觸發此派生邏輯：
- 印件送審完成（合格 / 不合格）
- 補件完成（不合格 → 已補件）
- 首次稿件上傳（稿件未上傳 → 等待審稿）
...
- **WHEN** 審稿人員送出審核結果為「不合格」，印件審稿狀態變為「不合格」
- **AND** 訂單位於審稿段
...
- **WHEN** 業務或會員完成補件，印件審稿狀態由「不合格」變為「已補件」
- **AND** 訂單下已無其他「不合格」印件
- **THEN** 系統 SHALL 重新派生訂單狀態
...
- **THEN** 系統 SHALL 重新派生訂單狀態
- **AND** 訂單狀態 SHALL 變為「製作等待中」（離開審稿段）

#### Scenario: 免審稿路徑不觸發 bubble-up

- **WHEN** 訂單回簽 / 付款後自動分配，所有印件經免審稿快速路徑直接進入「合格」
- **THEN** 訂單狀態 SHALL 直接進入「製作等待中」
- **AND** 訂單 MUST NOT 經過「稿件未上傳」、「等待審稿」、「待補件」
#### Scenario: 混合免審稿與需審稿未上傳印件不誤派為等待審稿

- **WHEN** 訂單混合印件：部分為免審稿（審稿狀態 = 「合格」）+ 部分為需審稿但尚未上傳稿件（審稿狀態 = 「稿件未上傳」）
- **THEN** 訂單狀態 SHALL 派生為「稿件未上傳」（規則 4）
- **AND** 訂單狀態 MUST NOT 派生為「等待審稿」（規則 3 不觸發，因為沒有印件處於「等待審稿」或「已補件」）

#### Scenario: QC 不合格不冒升至 Order 層

- **WHEN** 某生產任務的 QC 結果為「不合格」
- **THEN** 訂單狀態 MUST NOT 變為任何「不合格」相關狀態
- **AND** 訂單層永遠沒有「QC 不合格」狀態
...
**本 change 新增**：補件入口 SHALL 顯示**歷史輪次審稿備註清單**，讓業務於補件前可參照審稿意見並轉達客戶。歷史清單內容與呈現方式對齊 [prepress-review spec § B2B 業務補件](../prepress-review/spec.md)。
...
- **WHEN** 業務點選印件 X 的「補件」入口
- **THEN** 系統 SHALL 於上傳元件同頁顯示**歷史輪次審稿備註清單**（最新在上）
- **AND** 清單 SHALL 含每輪輪次編號、審稿結果、退件原因分類、審稿備註與時間
...
線下訂單由需求單「成交」轉入後 SHALL 以「草稿」為初始狀態（僅適用訂單類型 = 線下；線上訂單由 EC 付款自動推進、諮詢訂單為短路徑收尾，皆不進入草稿態）。**本 Requirement 取代既有「訂單建立」§ Scenario US-ORD-001 與 state-machines § 訂單狀態機 線下路徑列舉中「轉訂單直接進入報價待回簽 / 待業務主管審核」的描述**（既有主 spec 該兩處已由本 change MODIFIED § 訂單建立 與 state-machines § 訂單狀態機 整條取代校正，archive sync 自動完成）。
