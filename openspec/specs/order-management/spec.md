...
> 訂單集群 17 實體關聯總覽。實線框的外部實體（Customer / User / QuoteRequest / ConsultationRequest / BillingCompany / ReviewRound / AfterSalesTicket）僅畫邊界 FK，完整欄位在各自模組；Shipment / ShipmentItem 出貨實體見 [prototype-data-store spec](../prototype-data-store/spec.md)（ShipmentRecord）。
