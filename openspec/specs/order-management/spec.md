...
- **AND** 帶入規則詳見[§ Requirement: 需求單轉訂單欄位帶入規則](#requirement-需求單轉訂單欄位帶入規則)
...
訂單階段的三個備註欄位（訂單須知 / 交貨備註 / 付款備註）編輯權限 SHALL 對齊粗粒度模組權限（角色職責定義見 wiki [03-roles/](../../../../memory/Sens_wiki/wiki/erp/03-roles/)）。各角色的編輯權限 MUST 依下表配置：

| 角色 | 訂單模組粗粒度 | 訂單備註編輯權限 | 備註 |
|------|-------------|---------------|------|
| Supervisor | R/W | **唯讀模式** | Supervisor 角色行為限制：所有編輯按鈕 disabled（角色定義見 wiki [[Supervisor]]）|
| 訂單管理人 | R/W | 可編輯 | — |
