...
- **WHEN** 業務點擊「取消訂單」
- **THEN** 訂單 SHALL 進入「已取消」終態（不可逆）；系統 MUST 自動執行 top-down 連鎖：工單全數轉「已取消」→ 任務全數轉「已作廢」→ 生產任務依當前狀態轉「已作廢」或「報廢」；退款（若有應退差額）SHALL 走訂單收退款模型（退款訂單異動核可即生效 + 退款收款紀錄切已完成核銷應退差額，見 [order-billing spec § 退款流程三組件 / § 待出金退款清單](../order-billing/spec.md)），非「退款申請 → 退款處理中 → 已退款」EC 序列
...
業務 / 諮詢 / 訂單管理人 SHALL 可於訂單詳情頁印件清單操作欄點「編輯印件」開啟印件編輯面板直接編輯下列欄位：規格備註 / 購買數量 / 單位 / 難易度 / 免審稿 / 售價（**含調降**）。系統 SHALL 直接更新印件 / 訂單明細對應值，印件費（購買數量 x 售價）與訂單應收即時重算；活動紀錄 MUST 記錄變更內容（before / after）。**難易度 / 免審稿屬審稿相關欄位，其審稿生效時點另依規則**（已分派後次一輪生效；審稿維度到達「已確認可製作」後改之無實質效果——見 § 多印件管理 § 免審可編輯性、prepress-review § 印件自動分配機制 § 分配後的維持與重派）。
