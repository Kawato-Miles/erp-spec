...
> 訂單集群 17 實體關聯總覽。實線框的外部實體（Customer / Contact / User / QuoteRequest / ConsultationRequest / BillingCompany / ReviewRound / AfterSalesTicket）僅畫邊界 FK，完整欄位在各自模組；Shipment / ShipmentItem 出貨實體見 [prototype-data-store spec](../prototype-data-store/spec.md)（ShipmentRecord）。
> 聯絡人（Contact，廠客模組窗口聯絡人主檔）僅與 Order 關聯（`Order.contact_id`）；Invoice MUST NOT 關聯 Contact，買受人資訊（buyer_name / buyer_ubn / buyer_email）為發票開立當下的去正規化快照，不回指 Contact / Customer。Customer / Contact 尚無 OpenSpec 正本（見 [[ORD-038]]）。
...
| 付款方式 | payment_method | 單選 | | | route-C 遺留，新流程不寫入（付款事實見 § Payment）；既有值：現金 / 信用卡 / 銀行轉帳 / 支票 / 其他 |
| 付款備註 | payment_detail | 文字 | | | route-C 遺留，新流程不寫入（見 § Payment / § BillingInstallment）；既有語意：分期資訊 / 發票備註 |
| 付款時間 | paid_at | 日期時間 | | | route-C 遺留，新流程不寫入（收款時點見 § Payment.paid_at）；既有語意：線上訂單用 |
...
> **付款欄位遷移說明（route-C 後）**：`payment_status`（未付款 / 已付款 / 部分退款 / 已退款）為訂單層 derived 狀態，依未取消已完成 Payment 累計推導，保留。`payment_method` / `paid_at` / `payment_detail` 為 route-C 前平面付款模型遺留欄位；實際收款金流已遷至 § Payment（store.payments），這三欄於新模型不再作為付款事實來源（兼容舊 mock 資料保留，新流程不寫入）。應收計算以 § Payment + § PaymentAllocation + § BillingInstallment + 已執行 § OrderAdjustment 為準。
...
出貨單相關資料表（Shipment、ShipmentItem）見出貨模組（待建立）。收款紀錄統一見本 § Payment。
...
> route-C（refactor-order-receivable-refund-model）後付款金流統一存於 store.payments；取代 Order 平面 PaymentRecord（已廢止）。payment_method = 退款 時 amount 為負，與一般收款分軸（D12 折讓 / 退款分離）。
...
| 對帳附件 | attachments | 字串[] | | | 對帳憑證檔名；退款 Payment 切「已完成」MUST ≥ 1（物理錨點，路 C D4）；一般收款不強制 |
| 實際完成日 | actual_receive_date | 日期 | | | 退款 / 補收 Payment 切「已完成」時填；與 paid_at 同步（路 C 橋接欄位）|
| 建立時間 | created_at | 日期時間 | Y | Y | |
| 更新時間 | updated_at | 日期時間 | Y | Y | |
...
> 審核 derived 規則：退款（amount < 0）需業務主管核可；補收（amount > 0、業務手動 type）跳過審核中間態直達已執行；諮詢取消退費（系統內生）免審核。補收 OA amount > 50000（系統常數，待 [[OQ-BI-4]] 拍板）觸發大額事後監督（ActivityLog 紅標 + 通知業務主管）。「已執行」= 核可後應收即生效（route-C：移除舊「綁 Payment 累計推進 + 回退機制」）。
