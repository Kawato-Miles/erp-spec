...
- **GIVEN** 待諮詢取消後諮詢訂單已是「已取消」、訂單異動-c1 狀態 = 已核可（系統代審建立、應收尚未認列）
- **WHEN** 諮詢人員與客戶確認退款金額後執行「確認」
- **THEN** 訂單異動-c1 狀態 SHALL 推進「確認可執行」（終態、不可逆）、執行時間 = 當前時間、應收於此刻認列減去 1000
- **AND** 諮詢訂單狀態 MUST 維持「已取消」

#### Scenario: 待諮詢取消退款收款紀錄切已完成（金流完結、不回寫單據）

- **GIVEN** 訂單異動-c1 狀態 = 確認可執行、退款收款紀錄（-1000, 狀態=處理中）存在
- **WHEN** 諮詢人員處理銀行退款後將退款收款紀錄切「已完成」並上傳退款證明附件
- **THEN** 退款收款紀錄付款狀態 SHALL 改為「已完成」（金流完結，對帳應退差額歸零）
- **AND** 訂單異動-c1 狀態 MUST 維持「確認可執行」、諮詢訂單狀態 MUST 維持「已取消」
