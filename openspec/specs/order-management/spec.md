...
> 訂單核心實體關聯圖。帳務實體（Invoice / Payment / BillingInstallment / SalesAllowance / PaymentAllocation 等）見 [order-billing ERD](../order-billing/spec.md#實體關聯圖erd)；異動實體（OrderAdjustment / OrderAdjustmentItem）見 [order-adjustment ERD](../order-adjustment/spec.md#實體關聯圖erd)。
...
erDiagram
    ...
    Order ||--o{ OrderAdjustment : "金額異動（見 order-adjustment spec）"
...
> 異動實體（OrderAdjustment / OrderAdjustmentItem）已搬遷至 [order-adjustment/spec.md](../order-adjustment/spec.md#data-model)。
