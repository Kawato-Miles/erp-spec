...
工單異動時 MUST NOT 設置「已確認異動內容」中間態（與任務異動不同）。

#### Scenario: 工單異動直接回退

WHEN 工單於製作中需要異動
THEN 工單 SHALL 直接回退至「重新確認製程」狀態
AND 系統 MUST NOT 產生「已確認異動內容」的中間狀態
...
生產任務（Production Task）SHALL 依以下基礎狀態流轉：

待處理 → 製作中 → 已完成

依工廠類型有不同路徑：
- 自有工廠：待處理 → 製作中 → 已完成
- 外包工廠：待處理 → 製作中 → 運送中 → 已完成
- 中國工廠：待處理 → 製作中 → 已送集運商 → 運送中 → 已完成
...
WHEN 生產任務因異動或其他原因需作廢
THEN 狀態 SHALL 變為「已作廢」，為終態，不可恢復
