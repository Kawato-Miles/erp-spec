...
- SPLIT
- CANCELLED
- PAYMENT_ALLOCATION_SET（業務手動建立 / 修改入帳明細；取代已廢自動分配模型的 PAYMENT_ALLOCATION_OVERRIDDEN + PAYMENT_ALLOCATION_ADJUSTED_AFTER_COMPLETE 兩事件）
...
- `getOrderPaymentChangeRate(orderId)`：query 訂單下所有期次與入帳相關修改事件次數（業務層級彙總 = 平均每訂單修改次數，不除以 Payment 數——Payment 數無營運管理意義；Miles 指標 ⑩，見 order-management § 訂單收款變更率指標）
