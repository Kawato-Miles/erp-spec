...
系統 SHALL 實作 TransferTicket 實體的資料模型。欄位正本：[wiki 轉交單實體卡](../../../memory/Sens_wiki/wiki/erp/05-entities/轉交單.md) § 欄位（業務可見）。

技術層補充（非業務欄位正本）：
- 條件必填規則：targetType = 內部產線時 destinationLineId 必填；外部廠商時 destinationVendorId 必填；deliveryMethod = 貨運行時 carrierName 必填
- Slack 通知連結（slackMessageUrl，URL 字串）：整合追蹤欄位，Webhook 發出訊息後回填供查找（對齊需求單 Slack 連結作法）
- lines 陣列至少一條 TransferTicketLine；id / FK / 時間戳等技術欄位依實作慣例
...
系統 SHALL 實作 TransferTicketLine 子結構，儲存於 TransferTicket.lines 陣列，每條 line 代表從某個生產任務抽取的部分數量。欄位正本：[wiki 轉交單實體卡](../../../memory/Sens_wiki/wiki/erp/05-entities/轉交單.md) § 轉交明細。

技術層補充（非業務欄位正本）：productionTaskId 必須屬於 Ticket 的 printItemId；productionTaskName 為快照（顯示用，通常為 taskNo + process）；quantity > 0 且不超過 line-level 上限。
...
- 產線欄位正本：[wiki 產線實體卡](../../../memory/Sens_wiki/wiki/erp/05-entities/產線.md) § 欄位（業務可見）
- 工序欄位正本：[wiki 工序主檔實體卡](../../../memory/Sens_wiki/wiki/erp/05-entities/工序主檔.md) § 欄位（業務可見）（工序分類以 wiki 工序群組模型為準，取代舊三值 category 分類）
- 報工紀錄欄位正本：[wiki 報工紀錄實體卡](../../../memory/Sens_wiki/wiki/erp/05-entities/報工紀錄.md) § 欄位（業務可見）
