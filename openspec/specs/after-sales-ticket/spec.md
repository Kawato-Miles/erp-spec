...
售後服務模組 -- 客訴 / 不良 / 規格不符 / 物流問題 / 工法限制 / 交期延誤等售後事件的承載容器，限訂單進終態（訂單完成、已取消）後可建。
...
系統 SHALL 提供售後服務單實體，作為**訂單進終態後**客訴 / 不良 / 規格不符 / 物流問題 / 工法限制 / 交期延誤等售後事件的承載容器。一張售後服務單 SHALL 屬於單一訂單，記錄業務／諮詢與業務主管討論後的決議結果、售後類型分類，並關聯下游動作（訂單異動、補做生產任務）。

**建單 SHALL 限終態訂單**：所屬訂單進終態（訂單完成、已取消）後才可建單，其餘訂單狀態 SHALL 拒絕（規格見 § 售後服務單限終態訂單建立）。

**售後服務單 SHALL NOT 持有印件、SHALL NOT 建立印件**：「相關印件」為受理層的指認（選填、多選，記客戶反映的是哪幾件印件），補做的載體 SHALL 為加開在原工單上的補做生產任務（見 § 售後服務單決議觸發補做）。

欄位正本見 wiki [售後服務](../../../memory/Sens_wiki/wiki/erp/05-entities/售後服務.md) § 欄位（業務可見）；關聯（0..N 訂單異動、N:M 相關印件、0..N 補做生產任務；退款的款項紀錄掛所屬訂單、不掛售後服務單，兩者只在對帳總額層對齊）見同卡 § 關鍵關聯。本 spec 不複寫欄位表。
...
**變更理由**: 三處需對齊——(1) next action「待建關聯動作」的決議值域寫「補印」、下游動作寫「補印印件」，值域正名為「補做」、下游動作改為補做生產任務（判定改看補做來源是否已掛上）；(2) 空列表說明文的建單時機措辭改為與受理範圍一致（訂單完成或取消後才建得出來）；(3) 版型段原以 Prototype 檔案路徑與元件名逐項規定 layout，Prototype 已為介面與互動正本，改為只承載資料範圍與分組定義。四組分組定義、互斥規則、逾期優先、排序與徽章行為皆不變。
