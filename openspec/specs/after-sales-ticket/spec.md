...
- **GIVEN** AfterSalesTicket.status = 處理中、resolution = 退款、關聯 OrderAdjustment 確認可執行、退款 Payment 已建立
- **WHEN** 業務點擊「結案」
...
- 訂單完成後補收（客戶承擔）：ticket 內建 OA(+N) → 跳過審核中間態直達確認可執行（補收 OA 新規則）→ 業務新增 BillingInstallment 承載補收應收 → 開票 + 收款核銷
...
售後服務單於「處理中」狀態 SHALL 顯示三組件進度提示，待三組件全部完成（訂單異動單確認可執行、負值收款紀錄已建立、發票異動已完成）才可結案售後服務單。
