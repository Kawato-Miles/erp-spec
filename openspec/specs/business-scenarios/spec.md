...
| 13 | 廠務把首批 500 個送抵品檢站，品檢人員驗收（通過 500） | 製作中 | 合格 | 製作完成 | 已完成 | 合格 | 製作中 | 製作中 | **入庫累計 500** |
| 14 | 第二批 500 個送抵，品檢人員驗收（通過 500） | 製作中（大貨累計 500） | 合格 | 製作完成 | 已完成 | 合格 | 製作中 | 製作中 | **入庫累計 1,000** |
...
本情境 SHALL 驗證品檢缺口的補做接力，以及分次驗收搭配分批出貨的完整追蹤。品檢以印件層品檢紀錄承載（品檢型生產任務與獨立品檢單據的舊模型已於 2026-07-21 廢除）。

#### Scenario: 品檢缺口補做

**背景**：印件目標 1,000 個，工單旗下生產任務全部報工完成，貨送抵品檢站。

| 步驟 | 角色動作 | 工單 | 印件品質帳 | 補做任務 |
|---|---|---|---|---|
| 1 | 品檢人員驗收 1,000 個，記一筆（通過 950、不通過 50、原因色差）| 已完成 | 入庫 950、缺口 50 | -- |
| 2 | 印務於印件層決定處置：補做 × 瑕疵品報廢 | 已完成 | 處置紀錄一筆（補做 50）| -- |
| 3 | 系統在原工單經異動加開生產任務（預帶全工序可刪、數量預填 50）| **異動** | 入庫 950 | **待處理** |
| 4 | 師傅補做 50 個並報工 | 製作中 | 入庫 950 | **已完成** |
| 5 | 品檢人員對補做這批驗收（通過 50）| **已完成** | 入庫 1,000、缺口 0 | 已完成 |

**系統行為**：

- 補做 SHALL 在原工單加開生產任務、SHALL NOT 建新工單（見 `work-order` § 補生產承接）
- 品檢紀錄 SHALL 由品檢人員逐筆累計、系統 SHALL NOT 代寫（見 `qc`）
- 工單完成判定與品檢脫鉤：旗下生產任務全部完成即「已完成」，品檢缺口不擋收尾

#### Scenario: 分次驗收與分批出貨

**背景**：印件目標 1,000 個，生產分兩批完成，業務分兩批出貨。

| 步驟 | 角色動作 | 印件（印製）| 入庫累計 | 可出貨額度 | 出貨單 A | 出貨單 B |
|---|---|---|---|---|---|---|
| 1 | 首批 500 個做完送品檢站，品檢人員驗收通過 500 | 製作中 | 500 | 500 | -- | -- |
| 2 | 業務建出貨單 A（500 個）| **出貨中** | 500 | 0 | **未處理** | -- |
| 3 | 揀貨人員開始揀貨、回報裝箱 | 出貨中 | 500 | 0 | **打包中 → 待出貨** | -- |
| 4 | 出貨人員出貨確認（第三方物流、填托運單號）| 出貨中 | 500 | 0 | **運送中** | -- |
| 5 | 第二批 500 個做完送驗，品檢人員驗收通過 500 | **製作完成** | 1,000 | 500 | 運送中 | -- |
| 6 | 業務建出貨單 B（500 個）| 出貨中 | 1,000 | 0 | 運送中 | **未處理** |
| 7 | 出貨單 A 送達確認 | 出貨中 | 1,000 | 0 | **已送達** | 未處理 |
| 8 | 出貨單 B 走完揀貨與出貨、送達確認 | **已送達** | 1,000 | 0 | 已送達 | **已送達** |

**系統行為**：

- 可出貨額度 SHALL 隨每筆品檢驗收逐批放大、隨出貨單明細消耗（公式見 wiki 齊套邏輯卡）
- 印件印製維度轉「已送達」SHALL 以所有出貨單皆送達且累計送達數等於印件數量為判準
- 分批 SHALL NOT 需要專屬流程或旗標（見 `shipment` § 分批出貨）
