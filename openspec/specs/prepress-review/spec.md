...
- **NG-製程問題**（打樣品質問題，稿件本身無問題）→ 系統 SHALL 自動於同一打樣印件下建立新打樣 WorkOrder（`type = 打樣`、初始狀態 = 草稿；一律重打、不由業務決定），同步執行：印件印製維度自「已送達」轉回「等待中」（僅打樣印件的回頭轉換，大貨印件無回頭）、`sampleResult` 重置回「待確認」、舊週期樣品報廢（自入庫數與累計送達數移出、數量帳按打樣週期歸零，報廢量計入成本，公式正本見 wiki 齊套邏輯卡）。重做成本以印件為單位累積（見 wiki [印件](../../../memory/Sens_wiki/wiki/erp/05-entities/印件.md)）；本次判定的留底即 `sampleResult` 欄位值與那張新打樣工單，SHALL NOT 另建不符合報告實體、SHALL NOT 另記根因分類（QC-005 拍板）
