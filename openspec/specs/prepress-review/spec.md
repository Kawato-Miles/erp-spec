...
系統 SHALL 提供訂單管理人專屬的「待分派審稿」清單：自待審訂單母集合中，僅列出「任一非棄用、非免審印件之審稿維度為『待分派』」的訂單，為訂單管理人預先篩好需要分派審稿人員的訂單。其餘功能（展開、勾選、分派審稿人員、進行審稿動線、搜尋篩選、訂單詳情入口）SHALL 與待審訂單模組一致。
...
審稿人員的待審工作介面 SHALL 為「待審訂單」模組（見 § 審稿訂單脈絡查詢視圖），不再提供以印件為單位的獨立列表頁。單印件**審稿詳情頁**保留：呈現**稿件備註**（`PrintItem.client_note`）、印件需求規格、原稿檢視、歷史輪次（含每輪 `review_note`）、上傳元件、送審操作；自待審訂單模組「進行審稿」進入。
...
**Rationale**: 不歸零則第二輪永遠湊不齊收尾條件（累計送達數 ≥ 購買數量會被上一輪的數提前滿足，印件會在新樣品還沒做出來時就判已送達）。界線取新打樣工單的建立而不另存週期序號，是因為多一個欄位就多一個要維護一致的真相；新工單的建立時點已經唯一決定了「從哪一刻起算」。界線只認由 NG-製程問題觸發的那張工單、且該草稿不可刪，是為了防界線位移：界線若取「最新一張打樣工單」，印務主管在同一輪加開一張就把界線推到後面，本輪已驗、已出的量突然掉出本週期，額度與收尾條件一起歸零，現場看到的是「數字自己不見了」；同理，刪掉起點那張草稿會讓界線退回上一輪，上一輪的量被算回本輪的帳。比較含等號，是因為「建新打樣工單」與「同一分鐘內接著驗收或建出貨單」會落在同一格時分，嚴格大於會把本輪第一筆事實判進上一輪。投入不歸零，是因為每一輪的紙與工都真的花掉了，成本要看得到累計；成品側歸零、投入側累加，正是「成本留、數量除」的分界。以計算界線而非刪紀錄實作，是因為刪掉之後就再也回推不出前幾輪發生過什麼。
...
系統 SHALL 提供「待審訂單」模組（原「依訂單檢視」改造），以「訂單（母）→ 印件（子）」兩層結構呈現審稿段未完結的訂單，為審稿人員、訂單管理人、審稿主管三角色**統一的訂單角度待審工作介面**。原以印件為單位的待審清單頁面（審稿人員「待審印件」、審稿主管「待審印件」）SHALL 取消，職能由本模組承接；單印件審稿詳情頁保留、自本模組「進行審稿」進入。
...
**單件審稿動線**：審稿人員對自己負責的待審印件 SHALL 可點「進行審稿」跳轉至審稿詳情頁執行單件審稿；對非自己負責的印件 SHALL 唯讀。
...
**Rationale**: 補件流程是審稿不合格後的必經路徑，須依 B2C/B2B 分流且回到原審稿人員以確保審稿一致性。
