...
- **GIVEN** 訂單原應收 5000、訂單異動 +20,000（已執行）、開立發票合計 25,000、已確認折讓 -10,000、已完成收款合計 25,000、已完成退款 -10,000
- **WHEN** 業務 / 會計開啟對帳檢視面板
...
對帳狀態標示規則：
- 退款收款紀錄仍處理中（訂單異動為已核可、未推進已執行）：標示「退費處理中」、應收 SHALL 顯示為 1000（已核可訂單異動即時計入應收）、收款淨額顯示為 2000（含+2000、扣除處理中-1000 = 不計入處理中收款紀錄規則，依既有對帳規則）；發票淨額 0 = 預期當下尚未開
- 退款收款紀錄已完成（訂單異動已推進已執行）且發票淨額 = 1000：標示「對帳通過 - 退費完成」
- 退款收款紀錄已完成（訂單異動已推進已執行）且發票淨額 ≠ 1000：標示「待對帳 - 發票金額需確認」、差額由既有對帳警示 banner 提示業務 / 諮詢人員處理

#### Scenario: 諮詢取消退費完成對帳通過

- **GIVEN** 諮詢訂單 訂單額外費用(諮詢費, 2000) + 訂單異動(諮詢取消退費, -1000, 已執行) + 收款紀錄(+2000, 已完成) + 收款紀錄(-1000, 已完成) + 發票(1000, 已開立)
- **WHEN** 業務 / 會計開啟對帳檢視面板
...
- **GIVEN** 諮詢訂單 訂單額外費用(諮詢費, 2000) + 訂單異動(-1000, 已執行) + 收款紀錄(+2000) + 收款紀錄(-1000, 已完成) + 發票(2000, 已開立，諮詢人員誤開全額)
- **WHEN** 業務 / 會計開啟對帳檢視面板
...
- **GIVEN** 收款紀錄 P-099（已完成、金額 = -5000、關聯訂單異動 OA-099 已執行）
- **WHEN** 業務於 OA-099 編輯介面 P-099 row 點「取消」
- **THEN** 系統 SHALL 邏輯刪除 P-099（已完成收款紀錄保留稽核軌跡）
- **AND** OA-099 SHALL 維持「已執行」（訂單異動已執行於核可時已生效、不回退已核可）；對帳應退差額 SHALL 重現並觸發不可忽略警示
...
- **AND** Migration 結束後訂單異動 invariant SHALL 仍滿足（既有「已執行訂單異動 → 必有關聯退款收款紀錄」已隱含「已完成」語意）
...
- **收款狀態 = '已完成'**：邏輯刪除（設是否已取消 = true、取消原因、取消時間 = now），SHALL NOT 從陣列移除；訂單收退款模型重構後 SHALL NOT 觸發訂單異動回退（訂單異動已執行於核可時已生效、不綁收款紀錄累計），改觸發對帳應退差額重現（收款淨額 − 應收 > 0）+ 差額警示引導重退
...
- **AND** OA-001 狀態 SHALL 維持 '已執行'（訂單異動已執行於核可時已生效、取消收款紀錄不回退）
...
系統 SHALL 提供「請款期次」作為訂單應收的單一規劃實體，合併原本「付款計畫」與「預計發票」雙頭維護。每筆請款期次同時承載：預計收款日、預計金額、預計開票日、品項、備註、雙維度狀態（開票/收款獨立）、來源類型、原始日期凍結基準、變更歷史。業務於訂單成立後（訂單狀態 = 報價待回簽 / 訂單確認）建立一筆或多筆請款期次規劃分期請款，建立期間各期金額合計 SHALL 等於訂單含稅總額 + Σ 已執行訂單異動金額（補收進期次、退款不進期次的不對稱規則，違反時系統 SHALL 顯示警示但允許儲存）。
...
系統 SHALL 依金額正負與異動類型自動判定訂單異動是否需業務主管審核：補收正項訂單異動（金額 > 0 且異動類型 ∈ 五項補收類型）SHALL 跳過「待主管審核」與「已核可」中間態直達「已執行」狀態（核可者 = 業務 user_id、執行時間 = now、應收 +N 立即認列）。
...
**訂單異動「已執行」語意統一**：補收與退款訂單異動的「已執行」一致定義為「核可後應收調整生效」（不綁收款紀錄切已完成累計達標）。補收訂單異動核可 = 建立即執行（免審）、應收即時 +N。此語意取代舊「退款訂單異動已執行需綁收款紀錄累計達訂單異動金額」（退款訂單異動改同此語意，見 § 退款 OA Requirement）。
...
- **THEN** 系統 SHALL 設定是否需主管審核 = false、OA-060 狀態直接 = 已執行（核可者=業務, 執行時間=now）
...
- **THEN** OA-061 狀態 SHALL = 已執行（仍立即執行、不阻擋）
...
訂單 SHALL 維護以下 invariant：訂單應收 = Σ 請款期次預計金額（未取消）。違反時對帳檢視面板 SHALL 顯示警示 banner「訂單異動已執行 N 元、但未對應期次規劃」+ action button「建立期次」，讓業務一鍵新增期次承載該補收金額。本警示為提示性質、不阻擋業務後續操作（沿用既有警示而非阻擋的設計精神）。
...
退款負項訂單異動（金額 < 0）SHALL 沿用業務主管核可把關（現金流出強把關），但「已執行」語意改為「核可後應收調整生效」（與補收對稱、不綁收款紀錄累計達標）：
...
- 是否需主管審核 = true
- 狀態流轉：草稿 → 待主管審核 → 已核可（業務主管核可）→ **已執行（核可後應收調整生效，系統自動推進、不等退款收款紀錄）**
- **[訂單收退款模型重構移除]** 舊「業務建退款收款紀錄切已完成 → 累計達訂單異動金額才推進已執行」推進綁定 **移除**；訂單異動已執行不再等退款收款紀錄。
...
- 誤建退款收款紀錄修正：取消退款收款紀錄 → 應退差額重新出現 → 對帳差額警示引導重退（訂單異動已執行 / 應收已調整維持不動）。
...
- **WHEN** 業務主管核可
- **THEN** OA-070 狀態 SHALL 直接 = 已執行（核可即生效）、應收 SHALL = 70000（不等退款收款紀錄）
...
- **GIVEN** 退款訂單異動 OA-070 已執行（應收已 -10000）、退款收款紀錄 P-070 已完成
