...
系統 SHALL 支援 Payment 從 ConsultationRequest 轉移至 Order 的單向能力，服務諮詢費四種收尾情境（不做大貨 / 做大貨成交轉訂單 / 需求單流失 / 待諮詢取消退費）。轉移後保留原始來源紀錄，且不可再次轉移。
...
諮詢訂單的發票 MUST NOT 由系統自動開立，改由收尾情境自動建待開發票（BillingInstallment）提醒、諮詢人員手動一鍵開立。諮詢取消情境不自動建待開發票，留存收入由業務手動開票、未開票靠對帳差額警示兜底。廢止既有依 `consultation_invoice_option` 自動開立邏輯。
...
**Rationale**: 退款 Payment 切「已完成」須上傳匯款證明作為金流實際發生的事實依據，無附件則無法確認退款已執行。
...
**Rationale**: 新增 paymentStatus 雙態後既有資料全為 null，不 backfill 則對帳收款淨額計算會把所有既有收款排除，帳務數字全面錯誤。
...
**Rationale**: 處理中 Payment 屬業務預登記、尚無金流事實依據，若計入應收應付帳本會讓月結報表虛胖，但 Prototype 階段無正式 GL 系統故可分期落地。
...
**Rationale**: 已完成 Payment 若物理刪除會造成稽核軌跡缺失，會計無法回查「為什麼這筆款項不見了」，違反實際發生事件不可抹除的會計準則。
...
業務登錄一筆 Payment 時，系統 SHALL 於「新增收款」Dialog 內 inline 顯示該訂單所有未取消收款項目，業務手動勾選並填入帳金額。入帳金額合計不可超過收款金額，不足部分自動記為預收桶。Payment 切「已完成」時觸發對應期次收款維度狀態推導。
...
系統 SHALL 沿用 v1.13 Payment 主結構，但移除 paymentPlanId 必填約束。Payment 與期次的關聯改為透過 PaymentAllocation N:M 推導。

**Priority**: P0

**Rationale**: 舊設計 Payment 直接綁單一付款計畫，無法支援一筆收款入帳多個期次的實務需求，移除後才能啟用 PaymentAllocation 核銷機制。
...
對帳三方差錯偵測 SHALL 涵蓋「`status ∈ {訂單完成, 已取消}` 且該訂單有 `status=開立` 的 Invoice」的訂單，取代既有「限 `status=訂單完成`」篩選。此修訂根治「已取消但有認列收入訂單從對帳差錯偵測消失」的漏帳。
