...
旗標 SHALL 與計價方法脫鉤：標為印刷類的工序 SHALL 仍需依既有計價方法擇一並配對應價格表（主檔價格一律必填，不設 0 元價格表）。系統 SHALL NOT 新增「依設備計價」一類的計價方法值。

旗標改動 SHALL NOT 回溯既有生產任務——已建立的生產任務其色數登記與設備費 SHALL 維持原狀。

**Priority**: P1

**Rationale**: 色數是工序自身的屬性（幾色、哪些色不隨設備變），現行以「設備有色數模式 且 廠商類別＝自有工廠」當判準，讓外發印刷工序永遠登記不到色數、也逼印務先選機才填得了色，成本因此系統性低估。
...
系統 SHALL 支援生產任務引用工序，除主檔項目外另記錄 `pricing_selection`（計價鍵）。生產任務的**廠商類別 SHALL 由所引用工序主檔項目的承作廠商決定且對印務唯讀**——承作廠商留空即自有工廠，填外部廠商時依該廠商的類別（加工廠／外包廠／中國廠商）帶出。BOM SHALL NOT 規定某類主檔只能配某類廠商。
