...
材料規格 SHALL 有「預設放損率」欄（百分比），建立時 SHALL 必填、預設 0，值 SHALL 不小於 0。生產任務引用該項目時 SHALL 帶入此值為唯讀參考並凍結於任務，主檔事後改率 SHALL NOT 回寫；該值 SHALL NOT 驅動任何數量預填或校驗。
...
### MaterialSpecPrepEntry（備料規格子表）

適用 pricing_type = 按重量。每筆 MaterialSpec 可擁有多列備料，供生產任務挑選為計價鍵。

| 欄位 | 英文名稱 | 型別 | 必填 | 唯讀 | 說明 |
|------|---------|------|------|------|------|
| 識別碼 | id | UUID | Y | Y | |
| 所屬規格 | material_spec_id | FK | Y | Y | FK -> MaterialSpec |
| 備料名稱 | prep_name | 字串 | Y | | 例：名片八開 |
| 寬 | width | 小數 | Y | | |
| 長 | length | 小數 | Y | | |
| 尺寸單位 | dim_unit | 單選 | Y | | 英吋 / 公分 / 公釐 |
| 開料數 | pieces_per_sheet | 整數 | Y | | 一張供應商原料可開出幾張備料 |
| 所屬供應商原料 | weight_entry_id | FK | Y | | FK -> PricingRuleWeightBased，限同一 MaterialSpec 下的列 |
| 顯示排序 | display_order | 整數 | | | |

**約束**：備料 MUST NOT 帶價格欄與設備關聯欄；整張上機以一筆同尺寸、開料數 1 的備料表達。
...
| 計價選擇 | pricing_selection | JSON | Y | 形狀依 Material.pricing_type 不同；選定當下凍結，主檔事後改列不回寫 |

**pricing_selection 形狀**：

| pricing_type | pricing_selection JSON 結構 |
|--------------|---------------------------|
| 按重量 | `{ "prep_entry_id": "...", "weight_entry_id": "..." }` |
...
- **生產任務**：目標數量 10000 張備料，所挑備料為整張上機（A2 同尺寸、開料數 1）
- **pricing_selection**：`{ prep_entry: "A2 整張", weight_entry: "A2" }`
- **查表**：size_table A2 列 → weight_g = 128, ton_price = 5000
- **計算**：整紙單張價 = 面積(㎡) × 128 ÷ 1000 × 5000 ÷ 1000；整紙張數 = ceil(10000 ÷ 1) = 10000；材料成本 = 整紙單張價 × 10000

### Scenario E2E-2：按面積-單價面積材料

- **材料**：白卡皮·永豐（按面積），250g 規格，pricing_method = 單價面積
- **生產任務**：150 張，單張面積 0.5 m²，總面積 75 m²
- **pricing_selection**：`{ area_range: "1-99", qty_range: "100-199" }`
...
- **材料**：壓克力板（按數量），3mm 規格
- **生產任務**：150 件
- **pricing_selection**：`{ qty_tier: "100-199" }`
