...
**「已執行」推進機制（核可即生效、不綁 Payment 累計）**：

- **補收正項 OA**（amount > 0、免審）：建立並「儲存並執行」時直接推進「已執行」，應收即時 +N。
...
|---------|----------------------------|---------|
| 補收正項（amount > 0） | false | 草稿 → 已執行（跳過審核）|
| 退款負項（amount < 0） | true | 草稿 → 待主管審核 → 已核可 → 已執行 |
| 諮詢取消退費（系統內生）| false | 草稿 → 已核可（系統建）→ 已執行 |
| 規格變更（amount = 0） | - | 不建 OA |
