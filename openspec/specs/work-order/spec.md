...
#### Scenario: 打樣工單建立並獨立追蹤
- **WHEN** 打樣印件的工單建立（系統自動或印務主管手動）並設定 type = 打樣
...
角色權責詳見 wiki [工單狀態機卡](../../../memory/Sens_wiki/wiki/erp/06-state-machines/工單狀態.md)。
...
**Rationale**: 工單狀態機為生產中樞的核心流轉規則，任務獨立交付模式與異動 bubble-up 需明確定義推進邏輯。
...
- **WHEN** 被分派工單的印務填寫完製程資訊並提交（工單草稿由系統自動產生或印務主管建立、分派，見 § 工單建立相關 Requirement）
...
工單詳情頁 SHALL 包含 5 個 Tab，順序：`資訊（首位，defaultValue）→ 生產任務 → QC 記錄 → 異動紀錄 → 活動紀錄`。「資訊」Tab 承載原 Tabs 之上資訊區的工單資訊卡 / 生產資訊卡 / 印件檔案卡（依 DESIGN.md §0.1「印件三分類獨立 ErpDetailCard」規範單欄垂直排列）。
...
工單詳情頁 SHALL 採用 Tabs 化版型（依 DESIGN.md §6.3.1），結構：`ErpPageHeader → （條件性退回原因 InfoBanner）→ Tabs container（首位「資訊」Tab，defaultValue）`。
...
**Rationale**: 不同印件類型的標準化程度差異大，帶入規則須依類型分流以兼顧效率與彈性。
...
**Rationale**: 數量計算為印件完成度的基礎，邊界防呆避免錯誤設定導致完成度計算失真。
...
**Rationale**: 數量欄位異動若不即時重算，印件完成度將與實際生產進度脫節。
...
**Rationale**: 任務異動狀態機為工單異動流程的任務層收尾機制，與工單 bubble-up 配合確保異動完整收斂。
...
交付以任務為單位（任務獨立交付模式，見 § 任務獨立交付操作）；亦可自工單層批次交付全部任務，逐任務生效。終態：已完成、作廢（由生產任務層 bottom-up 觸發）。

狀態列舉正本見 wiki [任務狀態機卡](../../../memory/Sens_wiki/wiki/erp/06-state-machines/任務狀態.md)。

**Priority**: P0

**Rationale**: 任務為生產任務的分組層，明確定義狀態流轉確保任務與工單層的 bubble-up 正確運作。

#### Scenario: 任務從待交付到已交付

- **WHEN** 印務交付該任務（單筆交付，或自工單層批次交付、逐任務生效）
...
**Rationale**: 齊套性邏輯是工單完成度的核心計算機制，requires_inspection 混合模式確保精確追蹤生產進度。
...
#### Scenario: 分批出貨情境下 QC PT 累計與 pi_warehouse_qty 計算（C1 過渡期）
...
- 工單欄位正本（含異動紀錄子表）：[wiki 工單實體卡](../../../memory/Sens_wiki/wiki/erp/05-entities/工單.md) § 欄位（業務可見）
