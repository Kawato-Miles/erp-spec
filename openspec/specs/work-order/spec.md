...
系統 SHALL 支援三種工單草稿建立方式：(1) 印件已指定印件配方時，由印務在印件層引用印件配方展開，依部件清單與是否獨立開工單旗標一次建立多張草稿；(2) 線上單審稿通過後，系統依印件帶入的印件配方自動展開；(3) 印件未指定印件配方時，由印務主管手動建立、印務逐一新增生產任務。工單號格式：W-[YYYYMMDD]-[NN]。

依印件配方展開時，每個配方工序段 SHALL 展開為生產任務（同段同時掛材料與工序時拆為材料型與工序型兩個任務），並帶入該段引用的材料／工序／裝訂主檔項目、產線、每份產出倍率、依賴前段與是否計入完成度；生產單位類別 SHALL 由所引用主檔項目的承作廠商決定，計畫設備 SHALL 留空待製程規劃時選定。展開規則正本見 wiki [配方展開規則](../../../../memory/Sens_wiki/wiki/erp/04-business-logic/營運規則/訂單到交付/配方展開規則.md)。

**Priority**: P0

**Rationale**: 客製回頭單重複規劃製程是實際痛點；印件配方展開讓做過一次的品項不必重填，而一款多部件時要能一次把所有工單開出來，不靠印務記得該開幾張。

#### Scenario: 印務引用印件配方展開多張工單草稿

- **GIVEN** 印件已指定印件配方「吊牌鑰匙圈禮盒組」（吊牌與鑰匙圈獨立開單、外盒不獨立）
...
- **WHEN** 線上單印件審稿通過，且該印件由 EC 商品主檔帶入印件配方
- **THEN** 系統自動依該印件配方展開工單草稿，無需印務操作
- **AND** 展開結果的生產任務計畫設備留空

#### Scenario: 印件未指定印件配方時手動建立

- **GIVEN** 印件的印件配方欄為空（一次性全客製品）
- **WHEN** 印務主管為該印件建立工單
...
系統 SHALL 支援印務填寫工單內容，包含製程說明、品檢需求與生產任務規格；生產任務可由印件配方展開帶入後調整，或由印務逐項新增。展開帶入的生產任務與手動新增的生產任務在後續製程規劃、審核與交付上行為一致。
...
- **WHEN** 印務開啟草稿狀態工單進行填寫
- **THEN** 印務可設定製程說明與品檢需求，並新增、修改、刪除生產任務（不論該任務來自印件配方展開或手動新增）

#### Scenario: 調整展開帶入的生產任務

- **GIVEN** 工單的生產任務由印件配方展開帶入，計畫設備為空
...
**印件類型欄位**：印件總覽 SHALL 在表頭呈現「印件類型」欄位，值域三值（打樣印件／大貨印件／補印印件，正本見 wiki [印件](../../../memory/Sens_wiki/wiki/erp/05-entities/印件.md) 欄位表），並提供可複選的三選項篩選。
...
- **WHEN** 列表渲染
- **THEN** 每筆印件 SHALL 在「印件類型」欄位顯示其類型值
