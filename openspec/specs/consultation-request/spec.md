...
- **完成諮詢（不做大貨）**：系統建立諮詢訂單（type=諮詢訂單）+ OrderExtraCharge(consultation_fee) + Payment 從 ConsultationRequest 轉移至諮詢訂單 + **自動建立 BillingInstallment 1 筆**（scheduled_amount = 諮詢費全額 2000、description = 「諮詢費」、due_date / scheduled_issue_date = 完成諮詢時點當天、source_type = consultation_end_no_production、invoicing_status = 未開立、created_by = system）；諮詢訂單推進至訂單完成路徑（**不自動開 Invoice**，由諮詢人員後續手動將 BillingInstallment 一鍵開立為實際 Invoice）；ConsultationRequest 推進至「完成諮詢」（完整自動建請款期次規則見 § Requirement: 諮詢結束不做大貨情境自動建請款期次，為本機制單一正本）
- **轉需求單（做大貨）**：系統建立需求單（QuoteRequest, status=需求確認中, linked_consultation_request_id 寫入）；ConsultationRequest 推進至「已轉需求單」終態（**狀態就此定格**：下游需求單成交或流失皆不再改變諮詢單狀態；諮詢單狀態承載「進入大貨流程」的商業出口分類，下游結局由需求單狀態承載）；**MUST NOT 建任何訂單**；Payment 維持綁 ConsultationRequest，等需求單流程結局明確時才轉移

#### Scenario: 完成諮詢 - 不做大貨（建諮詢訂單收尾 + 自動建 BillingInstallment）
...
- **AND** 諮詢訂單 SHALL 推進至完成路徑（諮詢人員後續手動將 BillingInstallment 一鍵開立為實際 Invoice 後訂單完成）
...
- **AND** 系統 MUST NOT 建立任何 Order
- **AND** 系統 MUST NOT 建立任何 BillingInstallment（請款期次的建立時機延後到需求單結局明確時：成交轉訂單由業務於主訂單規劃時自行加入；流失時走「需求單流失觸發建諮詢訂單收尾」分支自動建）
...
4. **系統 SHALL 自動建立 BillingInstallment 1 筆**（scheduled_amount = 諮詢費全額 2000、description = 「諮詢費」、due_date / scheduled_issue_date = 需求單流失時點當天、source_type = quote_lost、invoicing_status = 未開立、created_by = system）
5. 諮詢訂單推進至完成路徑（**不自動開 Invoice**，由諮詢人員後續手動將 BillingInstallment 一鍵開立為實際 Invoice）

ConsultationRequest 狀態 MUST 維持「已轉需求單」終態不變（諮詢單狀態承載商業出口分類，流失收尾不回寫諮詢單；本流程的觸發判斷依需求單 `linked_consultation_request_id` 欄位與流失事件，與諮詢單狀態無關）。`linked_consultation_order_id` 寫入新建諮詢訂單 ID。
...
#### Scenario: 議價中需求單流失觸發建諮詢訂單與自動建 BillingInstallment
...
- **AND** 系統 MUST NOT 自動開立 Invoice（不論 `consultation_invoice_option` 值為何）
- **AND** 諮詢訂單 SHALL 推進至完成路徑（諮詢人員後續手動一鍵開立 Invoice 後訂單完成）
- **AND** ConsultationRequest 狀態 MUST 維持「已轉需求單」不變（不更新為「完成諮詢」）
...
- **WHEN** 系統處理需求單流失事件
- **THEN** 系統行為 SHALL 與「議價中流失」情境相同（建諮詢訂單 + Payment 轉移 + 自動建 BillingInstallment 2000 + ConsultationRequest 狀態維持不變）
...
`consultation_invoice_option` SHALL 由客人於 surveycake 表單 Q4 自選，**作為客戶意向參考保留於諮詢單實體欄位**。本 change 後此欄位 **MUST NOT 驅動任何系統發票自動化行為**（包含 Invoice / SalesAllowance / BillingInstallment 的建立），所有諮詢費 Invoice 統一由諮詢人員於 BillingInstallment 既有一鍵開立流程處理。
...
BillingInstallment 自動建立規則統一改為依「諮詢訂單收尾情境」分類（見 § 諮詢結束不做大貨情境自動建請款期次、§ 需求單流失情境自動建請款期次、§ 諮詢取消半額退費自動建請款期次 三個 Requirement，為各情境機制單一正本）：

| 情境 | BillingInstallment 自動建立 |
|------|------------------------|
| 諮詢結束不做大貨 | 1 筆 2000 元（description = 「諮詢費」、source_type = consultation_end_no_production）|
| 需求單流失（已轉需求單後）| 1 筆 2000 元（description = 「諮詢費」、source_type = quote_lost）|
| 諮詢取消 | **MUST NOT 自動建**（2026-05-30 converge-consultation-cancel-to-order-cancel-flow 收斂、廢除諮詢專屬自動建待開發票；留存 1000 元收入由業務循一般訂單取消發票開立路徑於需要時手動開立，未開票由對帳「應收 > 發票淨額」差額警示兜底；source_type = consultation_cancellation enum 保留供業務手動建期次時標示來源）|
...
- **THEN** 系統 MUST NOT 依 `consultation_invoice_option` 決定 Invoice / SalesAllowance / BillingInstallment 的建立或不建立
- **AND** BillingInstallment 與 Invoice 的自動建立規則 SHALL 完全依「諮詢訂單收尾情境」決定
...
- **WHEN** 業務於主訂單規劃 BillingInstallment 時程
- **THEN** 業務 SHALL 可參考客戶意向決定是否將諮詢費獨立成單張 BillingInstallment 或併入其他 BillingInstallment
