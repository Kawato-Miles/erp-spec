...
生管角色 SHALL 僅擁有任務模組的 R/W 權限，且可編輯範圍 MUST 限於任務進度回覆，不可更改任務內容本身。

#### Scenario: 生管更新任務進度

WHEN 生管角色開啟任務詳情頁面
THEN 系統 SHALL 允許編輯任務進度回覆欄位（如完成數量、預計完成時間）
AND MUST NOT 允許修改任務內容欄位（如製程參數、規格、數量）

#### Scenario: 生管安排工作任務

WHEN 生管角色需要安排生產排程
THEN 系統 SHALL 提供任務排程介面
AND 生管 SHALL 可調整任務的執行順序與時間

---

### Requirement: 師傅角色職責

師傅角色 SHALL 擁有任務模組的 R/W 權限，負責執行生產任務並回報生產狀態。
...
### Requirement: 外部廠商角色（中國廠商與外包廠商）

中國廠商與外包廠商 SHALL 僅擁有任務模組的 R/W 權限。中國廠商額外負責報價功能，外包廠商負責確認成品如期完成。
...
QC 角色 SHALL 擁有工單模組的 R/W 權限，但可編輯範圍 MUST 限於 QC 紀錄，不可修改工單其他內容。
