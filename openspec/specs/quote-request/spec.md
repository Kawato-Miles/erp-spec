...
每筆印件項目 SHALL 另帶一個選填欄「印件內部完成日」，代表業務與客戶談定的內部完成那一天；需求單不標示急件，本欄不扣任何天數。本欄 SHALL 逐列手填：**需求單單頭 SHALL NOT 設任何交期欄**，系統 SHALL NOT 對印件項目列做預填、SHALL NOT 於新增列時帶入同單其他列的日期。留空 SHALL 可存檔。

**需求單階段 SHALL NOT 設急件選項欄**：需求單的印件項目一律視為一般件、不扣天數，急件於成交轉訂單後由業務在訂單印件選定（見 [order-management spec § 印件急件選項](../order-management/spec.md)）。

**推得值 SHALL 為唯讀顯示**：每列 SHALL 顯示由本欄推得的「印件預計交期」（印件內部完成日的下一個工作天，算法見 [order-management spec § 交期鏈日期以工作天計算](../order-management/spec.md)）。印件內部完成日留空時印件預計交期 SHALL 顯示「－」。

**例外——複製既有需求單建單**：複製屬承接性質，各印件項目的印件內部完成日 SHALL 照原需求單的值帶入。
...
#### Scenario: 新增印件項目時印件內部完成日為空白

- **GIVEN** 需求單已有一筆印件項目、其印件內部完成日為 2026-10-20
- **WHEN** 業務新增第二筆印件項目
- **THEN** 該列的印件內部完成日 SHALL 為空白
- **AND** 系統 SHALL NOT 帶入 2026-10-20 或任何預設值
- **AND** 需求單單頭 SHALL NOT 出現交期欄

#### Scenario: 業務逐列手填印件內部完成日

- **GIVEN** 需求單含兩筆印件項目（名片、型錄），印件內部完成日皆為空白
- **WHEN** 業務把名片填為 2026-10-15、型錄填為 2026-10-20
- **THEN** 兩筆各自保存所填日期，互不影響
- **AND** 業務 MAY 只填其中一筆、另一筆維持空白並存檔

#### Scenario: 需求單列顯示推得的印件預計交期

- **GIVEN** 需求單一筆印件項目的印件內部完成日為 2026-10-15（四）
- **WHEN** 業務檢視該列
- **THEN** 印件預計交期 SHALL 顯示 2026-10-16（五）
- **AND** 印件預計交期 SHALL 為唯讀
- **AND** 畫面 SHALL NOT 另外顯示唯讀的印件內部完成日
...
- **AND** 印件內部完成日 SHALL NOT 扣任何急件天數

#### Scenario: 印件內部完成日留空時印件預計交期顯示破折號

- **GIVEN** 需求單一筆印件項目的印件內部完成日為空白
- **WHEN** 業務檢視該列
- **THEN** 印件預計交期 SHALL 顯示「－」

#### Scenario: 複製需求單時印件內部完成日照原值帶入

- **GIVEN** 一張需求單含兩筆印件項目，印件內部完成日分別為 2026-10-15 與空白
- **WHEN** 業務複製該需求單建立新需求單
- **THEN** 新需求單兩筆印件項目的印件內部完成日 SHALL 分別為 2026-10-15 與空白
...
系統 SHALL 支援成交後一鍵轉建訂單，自動帶入需求單的客戶資料、印件規格等基本資料至訂單。交期 SHALL 只在印件一層帶入：需求單各印件項目的「印件內部完成日」逐件帶入對應訂單印件的「未扣急件內部完成日」，**空值即為空、系統 SHALL NOT 以任何值回補**。需求單與訂單皆無單頭交期欄，系統 SHALL NOT 於轉單時建立訂單層交期欄位。急件選項 SHALL NOT 自需求單帶入（需求單無此欄），轉入的印件由業務於訂單編輯時逐件選定，選定後系統重算印件內部完成日與印件預計交期。
...
- **THEN** 系統建立新訂單，自動帶入客戶資料、印件項目，各印件項目的印件內部完成日帶入成為印件的未扣急件內部完成日

#### Scenario: US-QR-007 需求單成交轉訂單
- **WHEN** 需求單「成交」後業務點擊「轉建訂單」
- **THEN** 系統 SHALL 自動帶入客戶、業務、印件基本資料；訂單 MUST 與需求單建立關聯，訂單 MUST 包含需求單的所有必要資訊

#### Scenario: 印件項目的印件內部完成日逐件帶入

- **GIVEN** 需求單兩筆印件項目（名片、型錄）的印件內部完成日分別為 2026-10-15 與 2026-10-20
- **WHEN** 業務執行「轉訂單」
...
- **GIVEN** 需求單一筆印件項目的印件內部完成日為 2026-10-15，需求單無急件選項欄
- **WHEN** 業務執行「轉訂單」
...
#### Scenario: 印件項目印件內部完成日為空時轉入即為空

- **GIVEN** 需求單一筆印件項目的印件內部完成日未填（空值）
- **WHEN** 業務執行「轉訂單」
