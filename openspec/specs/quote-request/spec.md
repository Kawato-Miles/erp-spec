...
需求單（Quote Request）SHALL 依以下狀態流轉：需求確認中 → 待評估成本 → 已評估成本 → 議價中 → 成交 / 流失。其中：
...
| 帳務公司 | billing_company_id | FK | Y | Y | FK -> BillingCompany；對應發票主體（藍新 MerchantID_）；系統依 account_company 硬推導（SSP/KAD → bc-ssp；BRO/EC → bc-bro），業務無法手動指定或覆寫；成交轉訂單時帶入訂單 |
