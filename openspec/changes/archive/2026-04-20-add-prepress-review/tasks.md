...
- [x] 7.1 工作台列表頁：分類「首審」與「補件重審」，各自計數
- [x] 7.2 列表預設排序（PI-006 未定案前用「分配時間」）
- [x] 7.3 列表每列呈現：印件名稱、訂單編號、難易度、分配時間、狀態
- [x] 7.4 工作台詳情頁：左側印件需求規格、原稿檢視
- [x] 7.5 詳情頁：下載原稿按鈕
- [x] 7.6 詳情頁：上傳加工檔 + 縮圖元件（`FileUploadPanel`）
- [x] 7.7 詳情頁：送審操作（合格 / 不合格單選；不合格時 reject_reason_category LOV 下拉必選、review_note 選填補充）
- [x] 7.8 詳情頁右側：ReviewRoundTimeline（歷史輪次）
- [x] 7.9 詳情頁底部：ActivityLogTimeline
- [x] 7.10 送審後返回列表頁，該筆印件從待審列表移除

## 8. 審稿主管工作台（Supervisor）

- [x] 8.1 能力維護面板：表格列出所有審稿人員 + `max_difficulty_level` 欄位可編輯
- [x] 8.2 能力維護：新增 / 停用審稿人員操作
- [x] 8.3 能力變更記錄：呈現近 N 筆變更（時間、操作者、from、to）
- [x] 8.4 負擔儀表板：每位審稿人員進行中審稿數 / 完成數 / 待審數（進行中 = 等待審稿 + 已補件且被分配者）
- [x] 8.5 負擔儀表板：支援依能力值、負擔數排序
- [x] 8.6 覆寫分配入口：選取進行中印件 → 選取目標審稿人員 → 驗證能力 → **必填 reason** → 確認轉指派
- [x] 8.7 覆寫分配：拒絕能力不足者、拒絕未填 reason，並顯示提示
- [x] 8.8 KPI 儀表板（design § Success Metrics）：
  - 自動分配命中率（非覆寫比例）
  - 首審通過率
  - 付款到合格平均時長
  - 補件 loop 平均輪數
...
- [x] 8.9 覆寫待辦清單：列出「原審稿人員不在崗且處於已補件」的印件

## 9. B2C 補件 Mock 介面（非 ERP 範疇，但需 Mock 展示）

- [x] 9.1 Mock 電商前台會員訂單詳情頁（簡化版）
- [x] 9.2 會員上傳補件動作觸發 Mock ERP 介面回寫補件檔案
- [x] 9.3 印件狀態轉「已補件」並驗證待審列表重新出現於原審稿人員

## 10. 情境驗證（End-to-End）

- [x] 10.1 情境 A：B2C 難易度 5 商品 → 付款 → 自動分配（驗證命中規則）
- [x] 10.2 情境 B：B2B 補件 loop 一輪後通過 → 驗證 round_no=2 的 ReviewRound 正確建立、`PrintItem.current_round_id` 正確切換至 round_no=2
- [x] 10.3 情境 C：B2C 多輪補件（3 輪）最終通過 → 驗證 ActivityLog 完整性
- [x] 10.4 情境 D：免審稿印件 → 驗證不入待審列表、不產生 ReviewRound、直接合格
- [x] 10.5 情境 E：主管覆寫分配 → 驗證 ActivityLog 記錄完整（from_user、to_user、reason）
- [x] 10.6 情境 F：原審稿人員不在崗 + 已補件 → 驗證進入主管覆寫清單
- [x] 10.7 情境 G：能力不足邊界 → 依 PI-004 定案行為驗證
