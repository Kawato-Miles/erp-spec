...
10. 業務手動關聯 SalesAllowance.refund_payment_id = 步驟 8 的退款 Payment
11. 訂單詳情頁對帳檢視面板顯示警示 banner「歷史對帳需重新核對 — 訂單已於 2026-03-15 完成，異動於 2026-05-06 執行」
12. 業務確認客戶滿意（收到退款）後點 ticket 上「結案」按鈕
13. ticket.status → 已結案，closed_at = 當下、closed_by = 業務
...
- **AND** 點擊 ticket 可進入詳情頁查看 case_category、responsibility、關聯 OrderAdjustment、補印 PrintItem 完整歷程
...
- 客戶於 2026-05-10 反映輕微印件瑕疵但表示「公司若認賠則接受不處理」
...
系統 SHALL 通過此端到端情境驗證「補印免費（公司認賠）」結局，確認補印 PrintItem 與 ticket 的 FK 關聯運作，且不建 OrderAdjustment。

**情境設定**：
- 訂單 #O-25040301 印製 1,000 份名片，已於 2026-04-15 完成
- 客戶於 2026-05-05 反映 100 份規格不符（紙色與打樣不一致）、公司認賠補印
...
5. 系統 MUST NOT 建立 OrderAdjustment（免費補印）
...
- **GIVEN** AfterSalesTicket.resolution = 補印、responsibility = 公司認賠
...
系統 SHALL 通過此端到端情境驗證「補印收費（客戶承擔）」結局，確認 ticket 加掛 OrderAdjustment + 補印 PrintItem 雙實體並行運作。

**情境設定**：
- 訂單 #O-25040502 印製 500 份手冊，已於 2026-04-25 完成
- 客戶於 2026-05-08 表示「自己提供的檔案有誤，要求補印 100 份並承擔補印費 8,000」

**端到端步驟**：
1. 業務建立 AfterSalesTicket
   - customer_complaint = 「客戶補檔錯誤要求補印 100 份，承擔補印費」
   - case_category = 規格不符
   - responsibility = 客戶承擔
2. 業務於 Slack 討論後填 resolution = 補印 並點「送出決議」，ticket.status → 處理中
3. 系統 SHALL 提示「客戶承擔，請於 ticket 內建立補印費 OrderAdjustment」
4. 業務於 ticket 內點「建立補印費異動單」建 OrderAdjustment
   - adjustment_type = 補退
   - amount = +8,000
   - linked_after_sales_ticket_id = 此 ticket
5. OrderAdjustment 走原狀態機 → 業務主管核可 → 業務執行
6. 業務於 ticket 內建補印 PrintItem(數量=100, related_after_sales_ticket_id=此 ticket)
7. PrintItem 走原審稿 / 工單 / 出貨流程
8. 業務新增 PaymentPlan 一期（8,000）追補印費
9. 客戶付款後業務記錄 Payment(+8,000) + 開立 Invoice #2(+8,000)
10. 業務確認客戶收件滿意 + 收到補印費後點「結案」
...
- **GIVEN** AfterSalesTicket.resolution = 補印、responsibility = 客戶承擔
