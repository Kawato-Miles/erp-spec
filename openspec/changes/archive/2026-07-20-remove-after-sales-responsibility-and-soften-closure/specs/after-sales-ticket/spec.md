...
**Reason**: Miles 2026-07-20 拍板移除。此欄位宣稱驅動「補印是否收費、退款是否由公司吸收」，但系統行為早已與其解耦（不自動帶建補費 OA），形成「宣稱（欄位值）與事實（有無補收異動單）」雙寫打架的設計債。收費事實回歸單一真相：售後單下有無補收異動單。

**Migration**: 補印收費判斷改為行為驅動——要向客戶收費就是業務手動建補收異動單（金額為協商價），公司吸收就只建補印印件不建異動單（見「與 OrderAdjustment 關聯」Requirement）。列表篩選需求由售後類型（case_category）承接；「公司認賠金額」類管理切片由售後單下退款異動單總額推導（dashboard 屬後驗 epic）。Prototype 既有 mock 資料的 responsibility 欄位同批移除，無正式資料遷移。
