...
業務 / 諮詢角色查看印件詳情頁時，僅 SHALL 顯示兩個 Tab：**審稿紀錄 / 活動紀錄**。「工單與生產任務 / 品檢紀錄與缺口處置」SHALL 隱藏，原因為業務 / 諮詢工作流不需直接介入生產層細節，且開放這些 Tab 可能誘發業務跨層介入印務排程（破窗效應）。固定區塊中的成本區塊 SHALL 對業務 / 業務主管 / 會計整塊不顯示（不顯示為空值），其餘固定區塊 SHALL 可見。
