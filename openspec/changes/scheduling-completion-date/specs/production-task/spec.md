...
輸入欄位：母版尺寸（固定清單下拉，選後自動帶入 W×H）、成品寬度（mm）、成品高度（mm）、出血（mm）、咬口（mm）。
...
- **AND** 輸入母版尺寸（如菊全 636×939mm）、成品尺寸（如 210×297mm）、出血（3mm）、咬口（10mm）
- **THEN** 系統即時計算並顯示：拼法 4 模（2×2）、需要印張 750 張（以 3000 份為例）、紙張利用率 82%
- **AND** 印務可依據結果手動填入印刷生產任務的數量欄位（750 張）
