...
- `ConsultationRequestDetail` 顯示活動紀錄區（沿用 `ActivityTimeline` 元件）
...
- 不改 `ConsultationRequestDetail` 版型（不導入 Tab 結構）
...
### D2：版型維持 vertical card stack，最後加「活動紀錄」`ErpDetailCard`

在「付款紀錄」`ErpDetailCard` 之後 push 一張新卡：

```tsx
<ErpDetailCard title="活動紀錄">
  {cr.activityLogs.length === 0 ? (
    <div className="text-sm text-muted-foreground text-center py-4">
      尚無活動紀錄
    </div>
  ) : (
    <ActivityTimeline logs={cr.activityLogs} />
  )}
</ErpDetailCard>
```

**理由**：
- 諮詢單詳情頁目前是 4 張卡（諮詢進度 / 諮詢單資訊 / 諮詢單內容 / 付款紀錄）的 vertical stack，視覺一致
- 不引入 Tab，避免空 Tab 切換成本（諮詢單欄位較少，Tab 化反而增加操作負擔）
- 等諮詢單功能後續增加（諮詢備註編輯區、主管代為認領 dialog 等）視需要再評估 Tab 化
