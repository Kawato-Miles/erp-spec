...
- **業務角色影響**（依 erp-consultant Round 1 P2 修正補強）：
  - **業務**：NCR Disposition = `use_as_is` 時收通知，至訂單異動模組手動發起退款；出貨期決定數量（責任不變）
  - **印務**：規劃工單時設定 PT 屬性（type / scope / requires_inspection / require_transfer / previous_production_task_ids）；NCR Disposition 三選一 + 必填 defect_category
  - **QC / 品檢人員**：派工板顯示三種 type 任務、依 type 切換驗收規格；分批驗收依儀表板自主節奏
  - **生管**：派工 PT 前置條件檢查 AND 邏輯，QC 通過數不足不阻擋下游
...
- `state-machines`：移除 QC 單獨立狀態機段落；QC 與品檢狀態流轉指向 ProductionTask 狀態機；齊套規則 § 完成度計算 引用更新；新增 MODIFIED「跨實體狀態向上傳遞鏈」明示 QC / inspection PT 不走傳遞鏈
- `user-roles`：QC 角色（兼任品檢執行）權責更新（執行 ProductionTask type = `qc` / `inspection`）
- `business-processes`（依 doc-audit Round 1 跨檔稽核補強）：MODIFIED「四層計算精確流程」— 層級 1 公式從 `sum(QCRecord.passed_quantity)` 改為 `sum(ProductionTaskWorkRecord.passed_quantity where type IN ('qc', 'inspection'))`
- `work-order`（依 doc-audit Round 1 跨檔稽核補強）：MODIFIED「QC 單建立」/「QC 執行與結果記錄」— 工單詳情頁 QC 觸發從「建立 QCRecord」改為「派工 QC PT」，工單模組不再經過 QCRecord 中介，UI 顯示透過 `ProductionTask.work_order_id + type = qc` 篩選彙總
