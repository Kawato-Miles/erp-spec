...
- 新增批次審稿：審稿人員可勾選同訂單內多個自己負責的待審印件一次判定（整批同一結果）；批次合格時稿件縮圖一張套用整批、審稿後檔案逐印件分別上傳；批次不合格時退件原因整批共用、審稿備註可逐件覆寫。線上線下皆適用。
...
- **prepress-review**：「印件自動分配機制」改寫為分派執行者雙型（線上自動／線下訂單管理人手動）；「審稿主管覆寫分配」改寫為「改派」（雙角色、候選不過濾）；「審稿階段流程（端到端）」「印件審稿狀態與 Round 同步」納入待分派狀態；新增「Slack 審稿討論串」「待分派清單與分派操作」「批次審稿」Requirement；「審稿主管工作台」調整（覆寫相關與自動分配命中率移除）。
