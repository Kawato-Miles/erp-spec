...
管道面板 SHALL 將印件依工單建立進度分為五個階段欄位：
1. **待建工單**：審稿通過但尚未建立工單的印件
...
印務主管 SHALL 可在管道面板上直接操作：
- 在「待建工單」欄位：點擊「建立工單」建立工單草稿，並指派給印務
- 在其他欄位：點擊卡片導航至對應工單詳情頁

#### Scenario: 印務主管查看待建工單

- **WHEN** 印務主管開啟印件工單管道面板
- **THEN** 系統 SHALL 在「待建工單」欄位顯示所有審稿通過但尚未建立工單的印件
- **AND** 每張卡片 MUST 顯示印件名稱、訂單編號、客戶名稱、交貨日期

#### Scenario: 印務主管在管道面板建立工單並指派

- **WHEN** 印務主管在「待建工單」欄位點擊某印件的「建立工單」按鈕
- **THEN** 系統 SHALL 顯示快速建立表單（選擇工單類型、指派印務）
- **AND** 建立完成後，該印件卡片 SHALL 移至「已指派待填寫」欄位
