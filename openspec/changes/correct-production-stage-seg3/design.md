...
| A2 | `order-management` § 印件印製維度狀態機 未承載「出貨中 → 已送達」與「製作完成 → 出貨中」兩條轉換（收尾判定落在 `shipment` § 送達後的收尾判定），兩份 spec 的印製維度轉換規則因此分散兩處 | 兩份 main spec 的檔頭與交叉引用 | 屬落點邊界問題而非規格錯誤（出貨觸發的轉換寫在出貨規格合理）；要不要把印製維度的全部轉換集中到 `order-management` 是一次跨檔重整，混在段 3 做會擴大 diff |
