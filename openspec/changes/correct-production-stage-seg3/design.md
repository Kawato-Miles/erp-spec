...
### D3 打樣週期歸零以新 Requirement 承載

G15 落點為 `prepress-review`，以 ADDED § 打樣週期數量歸零 承載，不改寫 § 打樣結果業務判定（八十餘行、含四個分支敘述與五個 Scenario）。既有那句「數量帳按打樣週期歸零，公式正本見 wiki 齊套邏輯卡」不錯、只是不足，新 Requirement 補足六欄與週期界線並回指該段，無相斥。
...
| A2 | `order-management` § 印件印製維度狀態機 未承載「出貨中 → 已送達」與「製作完成 → 出貨中」兩條轉換（收尾判定落在 `shipment` § 送達後的收尾判定），兩份 spec 的印製維度轉換規則因此分散兩處 | 兩份 main spec 的檔頭與交叉引用 | 屬落點邊界問題而非規格錯誤（出貨觸發的轉換寫在出貨規格合理）；要不要把印製維度的全部轉換集中到 `order-management` 是一次跨檔重整，混在段 3 做會擴大 diff。**批 7 端到端推演補記**：「製作完成 → 出貨中」在 prototype 亦無觸發者（建出貨單只寫出貨單自己、不推印件），故對還在製作中／製作完成的印件建單後，送達收尾判定接不上（該判定只推「出貨中」的印件）。mock 已把要驗收尾的印件預設在「出貨中」，動線走得完；實作待本案的落點定案後與 spec 一併補 |
