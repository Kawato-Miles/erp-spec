...
系統 SHALL 支援印務在工單已交付或製作中狀態發起異動。印務發起異動時 MUST 選擇「是否需重新審核製程」並填寫異動原因。系統自動通知生管待確認。異動期間工單完成度計算持續運作，不暫停。

印務發起異動的 UX 流程：點擊「異動」→ 進入生產任務編輯介面 → 調整生產任務（新增/作廢/報廢/修改） → 設定「是否需重新審核製程」 → 填寫異動原因 → 送出。送出後工單方進入「異動」狀態。
...
- **AND** 路徑 C（需重新審核）：工單 SHALL 轉為「異動」狀態，生管確認後工單回到「重新確認製程」，印務修改後重新送審；審核通過後，若已有生產任務在製作中，印務交付新任務時工單直接推進為「製作中」
- **AND** 系統 MUST 保留完整異動紀錄

#### Scenario: US-WO-008 確認工單異動（生管）

- **WHEN** 生管收到異動通知，查看異動內容確認無誤後點擊「確認異動」
- **THEN** 若為「不需重新審核」：工單 SHALL 從「異動」狀態返回發起時的原狀態（工單已交付 或 製作中）
- **AND** 若為「需重新審核」：工單 SHALL 從「異動」狀態進入「重新確認製程」
