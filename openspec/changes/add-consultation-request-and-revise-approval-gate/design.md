...
當前 ERP 沒有諮詢前置流程的紀錄。客人若先諮詢再決定下單，整段足跡（諮詢費收款、討論結果、後續是否轉需求單）散落在 surveycake 表單與業務筆記中。

本 change 新增諮詢前置實體 ConsultationRequest，含表單 webhook、雙出口（諮詢訂單 / 轉需求單）、Payment 跨實體轉移、order_type 三值化等資料層支撐。需求單側僅新增 entry point（從 ConsultationRequest 轉入）+ 流失收尾路徑（諮詢來源需求單流失自動建諮詢訂單）。

業務主管 gate 的位置調整由獨立 change `relocate-sales-manager-approval-from-quote-to-order` 處理，本 change 不涉及。

設計參照：
- [state-machines](../../specs/state-machines/spec.md)：需求單 / 訂單上層狀態機
- [business-processes](../../specs/business-processes/spec.md)：需求單轉訂單的欄位帶入規則
- [user-roles](../../specs/user-roles/spec.md)：業務 / 諮詢角色權責
...
### D3：業務主管 gate 由獨立 change 處理

業務主管 gate 的位置調整（從需求單議價前 → 訂單建立後）由獨立 change `relocate-sales-manager-approval-from-quote-to-order` 處理。本 change 不重新定義業務主管職責，也不調整 quote-request / state-machines / user-roles 中業務主管 gate 相關 Requirement。

本 change 對需求單的修訂限於：(1) 新增 `linkedConsultationRequestId` / `requirementNote` 欄位，(2) 新增從 ConsultationRequest 轉入的 entry point，(3) 諮詢來源需求單流失（議價中前）觸發建諮詢訂單收尾流程。
...
| 來源關聯 | consultation_request_id | `linked_consultation_request_id` | 反向關聯，非空時詳情頁顯示「來自諮詢單」連結 |
...
| 諮詢角色權限邊界不清，可能誤改其他需求單 | OQ #3 已解，諮詢角色與業務同等（refactor-order-payment-and-invoice change § 8） |
| 業務主管 gate 位置與本 change 衝突 | 業務主管 gate 由獨立 change `relocate-sales-manager-approval-from-quote-to-order` 處理，本 change 不涉及 |
...
3. **Phase 3：跨單據付款 mapping 驗證**
   ...
   - 情境驗證：諮詢來源需求單流失 → 自動建諮詢訂單 + Payment 轉移 + ExtraCharge 建立
