...
- `ErpSummaryGrid` SHALL 緊接 `ErpPageHeader` 下方，**5 欄**依序：`設備費` / `材料小計` / `工序小計` / `裝訂小計` / `色數加價`
  ...
  - 本頁 SHALL NOT 另列「總計」欄，避免總計掩蓋細項構成；使用者需要總和時由各欄相加
...
#### Scenario: 頁面頂部成本摘要（5 欄分項）

- **WHEN** 生管於新增生產任務頁尚未輸入任何 draft
- **THEN** 頁面頂部 `ErpSummaryGrid` 5 欄 SHALL 全顯示 `—`
- **WHEN** 生管於材料 Tab 新增 2 筆 draft，合計 `unitPrice × qty = NT$ 1,500`
- **THEN** 「材料小計」SHALL 顯示 `NT$ 1,500`；其他 4 欄 SHALL 維持 `—`
- **WHEN** 生管再於工序 Tab 新增 1 筆 `unitPrice × qty = NT$ 800`、選設備（setupFee=NT$ 200）、色數加價 NT$ 50
- **THEN** 「工序小計」SHALL 顯示 `NT$ 800`、「設備費」SHALL 顯示 `NT$ 200`、「色數加價」SHALL 顯示 `NT$ 50`
- **AND** 各欄獨立，使用者若需總和由各欄相加（`2,550`）
