...
系統 SHALL 在印件詳情頁（`/print-items/:printItemId`）以三個獨立 `ErpDetailCard` 區塊呈現印件資料，卡片順序固定為 **印件資訊 → 印件檔案 → 生產資訊**：

1. **印件資訊（合併單據上下文 + 印件屬性）**：以單一卡承載訂單關聯上下文（A 段）與印件實體屬性（B 段），**卡內分為兩個獨立 `ErpInfoTable` 子區塊**，靠獨立邊框 + `space-y-5` 間距視覺區分。
   - **A 段（訂單上下文）**：訂單編號（連結至訂單詳情）、案名、客戶
   - **B 段（印件屬性）**：印件實體屬性，欄位集合對齊**需求單 PrintItem**（`quote-request` spec）+ 訂單階段系統欄位。三頁共用相同欄位集。以 `ErpInfoTable cols={2}` 呈現 11 個欄位：印件編號、印件類型、**印製狀態（Badge）**、**審稿狀態（Badge）**、難易度（Badge）、免審稿快速路徑、訂單來源、出貨方式、**預計產線**、規格備註、包裝備註。狀態以 label/value row 格式呈現（而非裸 Badge 排於卡頂端），每個狀態前都有明確 label。**規格備註與包裝備註因內容較長，SHALL 獨佔整列（`span: 2`）**。當前輪次不在 B 段範圍（審稿輪次資訊由審稿詳情頁的 Stepper 承擔）。生產任務相關欄位（紙張/材質、製程內容、產品名稱等）**不在本卡範圍**，對應到工單或生產任務詳情頁呈現。
2. **印件檔案**：印件檔案區。以 `ErpSummaryGrid cols={3}` 呈現原始印件檔 / 審稿後印件檔 / 縮圖三個並列區塊（重用 `buildArtworkSummaryItems`）。本卡聚焦「最終印製檔案參考」。**無檔案時仍維持三欄位顯示**，cell 內以「—」佔位，不隱藏整組 layout。
3. **生產資訊**：印件層的**工單齊套視角**—— 多張關聯工單彙總為印件整體生產進度（與工單詳情頁生產資訊卡的「生產任務齊套」視角不同）。SHALL 包含訂購數量、交貨日期，以及印件層聚合「累計預計總數 / 累計完成數 / 累計入庫數 / 工單完成」（label 前綴「累計」，傳達跨工單聚合語意）。**預計產線不在本卡範圍**（屬印件屬性，歸印件資訊卡 B 段）。

印件詳情頁 SHALL 顯示全部三個區塊。印件資訊卡與印件檔案卡 SHALL 使用共用元件 `PrintItemSpecCard` / `PrintItemArtworkCard`（吃 ViewModel interface），確保與工單 / 審稿詳情頁語彙一致。`PrintItemSpecCard` SHALL 支援 `title` / `topContent` / `systemItems` props 讓各頁傳入自身的 A 段內容。
...
- **THEN** 頁面 SHALL 依序顯示三張獨立卡：印件資訊 / 印件檔案 / 生產資訊
- **AND** 印件資訊卡 SHALL 分為兩個獨立 `ErpInfoTable` 子區塊：A 段（訂單編號 / 案名 / 客戶）+ B 段（11 欄印件屬性），子區塊間以間距視覺區分
- **AND** 印件資訊卡 MUST NOT 顯示紙張/材質、製程內容、產品名稱等生產任務相關欄位
- **AND** 印件資訊卡的規格備註與包裝備註欄位 SHALL 獨佔整列（`span: 2`），以容納較長內容
- **AND** 生產資訊卡 MUST NOT 顯示預計產線（已歸印件資訊卡 B 段）
