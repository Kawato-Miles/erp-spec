...
1. **印件資訊（合併單據上下文 + 印件屬性）**：以單一卡承載訂單關聯上下文（A 段）與印件實體屬性（B 段），兩段視覺融合於同一 `ErpInfoTable`。
...
- **AND** 印件資訊卡 SHALL 以單一 `ErpInfoTable` 呈現 A 段（訂單編號 / 案名 / 客戶）+ B 段（11 欄印件屬性）共 14 個 label/value row
