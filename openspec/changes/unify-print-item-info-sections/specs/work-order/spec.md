...
2. **製作狀況（D）**：**本工單層**交期與進度（非印件層聚合）。以**單一 `ErpInfoTable cols={2}`** 呈現，欄位順序依工作流程分層排列：**先交期/排程層**（交貨日期、預估完成日含交期預警、排程進度）**後產量/進度層**（目標數量、每份印件需產、生產數量、入庫數量、完成度）。完成度以**百分比文字**呈現，不放進度條。label 使用單工單語境命名（不加「累計」前綴）。日期類與數量類欄位 MUST NOT 穿插排列。
3. **印件資訊（B）**：透過 `wo.printItemId` 查詢對應 `PrintItem`，以與印件詳情頁 / 審稿詳情頁**完全相同**的欄位集呈現。以 `ErpInfoTable cols={2}` 列出 10 欄位：印件編號、印件類型、**印製狀態（Badge）**、**審稿狀態（Badge）**、難易度（Badge）、免審稿快速路徑、訂單來源、出貨方式、規格備註、包裝備註。狀態以 label/value row 格式呈現，每個狀態前都有明確 label。規格備註與包裝備註 SHALL 獨佔整列（`span: 2`）。生產相關欄位（紙張/材質、製程內容、產品名稱）**不在本卡範圍**。
...
- **AND** 製作狀況卡欄位順序 SHALL 為：交貨日期 → 預估完成日 → 排程進度 → 目標數量 → 每份印件需產 → 生產數量 → 入庫數量 → 完成度（日期類與數量類分層，不穿插）
- **AND** 印件資訊卡 SHALL 顯示 PI-001 的 10 欄位，印製狀態與審稿狀態各佔一個 label/value row
