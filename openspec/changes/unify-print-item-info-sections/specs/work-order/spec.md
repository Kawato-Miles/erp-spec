...
1. **工單資訊（A）**：當前工單的系統欄位。SHALL 包含工單編號、工單類型、工單狀態（沿用 STATUS_STEPS 步驟條）、關聯訂單（連結）、負責印務、印務主管、建立日期。MUST NOT 含印件名稱、印件編號、印件規格、印製狀態、審稿狀態等印件屬性（工單的 `printItemName` 欄位可能與實際印件屬性不一致，應以 B 卡派生的真實印件資料為準）。MUST NOT 含「製程說明」（`processDescription` 與印件的製程資訊重複，避免混淆；印件相關製程歸 B 卡或生產任務詳情）。MUST NOT 重複顯示 D 卡的交貨日期與生產/入庫數量。
2. **生產資訊（D）**：工單層的**生產任務齊套視角**—— 工單內的生產任務彙總為工單完成度（與印件詳情頁 D 卡的「工單齊套」視角不同）。以**單一 `ErpInfoTable cols={2}`** 呈現，欄位順序依工作流程分層排列：**先交期/排程層**（交貨日期、預估完成日含交期預警、排程進度）**後產量/進度層**（目標數量、每份印件需產、生產數量、入庫數量、完成度）。完成度以**百分比文字**呈現，不放進度條。label 使用單工單語境命名（不加「累計」前綴）。日期類與數量類欄位 MUST NOT 穿插排列。
3. **印件資訊（B）**：透過 `wo.printItemId` 查詢對應 `PrintItem`，以與印件詳情頁 / 審稿詳情頁**完全相同**的欄位集呈現。以 `ErpInfoTable cols={2}` 列出 11 欄位：印件編號、印件類型、**印製狀態（Badge）**、**審稿狀態（Badge）**、難易度（Badge）、免審稿快速路徑、訂單來源、出貨方式、預計產線、規格備註、包裝備註。狀態以 label/value row 格式呈現，每個狀態前都有明確 label。規格備註與包裝備註 SHALL 獨佔整列（`span: 2`）。生產任務相關欄位（紙張/材質、製程內容、產品名稱）**不在本卡範圍**。
4. **印件檔案（C）**：與印件詳情頁 / 審稿詳情頁相同內容，三區塊 `ErpSummaryGrid` 呈現原始印件檔 / 審稿後印件檔 / 縮圖。**無檔案時仍維持三欄位顯示**，cell 內以「—」佔位，不隱藏整組 layout。
...
- **THEN** 頁面 SHALL 依序顯示四張卡：工單資訊 / 生產資訊 / 印件資訊 / 印件檔案
- **AND** 工單資訊卡 MUST NOT 顯示「印件名稱」欄位（改由 B 卡呈現實際印件屬性）
- **AND** 工單資訊卡 MUST NOT 顯示「製程說明」欄位（避免與印件層製程資訊重複）
- **AND** 生產資訊卡 SHALL 為單一 `ErpInfoTable`，完成度欄位以百分比文字（非進度條）呈現
- **AND** 生產資訊卡欄位順序 SHALL 為：交貨日期 → 預估完成日 → 排程進度 → 目標數量 → 每份印件需產 → 生產數量 → 入庫數量 → 完成度（日期類與數量類分層，不穿插）
- **AND** 印件資訊卡 SHALL 顯示 PI-001 的 11 欄位（含預計產線），印製狀態與審稿狀態各佔一個 label/value row
- **AND** 規格備註與包裝備註欄位 SHALL 獨佔整列（span=2）
- **AND** 印件資訊卡 MUST NOT 顯示紙張/材質、製程內容、產品名稱等生產相關欄位
...
- 四分類資訊卡：工單資訊（A）/ 生產資訊（D）/ 印件資訊（B）/ 印件檔案（C）（詳見「工單詳情頁資訊分組」requirement）
