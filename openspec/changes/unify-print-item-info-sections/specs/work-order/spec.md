...
1. **工單資訊（合併工單系統欄位 + 狀態步驟條 + 印件屬性）**：以單一卡承載當前工單的系統欄位與關聯印件的實體屬性，兩段視覺融合於同一 `ErpInfoTable`。
...
2. **生產資訊**：工單層的**生產任務齊套視角**—— 工單內的生產任務彙總為工單完成度（與印件詳情頁生產資訊卡的「工單齊套」視角不同）。以**單一 `ErpInfoTable cols={2}`** 呈現，欄位順序依工作流程分層排列：**先交期/排程層**（交貨日期、預估完成日含交期預警、排程進度）**後產量/進度層**（目標數量、每份印件需產、生產數量、入庫數量、完成度）。完成度以**百分比文字**呈現，不放進度條。label 使用單工單語境命名（不加「累計」前綴）。日期類與數量類欄位 MUST NOT 穿插排列。
...
- **AND** 工單資訊卡以單一 `ErpInfoTable` 呈現 A 段（工單系統 7 欄位）+ B 段（11 欄印件屬性）共 18 個 label/value row，並於頂端顯示 STATUS_STEPS 步驟條
