...
系統 SHALL 提供 AfterSalesTicket 實體，作為訂單已完成後客訴 / 不良 / 規格不符 / 物流問題 / 工法限制 / 交期延誤等售後事件的承載容器。一張 AfterSalesTicket 屬於單一 Order，記錄業務與業務主管討論後的決議結果、責任歸屬、售後類型分類，並關聯下游動作（OrderAdjustment / PrintItem / SalesAllowance）。
...
| `slack_thread_url` | URL | N | 業務與主管討論的 Slack thread URL，業務手動貼入 |
...
#### Scenario: 業務於已完成訂單建立 AfterSalesTicket

- **GIVEN** Order.status = 已完成、completion_date = 2026-03-15、訂單尚無關聯 AfterSalesTicket
- **WHEN** 業務於 2026-05-06 點擊訂單詳情頁的「建立售後服務單」
- **THEN** 系統 SHALL 開啟 AfterSalesTicket 建單表單
- **AND** 業務必填 customer_complaint、case_category、responsibility
- **AND** 業務可選填 slack_thread_url
- **AND** 系統 SHALL 寫入 case_no（AS-20260506-XX）、opened_at、opened_by
...
業務看板（業務工作首頁）SHALL 新增「我的未結案售後」分桶，列出 `opened_by = self` 且 `status ≠ 已結案` 的 AfterSalesTicket。

桶內 ticket SHALL 顯示：case_no、訂單編號、客戶名稱、case_category、opened_at、待辦提示（依 status 推導：受理中 → 待填 resolution；處理中 → 待結案或待建關聯動作）。

#### Scenario: 業務查看自己未結案售後

- **WHEN** 業務 Alice 登入業務看板
- **THEN** 「我的未結案售後」分桶 SHALL 列出所有 opened_by = Alice 且 status ∈ {受理中, 處理中} 的 ticket
- **AND** 桶內 SHALL 按 opened_at 升序排序（最久未處理優先）
- **AND** 超過 7 天未結案的 ticket SHALL 標紅色 badge

#### Scenario: 業務點擊看板 ticket 跳訂單詳情頁售後 Tab

- **GIVEN** 看板有一張 ticket
- **WHEN** 業務點擊該 ticket 卡片
- **THEN** 系統 SHALL 導向訂單詳情頁的「售後服務」Tab，並自動展開該 ticket
