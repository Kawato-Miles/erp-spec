...
- **刪除按鈕應直接移除**：印件在訂單已確認（回簽）後的任何生命週期變更，業務路徑皆不經「印件層刪除」：
  - 金額減少（部分退款，保留訂單）→ 走「訂單異動單（OrderAdjustment）退印 / 補退」流程（由 [refactor-order-adjustment-and-cleanup change](../refactor-order-adjustment-and-cleanup/proposal.md) 定義）
  - 金額增加（追加印件）→ 走「訂單異動單（OrderAdjustment）加印追加」流程
...
- **金額變更統一走訂單異動單**：訂單異動單（OrderAdjustment）以 adjustment_type 涵蓋金額增減（加印追加 / 退印 / 補退 / 規格變更 / 加運費 等），具體規範由 [refactor-order-adjustment-and-cleanup change](../refactor-order-adjustment-and-cleanup/proposal.md) 定義；僅「客戶要求全然取消訂單」或「業務誤建整筆訂單」才走取消訂單流程。
...
- **新增訂單印件編輯欄位白名單**：
  ...
  - Panel 中金額類欄位 SHALL 以 disabled 呈現，並於該區塊下方顯示引導文字：「金額變更請走：建立訂單異動單（OrderAdjustment）」
- **新增印件變更下游同步規則**：印件白名單欄位異動後，系統 SHALL 同步更新所有引用該印件的下游單據（工單、生產任務、出貨單）顯示值；下游單據 MUST NOT 保留印件欄位的獨立快照
- **新增訂單頂部「變更路徑導引」info icon**：於訂單詳情頁頂部「取消訂單」按鈕旁新增 info icon，hover 時顯示業務變更決策樹：「印件加項 → 建立訂單異動單（加印追加）｜印件減項（部分退款）→ 建立訂單異動單（退印 / 補退）｜規格改動（不影響金額）→ 點印件列編輯按鈕｜全然取消訂單 → 點取消訂單」
- **驗證並修復「檢視（Eye）」按鈕**：
...
- **依賴關係**：
  - 本 change 的「金額變動路徑」引用 [refactor-order-adjustment-and-cleanup change](../refactor-order-adjustment-and-cleanup/proposal.md) 定義的 OrderAdjustment 機制；該 change 須先歸檔以確保訂單異動單概念已 sync 至 main spec
...
- **相關 OQ**：
  ...
  - **既存相關 OQ 狀態更新**：
    - [ORD-001 訂單取消連鎖流程](https://www.notion.so/32c3886511fa81629e1cfe27878e31dd)：原決議「金額減少 → 取消訂單 + 重新走需求單」已被 refactor-order-adjustment-and-cleanup change 修訂為「金額減少 → 訂單異動單退印 / 補退」；ORD-001 在 Notion 應結案註記此後續決議
