...
業務主管核可 MUST 為單向狀態轉換動作（無「退回至待評估成本」按鈕）；若業務主管認為需重新評估成本，可使用「退回討論」非狀態轉換動作（寫入 ActivityLog 記錄理由），需求單仍維持「已評估成本」狀態，由業務看到 ActivityLog 後決定是否走 US-QR-006「重新評估報價」路徑。

業務主管核可動作 MUST 寫入 QuoteRequestActivityLog（事件描述 = 「核可進入議價」）。業務主管「退回討論」動作亦 MUST 寫入 ActivityLog（事件描述 = 「退回討論」、含退回理由 free text）。業務主管「首次查看需求單時間」MUST 由系統自動記錄一次至 ActivityLog（事件描述 = 「業務主管首次查看」），作為 Phase 2 lead time KPI 資料基礎。
...
- **WHEN** 業務主管於需求單詳情頁點擊「退回討論」並填寫退回理由（free text，必填）
- **THEN** 需求單狀態 MUST 維持「已評估成本」（非狀態轉換）
- **AND** 系統 MUST 寫入 ActivityLog（操作者 = 業務主管、事件描述 = 「退回討論」、退回理由 free text）
- **AND** 業務 SHALL 於需求單詳情頁與待辦清單看到此 ActivityLog 內容
- **AND** 業務 SHALL 自行決定是否走 US-QR-006 重新評估路徑
...
### Requirement: 業務主管核可待辦清單

業務主管的「待辦清單」 SHALL 為獨立路由的專屬頁面（`/sales-manager/approvals`），與業務 / Supervisor 使用的需求單列表頁（`/`）分離。此頁面為業務主管登入後的預設首頁。

待辦清單頁面 MUST NOT 重用需求單列表頁的版面結構（包含 4 張流程階段 KPI 統計卡 — 待確認需求 / 待評估成本 / 待報價 / 議價中），因業務主管視角不關注流程前段狀態 count。

待辦清單頁面 SHALL 預設套用篩選 `approved_by_sales_manager_id = self AND status = 已評估成本`，並按「進入已評估成本時間」ASC 排序（最久的優先）。業務主管 SHALL 可手動切換狀態篩選為「議價中」、「成交」、「流失」、或「全部」以查看自己被指定的所有需求單歷史，但篩選 MUST 仍限於業務主管可見的狀態（已評估成本、議價中、成交、流失）。

待辦清單頁面 MUST NOT 提供「新增」與「刪除」按鈕（業務主管無建立 / 刪除權限）。

#### Scenario: 業務主管登入後預設導航至待辦清單頁
...
- **AND** UI SHALL 顯示等待天數欄位（同「業務側等待可見性」的計算邏輯）

#### Scenario: 業務主管切換篩選查看其他狀態
...
#### Scenario: 業務主管不可存取需求單列表頁

- **WHEN** 業務主管嘗試透過 URL 直接 navigate 至 `/`（業務 / Supervisor 的需求單列表頁）
- **THEN** 系統 SHALL 透過路由守衛將其導向 `/sales-manager/approvals`
- **AND** 業務主管 MUST NOT 看到「待確認需求 / 待評估成本」等業務 / PM 流程階段視角的清單

---

### Requirement: 資料可見範圍

需求單模組 SHALL 依角色實施資料可見範圍規則。本 Requirement 不取代 [user-roles spec](../user-roles/spec.md) § 模組存取權限模型 的「R/W vs X」粗粒度設定，而是補充細粒度資料範圍。

| 角色 | 需求單可見範圍 |
|------|--------------|
| 業務 | `created_by = self` |
| 業務主管 | `approved_by_sales_manager_id = self` AND `status ∈ {已評估成本, 議價中, 成交, 流失}` |
| 兼具兩角色（業務 + 業務主管）的使用者 | 上述兩條件取聯集 |
| Supervisor | 所有需求單（既有規則，本 change 不改）|

業務 MUST NOT 看到他人建立的需求單（除既有「分享需求單給同事參考」US-QR-010 / US-QR-011 授權機制涵蓋的範圍）。業務主管 MUST NOT 看到他人指定範圍的需求單，亦 MUST NOT 看到處於「需求確認中」、「待評估成本」、「草稿」狀態的需求單（即印務主管尚未完成評估前，業務主管不需介入）。

#### Scenario: 業務主管不可見草稿狀態需求單

- **GIVEN** 需求單狀態為「草稿」、「需求確認中」或「待評估成本」
- **WHEN** 業務主管登入並查看需求單列表
- **THEN** 系統 MUST NOT 在列表中顯示此需求單
- **AND** 業務主管 MUST NOT 透過直接 URL 存取此需求單詳情頁

#### Scenario: 兼角色使用者可見範圍取聯集

- **GIVEN** 使用者 U 同時擁有業務與業務主管角色
- **WHEN** U 登入並查看需求單列表
- **THEN** 系統 SHALL 顯示符合下列任一條件的需求單：
  - `created_by = U`（U 作為業務建立的需求單）
  - `approved_by_sales_manager_id = U AND status ∈ {已評估成本, 議價中, 成交, 流失}`（U 作為業務主管被指定的需求單）
...
#### Scenario: 業務主管退回討論觸發通知

- **WHEN** 業務主管執行「退回討論」並填寫退回理由
- **THEN** 系統 MUST 自動透過 Slack Webhook 發送通知給需求單建立者（業務）
- **AND** 通知內容 SHALL 包含需求單編號、退回理由摘要、原文連結
