...
業務主管的「待辦清單」 SHALL 定義為：登入中台後的需求單列表頁，預設套用篩選 `approved_by_sales_manager_id = self AND status = 已評估成本`，並按「進入已評估成本時間」ASC 排序（最久的優先）。

待辦清單 MUST NOT 為獨立 Dashboard，而是需求單列表頁的預設 view。業務主管 SHALL 可手動切換篩選為其他狀態（議價中、成交、流失等）以查看自己被指定的所有需求單歷史。

#### Scenario: 業務主管登入後預設看到待辦清單

- **WHEN** 業務主管登入中台並開啟需求單模組
- **THEN** 系統 SHALL 預設套用篩選 `approved_by_sales_manager_id = self AND status = 已評估成本`
