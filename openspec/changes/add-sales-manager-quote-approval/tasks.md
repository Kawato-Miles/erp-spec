...
## 6. 業務主管待辦清單（獨立路由 + 獨立頁面）

- [x] 6.1 新增獨立路由 `/sales-manager/approvals` + 對應頁面 `SalesManagerApprovalListPage`，業務主管登入後預設導航至此頁
- [x] 6.2 待辦清單預設篩選 `approved_by_sales_manager_id = self AND status = 已評估成本`，按「進入已評估成本時間」ASC 排序
- [x] 6.3 顯示「等待天數」欄位
- [x] 6.4 可手動切換狀態為「議價中」、「成交」、「流失」、「全部」（限業務主管可見的 4 個狀態），仍限制 `approved_by_sales_manager_id = self`
- [x] 6.5 待辦清單頁面 MUST NOT 含 4 張流程階段 KPI 統計卡（待確認需求 / 待評估成本 / 待報價 / 議價中）— 業務主管視角不關注前段流程 count
- [x] 6.6 待辦清單頁面 MUST NOT 提供「新增」與「刪除」按鈕（業務主管無建立 / 刪除權限）
- [x] 6.7 業務主管側選單僅顯示「審核管理 → 需求單核可」單一入口，不再顯示「需求單管理」
- [x] 6.8 路由守衛：業務主管嘗試 navigate 至 `/`（需求單列表頁）時，自動導向 `/sales-manager/approvals`
