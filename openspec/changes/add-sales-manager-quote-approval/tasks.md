...
- ~~4.5 業務主管於需求單詳情頁看到「退回討論」按鈕~~（Miles 2026-04-27 修正：移除 ERP 內退回討論機制，討論透過 Slack thread 進行）
- ~~4.6 「退回討論」提交後狀態維持「已評估成本」~~（同上，移除）
...
- ~~5.4 業務主管已執行過「退回討論」時，業務詳情頁顯示最新一筆退回理由與時間戳~~（Miles 2026-04-27 修正：移除 ERP 內退回討論機制，業務從 Slack thread 自行查看溝通內容）

## 6. 業務主管側選單與兩個頁面（核可頁 + 列表頁）

- [x] 6.1 新增獨立路由 `/sales-manager/approvals` + 對應頁面 `SalesManagerApprovalListPage`，業務主管登入後預設導航至此頁
- [x] 6.2 核可頁預設篩選 `approved_by_sales_manager_id = self AND status = 已評估成本`，按「進入已評估成本時間」ASC 排序
- [x] 6.3 核可頁顯示「等待天數」欄位
- [x] 6.4 核可頁可手動切換狀態為「議價中」、「成交」、「流失」、「全部」（限業務主管可見的 4 個狀態），仍限制 `approved_by_sales_manager_id = self`
- [x] 6.5 核可頁 MUST NOT 含 4 張流程階段 KPI 統計卡（待確認需求 / 待評估成本 / 待報價 / 議價中）
- [x] 6.6 核可頁 MUST NOT 提供「新增」與「刪除」按鈕
- [x] 6.7 業務主管側選單顯示「需求單管理」單一 group，含兩個 sub item：「需求單核可」（`/sales-manager/approvals`）+「需求單列表」（`/`）（Miles 2026-04-27 修正：原為「審核管理」單一入口，改為與業務同 group label 但 sub items 不同）
- [x] 6.8 路由守衛：業務主管 ROLE_ALLOWED_PREFIXES 含 `/`、`/sales-manager`、`/quote`，可存取兩個頁面
- [x] 6.9 業務主管於「需求單列表頁」（`/`）可看全部需求單（不限自己被指定的，提供部門總覽能力）；對非自己被指定的需求單詳情頁 MUST NOT 顯示「核可進入議價」按鈕
- [x] 6.10 業務（業務平台）側選單僅顯示「需求單管理 → 需求單列表」單一 group + sub item（拿掉訂單管理；Miles 2026-04-27 修正）
...
- ~~9.3 新增「業務主管退回討論」時透過 Slack Webhook 通知需求單建立者（業務）~~（Miles 2026-04-27 修正：移除 ERP 內退回討論機制；業務主管 / 業務之間的討論透過既有 Slack thread 進行，不需 ERP 端額外通知）
...
- ~~10.3 業務主管退回討論動作寫入 ActivityLog~~（Miles 2026-04-27 修正：移除）
