...
**Reason**: 補印印件型別於本 change 廢除（售後與補做統一案，grill § 六條 25 拍板）。本 Requirement 的整套機制——業務於售後服務單內建立補印印件、自動寫入印件類型「補印印件」與「來源售後服務單」、複製來源印件的稿件檔案與審稿輪次、補一筆「售後補印自動通過輪次」（審稿輪次來源值域新增「售後補印」）、印件狀態直接設為待生產——全部不再成立。具名否決理由（設計正本 § 業界範式對照）：審稿輪次複製徒增一輪形式上的自動合格；印件數隨客訴膨脹；品質成本掛在一件 0 元的新印件上而看不出這批貨實際花多少；訂單完成判定必須為補印印件另設分母排除條款。對應的商業正本已改寫：wiki [售後服務規則](../../../../../memory/Sens_wiki/wiki/erp/04-business-logic/營運規則/訂單到交付/售後服務規則.md) § 補做的執行規則（只補做、不另建印件）、[印件](../../../../../memory/Sens_wiki/wiki/erp/05-entities/印件.md)（印件類型值域三值改二值、「來源售後服務單」欄廢除）、[審稿輪次](../../../../../memory/Sens_wiki/wiki/erp/05-entities/審稿輪次.md)（來源值域刪「售後補印」、自動合格輪次機制廢除）。

**Migration**: 補做的觸發、發起、承載與防重歸本 change 新增的 § 售後服務單決議觸發補做（印務於印件層發起、原工單經異動加開補做生產任務、補做來源掛售後服務單、同一張單只能承接一次）；兩個觸發來源的機制與數量帳分界歸 `qc` § 補做的兩個觸發來源；穿越印件印製終態的工單重開放行歸 `work-order` § 補生產承接；補做量的出貨歸 `shipment` § 出貨單建立與額度檢核（額度單軌）。原「補印同時改稿」的例外情境（原條文列為不適用、留 OQ）由「改規格一律走新訂單」承接，見 § 售後服務階段流程。審稿側零遷移——不建新印件即無新審稿輪次，原印件的稿件與審稿輪次原封不動。
