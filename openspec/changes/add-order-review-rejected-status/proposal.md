...
4. 否則（存在「等待審稿」或「已補件」印件，**且至少一件已送審過**）→ Order.status = **等待審稿**
5. 否則（例如混合「免審稿合格 + 需審稿未上傳」但尚未送審任何一件）→ Order.status = **稿件未上傳**

**規則 4 的「至少一件已送審過」前提**：避免 EC 混合訂單（免審稿印件直達「合格」+ 需審稿印件「稿件未上傳」）被誤派為「等待審稿」—— 此時還沒任何件被送審，業務動線上屬於「等待客戶上傳稿件」而非「等待審稿人員處理」。

**觸發時機**：
- `submitReviewForPrintItem`（印件送審完成）
- `onResupplyCompleted`（補件完成）
- `confirmSignBack`（訂單回簽後自動分配）
- `uploadArtworkFile`（稿件首次上傳可能觸發 稿件未上傳 → 等待審稿）
- **追加印件走新訂單**：若業務需在已進製作段的訂單追加印件，實務是**開新訂單 + 新印件設為免審稿**（對齊現場做法）；原訂單不觸發 bubble-up。此行為由業務流程承載，非系統強制
...
- `scenarioCoverage.test.ts` 或對應斷言補：
  ...
  - **情境 D（S5）**：打樣 NG 棄用 → 業務開新訂單 + 新印件設為免審稿 → 新訂單直達「製作等待中」，原訂單 status 不受影響
  - **情境 E（S10）**：訂單混合「免審稿合格印件 + 需審稿未上傳印件」→ Order 應為「稿件未上傳」直到首件送審
...
- **X6**：原訂單於製作段後追加印件 — 業務實務走**新訂單 + 新印件設為免審稿** 模式，本 change 不支援原訂單追加的回推邏輯
