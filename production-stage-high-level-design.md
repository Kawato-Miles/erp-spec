...
| 部件配方 | 重構為三層（2026-07-30 拍板）：粒度由「整件商品」改為「單一部件」、可跨印件配方重用；上加 [[印件配方]] 承載部件組成與每組數量，配方工序段補回裝訂欄。展開與沉澱規則正本歸 [[配方展開規則]] |
...
| M6 | 配方管理 | 印件配方與部件配方的維護：印件配方的部件組成與每組數量、部件配方的工序段序列；供 EC 商品設定與線下單引用展開 | 印務 |
| — | BOM 三類主檔／設備／尺寸範本／產線標籤 | 沿用現況主檔模組，僅欄位微調（放損兩成分、能力標籤、裝訂廠商欄等既有拍板） | 印務 |
...
| 功能 | 說明 |
|------|------|
| 印件配方維護 | 印件配方的部件組成：每行指定部件配方、每組數量、是否獨立開工單；印件層工序段記「湊齊後才做」的工序（整組裝盒）|
| 部件配方維護 | 一個部件的工序段序列（材料／工序／裝訂BOM 三類的項目、能力需求、用量倍率、相依、是否計入完成度）；一份配方可被多個印件配方引用 |
| 配方改版 | 同部件再次沉澱＝版號＋1、舊版轉停用、不回溯已展開工單；印務直接改版、不另設審核 |
| 引用展開 | 印件選印件配方 → 依部件清單與合併旗標一次建好所有工單草稿；引用時可增刪部件、改數量再確認（不回寫印件配方）|
| 工單沉澱為配方 | 一張工單存成一份部件配方（指定屬哪個印件配方的哪個部件，印件配方可順手建）；只存可重用骨幹，不存數量／日期／機台／色數／成本 |

> 規則正本歸 wiki [[配方展開規則]]，欄位正本歸 [[印件配方]] 與 [[部件配方]]。

### 1.3 與既有 openspec spec 的清整對照

| 既有 spec | 處置 | 去向 |
|-----------|------|------|
| work-order | 重寫 | M1（1084→458 行）；兩項待決均已處置——(1) BOM 行項目兩個 Requirement 依 [[PT-037-BOM行項目與配方層兩套展開來源並存]] 採 A 案廢止，印件料工彙總改由印件詳情內頁跨工單承接 (2)「線上單審稿通過自動展開」的 Scenario 保留，spec 另註明該路徑觸發點在 EC 端（下單與審稿通過）、生產階段介面不提供人工入口 |
| production-task | 重寫 | 任務結構歸 M1、報工歸 M2；兩處既存不同步已同步——生產任務「工廠類別由 BOM 行項目決定」改為材料／工序／裝訂三類主檔各自的承作廠商（2026-07-30 拍板）、產線的帶入來源改為配方工序段或印務手選（見 [[PT-037-BOM行項目與配方層兩套展開來源並存]]）|
...
| material-master／process-master／binding-master | 沿用微調 | BOM 三類主檔（放損兩成分等既有拍板落點） |
| （無）出貨 | 新建 | M4（解 SHP-006） |
| （無）派單 | 新建 | M5 |
| （無）配方管理 | 已新建 | M6：print-item-recipe／component-recipe／recipe-expansion 三個 capability 已於 2026-07-30 併入 main specs（change recipe-management 已封存）；Prototype 四頁已落 `prototype/production-stage` |
...
主檔（被引用，不入圖）：材料主檔／工序主檔／裝訂主檔（BOM 三類）、設備、尺寸範本、產線標籤（tag 模組）。

### 2.2 實體清單與本輪異動

| 實體 | 本輪處置 | 說明（欄位正本在 wiki 卡，此處只記異動方向） |
|------|---------|------------------------------------------|
| 工單 | 沿用＋修 | 沿用現況卡；「區域（台灣／中國）」欄已自欄位表移除（不拆廠，內外差異下沉至生產任務的生產單位類別）；工單類型維持打樣與大貨二值，中國線六值依 [[PT-032-工單類型與委外製作情境六值落層]] 判定為 [[派單]] 自身的「委外製作情境」欄位、與工單類型無關 |
...
| 印件配方 | 新增 | 印件配方主表＋部件清單（部件配方／每組數量／是否獨立開工單）＋印件層工序段；沉澱時可順手建立，不要求先維護字典 |
| 部件配方 | 重構 | 粒度由整件商品改為單一部件、可跨印件配方重用；工序段補裝訂欄；印件配方指向現行生效版、改版不回溯已展開工單 |
...
| 成本達成率 | （實際成本 − 預估成本）÷ 預估成本（正號＝超支；B7 拍板後更新） | 預估成本分項；實際成本彙總 |
...
- **估算參考沉澱**：M3 提供「類似品查詢」——依印件配方／部件配方／工序組合查歷史工單的預估 vs 實際（含投入 vs 合格數的耗損訊號），供印務下次製程規劃與數量拿捏參考。
...
| M6 | 印件配方清單＋詳情 | 篩選＋表格；詳情含部件清單（部件配方／每組數量／是否獨立開工單）與印件層工序段 |
| M6 | 部件配方清單＋詳情 | 篩選＋表格（版本、狀態、被幾個印件配方引用）；詳情含工序段序列與BOM 三類的項目 |
| M6 | 印件頁「選印件配方展開工單草稿」 | 選印件配方 → 預覽部件組合（可增刪部件、改數量）→ 確認後一次建好多張工單草稿 |
| M6 | 工單詳情「存成部件配方」 | 指定屬哪個印件配方的哪個部件（印件配方可順手建）→ 預覽將存入的骨幹 → 確認（同部件已有生效配方則改版）|

M1 工單詳情的製程規劃 Tab 已含生產任務三類的手動增刪修（2026-07-30 落地：BOM 三類主檔選擇鏈、放損率主檔帶入、色數依設備色數模式、預估四分項概算）。
