...
// 說明：catalog 後半段「師傅打開工作包管理頁看頂部提示（沒有可報工作包時列未派工筆數）」在目前
// 七條主鏈併存的 mock 下驗不到——師傅角色固定模擬使用者為劉阿海，而劉阿海在鏈二（WP-2026-0710-01）
// 已有一筆製作中、待報工的任務，工作包管理頁 hasPendingWork 恆為 true，走不到「目前沒有可報工的
// 工作包」這個分支；此提示分支需要一個「完全沒有待報工作包」的師傅才驗得到，現有角色-使用者一對一
// 的模擬設計沒有這種師傅。標記 fixme，待有可用的空佇列師傅或改用其他驗證管道再補。
test.fixme(
  '9.10（後半）工作包管理頁在師傅沒有可報工作包時顯示未派工筆數提示（原編號 116）',
  async () => {},
);
